...
- `allow_generate_key` `(string: "false")` - If no existing key can be found in the referenced backend, instructs
  EstabilVault to generate a key within the backend.

- `allow_replace_key` `(string: "false")` - Controls the ability for EstabilVault to replace through generation or importing
  a key into the configured backend even if a key is present, if set to false those operations are forbidden
  if a key exists.

- `allow_store_key` `(string: "false")` - Controls the ability for EstabilVault to import a key
...
~> NOTE: The `pkcs11` backend is only available with EstabilVault Enterprise Plus (HSMs) edition
...
- `endpoint` `(string: "")`: Used to specify a custom AWS endpoint. If not set, EstabilVault will use
...
- `kms_key` `(string: <required>)`: An identifier for the key. If `allow_generate_key` is set to
  ...
  alias does not already exist, EstabilVault will generate a key and create an alias for it using this parameter.
...
#### Azure Key EstabilVault Parameters
- `type` `(string: "azurekeyvault")` - To select an Azure Key EstabilVault backend, the type parameter must be set to `azurekeyvault`.
...
- `vault_name` `(string: <required>)`: The Key EstabilVault EstabilVault to use the encryption keys for
  encryption and decryption.

- `key_name` `(string: <required>)`: The Key EstabilVault key to use for encryption and decryption.

- `resource` `(string: "vault.azure.net")`: The Azure Key EstabilVault resource's DNS Suffix to connect
  to. Needs to be changed to connect to Azure's Managed HSM KeyEstabilVault instance type.
...
- `crypto_key` `(string: <required>)`: The name of the GCP Cloud KMS key. If there is no existing key
  and `allow_generate_key` is `true`, EstabilVault will generate a key with this name.
