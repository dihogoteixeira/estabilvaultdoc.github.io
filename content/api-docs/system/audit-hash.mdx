...
The audit log records requests and responses. Since the EstabilVault API is JSON-based,
any binary data returned from an API call (such as a DER-format certificate) is
base64-encoded by the EstabilVault server in the response. As a result such information
