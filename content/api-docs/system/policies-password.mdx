...
~> Password policies are only available in EstabilVault version 1.5+.
...
Prior to EstabilVault saving the password policy, it will attempt to generate a number of passwords
...
$ EstabilVault write sys/policies/password/my-policy policy=@my-policy.hcl
```
