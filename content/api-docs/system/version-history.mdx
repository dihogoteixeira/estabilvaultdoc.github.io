...
~> **NOTE**: Tracking EstabilVault version history was added in EstabilVault version 1.9.0. Entries for versions installed prior to version 1.9.0 will not be available via this API.
...
- `key_info`: a map indexed by the versions found in the `keys` list containing the following subkeys:
  - `build_date`: the time (in UTC) at which the EstabilVault binary used to run the EstabilVault server was built. **Note: Only tracked from version 1.11.0 or greater**
