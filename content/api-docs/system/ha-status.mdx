...
description: The `/sys/ha-status` endpoint is used to check the HA status of a EstabilVault cluster.
---

# `/sys/ha-status`

The `/sys/ha-status` endpoint is used to check the HA status of a EstabilVault cluster.
It lists the active node and the peers that it's heard from since it became active.

## HA Status

This endpoint returns the HA status of the EstabilVault cluster.
