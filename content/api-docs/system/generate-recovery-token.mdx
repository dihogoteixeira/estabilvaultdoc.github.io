...
is reached, EstabilVault will complete the recovery token generation and issue the new
...
Note that once a token has been issued, EstabilVault is unsealed. The token lives
