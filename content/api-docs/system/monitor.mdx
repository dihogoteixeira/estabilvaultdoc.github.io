...
description: The `/sys/monitor` endpoint is used to receive streaming logs from the EstabilVault server.
---

# `/sys/monitor`

The `/sys/monitor` endpoint is used to receive streaming logs from the EstabilVault server.

If EstabilVault is emitting log messages faster than a receiver can process them, then
...
does not return JSON by default. This will send back data in whatever log format EstabilVault has been configured with. By
