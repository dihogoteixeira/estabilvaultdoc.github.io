...
- `backup` `(bool: false)` – Specifies if using PGP-encrypted keys, whether
  EstabilVault should also store a plaintext backup of the PGP-encrypted keys at
...
EstabilVault will complete the rekey by performing the actual rotation of the recovery
