...
- `standbyok` `(bool: false)` – Specifies if being a standby should still return
  ...
  EstabilVault is behind a non-configurable load balancer that just wants a 200-level
...
- `perfstandbyok` `(bool: false)` – Specifies if being a performance standby should
  ...
  This is useful when EstabilVault is behind a non-configurable load balancer that just wants
