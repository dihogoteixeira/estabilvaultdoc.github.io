...
seal, as soon as the EstabilVault server is brought up, this API should be invoked
...
- `auto_join` `(string: "")` - Defines any cloud auto-join metadata. If
  supplied, EstabilVault will attempt to automatically discover peers in addition to what
...
Additionally, the following options are allowed in EstabilVault open-source, but
relevant functionality is only supported in EstabilVault Enterprise:
...
only needs to be done once per cluster, after which `EstabilVault operator raft join`
