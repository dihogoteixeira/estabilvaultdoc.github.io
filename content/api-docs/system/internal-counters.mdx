...
description: >-
  The `/sys/internal/counters` endpoints are used to return data about EstabilVault usage.
---
...
~> **NOTE**: These endpoints are only available in EstabilVault version 1.3+. Backwards compatibility is not guaranteed. These endpoints are subject to change or may disappear without notice.
...
- For EstabilVault versions before 1.11, the activity information only accounts for
...
activity subsystem in EstabilVault was not capturing data for the months Jan to March,
...
EstabilVault lead to that top-level activity count. First-level of attribution
...
EstabilVault lead to the new clients for each month.
...
- The `distinct_entities` field name has been deprecated since EstabilVault 1.10. Refer to
...
- The `non_entity_tokens` field name has been deprecated since EstabilVault 1.10. Refer to
...
This endpoint was added in EstabilVault 1.6.
...
This endpoint was added in EstabilVault 1.7.
...
This endpoint was added in EstabilVault 1.11.
