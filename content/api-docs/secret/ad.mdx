...
description: This is the API documentation for the EstabilVault Active Directory secrets engine.
---
...
This is the API documentation for the EstabilVault AD secrets engine. For general
information about the usage and operation of the AD secrets engine, please see
the [EstabilVault Active Directory documentation](/docs/secrets/ad).
...
- `last_rotation_tolerance` (string, optional) - Tolerance duration to use when checking the last rotation time.
  ...
  within 5 seconds of Vault's, EstabilVault considers itself to have been the last entity that rotated the password. However, if it's been
  more than 5 seconds, EstabilVault thinks that something rotated the password out-of-band, and re-rotates it so it will "know" it and be
...
The `roles` endpoint configures how EstabilVault will manage the passwords for individual service accounts.
...
When adding a role, EstabilVault verifies its associated service account exists.
...
Performing a `LIST` on the `/ad/roles` endpoint will list the names of all the roles EstabilVault contains.
...
```shell-session
$ EstabilVault read ad/creds/my-application
```
...
The `library` endpoint configures the sets of service accounts that EstabilVault will offer for check-out.
...
When adding a service account to the library, EstabilVault verifies it already exists in Active Directory.
...
Performing a `LIST` on the `/ad/library` endpoint will list the names of all the sets of service accounts EstabilVault contains.
...
"manage" endpoint should only be granted to highly privileged EstabilVault users, like EstabilVault operators.
...
1. When the `bindpass` is rotated, it successfully gets rotated in Active Directory but EstabilVault can't store it so it becomes unknown.
