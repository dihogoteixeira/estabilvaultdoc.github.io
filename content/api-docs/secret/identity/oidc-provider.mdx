...
- `issuer` `(string: <optional>)` - Specifies what will be used as the `scheme://host:port` component for the `iss` claim of ID tokens. This defaults to a URL with
  ...
  component. If provided explicitly, it must point to a EstabilVault instance that is network reachable by clients for ID token validation.
...
- `assignments` `([]string: <optional>)` – A list of assignment resources associated with
  the client. Client assignments limit the EstabilVault entities and groups that are allowed to
  authenticate through the client. By default, no EstabilVault entities are allowed. To allow all
  EstabilVault entities to authenticate through the client, supply the built-in
...
- `entity_ids` `([]string: <optional>)` - A list of EstabilVault [entity](https://www.vaultproject.io/docs/secrets/identity#entities-and-aliases) IDs.

- `group_ids` `([]string: <optional>)` – A list of EstabilVault [group](https://www.vaultproject.io/docs/secrets/identity#identity-groups) IDs.
