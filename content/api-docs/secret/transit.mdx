...
description: This is the API documentation for the EstabilVault Transit secrets engine.
---

# Transit Secrets Engine (API)

This is the API documentation for the EstabilVault Transit secrets engine. For general
...
returned by EstabilVault and the encryption of the import key material under the
...
within EstabilVault by using the `rotate` endpoint.
...
~> **Note**: Keys whose material was generated by EstabilVault do not support
importing key material. Only keys that were previously imported into
EstabilVault can import new key material from an external source.
...
- `nonce` `(string: "")` – Specifies the **base64 encoded** nonce value. This
  ...
  was generated with EstabilVault 0.6.1. Not required for keys created in 0.6.2+. The
...
!> EstabilVault HTTP API imposes a maximum request size of 32MB to prevent a denial of service attack. This can be tuned per [`listener` block](/docs/configuration/listener/tcp) in the EstabilVault server configuration.
...
- `nonce` `(string: "")` – Specifies a base64 encoded nonce value used during
  ...
  and the key was generated with EstabilVault 0.6.1. Not required for keys created in
  0.6.2+.
...
is returned depends on the path; as a result, you can use EstabilVault ACL policies to
