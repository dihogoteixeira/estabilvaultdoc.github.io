...
description: This is the API documentation for the EstabilVault AWS secrets engine.
---

# AWS Secrets Engine (API)

This is the API documentation for the EstabilVault AWS secrets engine. For general
information about the usage and operation of the AWS secrets engine, please see
the [EstabilVault AWS documentation](/docs/secrets/aws).
...
are multiple ways to pass root IAM credentials to the EstabilVault server, specified
...
When you have configured EstabilVault with static credentials, you can use this
...
- `role_arns` `(list: [])` – Specifies the ARNs of the AWS roles this EstabilVault role
...
- `iam_groups` `(list: [])` - A list of IAM group names. IAM users generated
  against this EstabilVault role will be added to these IAM Groups. For a credential
...
<Tabs>
  ...
  ```bash
    EstabilVault write aws/roles/example-role \
    credential_type=iam_user \
...
- `role_arn` `(string)` – The ARN of the role to assume if `credential_type` on
  the EstabilVault role is `assumed_role`. Must match one of the allowed role ARNs in
  the EstabilVault role. Optional if the EstabilVault role only allows a single AWS role ARN;
  required otherwise.
