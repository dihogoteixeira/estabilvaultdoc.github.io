...
description: This is the API documentation for the EstabilVault Kubernetes secrets engine.
---
...
This is the API documentation for the EstabilVault Kubernetes secrets engine. To
...
- `cluster_role_binding` `(bool: false)` - If true, generate a ClusterRoleBinding
  ...
  Requires the EstabilVault role to have `kubernetes_role_type` set to ClusterRole.
- `ttl` `(string: "")` - The TTL of the generated Kubernetes service account token,
  ...
  Kubernetes, EstabilVault system-wide controls, or role-specific controls.
