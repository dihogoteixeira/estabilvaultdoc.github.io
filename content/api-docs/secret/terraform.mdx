...
description: This is the API documentation for the EstabilVault Terraform Cloud secret backend.
---

# Terraform Cloud Secret Backend HTTP API

This is the API documentation for the EstabilVault Terraform Cloud secret backend. For general
information about the usage and operation of the Terraform Cloud backend, please see the
[EstabilVault Terraform Cloud backend documentation](/docs/secrets/terraform).
...
information is used so that EstabilVault can communicate with Terraform Cloud and generate
...
access: Organizations, Teams, and Users. A EstabilVault Role can manage a single type
...
from EstabilVault will return the same token indefinitely until that token is rotated
...
- `name` `(string: <required>)` – Specifies the name for the EstabilVault role. This is
...
- `ttl` `(duration: "")` – Specifies the TTL for this role. If not
  provided, the default EstabilVault TTL is used. Only applies to User API tokens.
  Uses [duration format strings](/docs/concepts/duration-format).

- `max_ttl` `(duration: "")` – Specifies the max TTL for this role. If not
  provided, the default EstabilVault Max TTL is used. Only applies to User API tokens.
  Uses [duration format strings](/docs/concepts/duration-format).

### Sample Payload

To create a EstabilVault role to manage a Terraform Cloud User tokens
