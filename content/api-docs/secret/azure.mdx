...
description: This is the API documentation for the EstabilVault Azure secrets engine.
---

# Azure Secrets Engine (API)

This is the API documentation for the EstabilVault Azure
...
- `environment` (`string:""`) - The Azure environment. This value can also be provided with the AZURE_ENVIRONMENT
  environment variable. If not specified, EstabilVault will use Azure Public Cloud.
...
```shell-session
$ EstabilVault read azure/config
```
...
```shell-session
$ EstabilVault delete azure/config
```
...
Create or update a EstabilVault role. Either `application_object_id` or
