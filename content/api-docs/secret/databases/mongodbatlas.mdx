...
Statements are configured during EstabilVault role creation and are used by the plugin to
...
- `creation_statements` `(string: <required>)` – Specifies the database
  ...
  must also contain a `roles` array, and from EstabilVault version 1.6.0 (plugin
