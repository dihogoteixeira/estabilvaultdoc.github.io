...
- `username` `(string)` – Specifies the IAM access_key_id for EstabilVault to use. If omitted, authentication fallbacks on the AWS credentials provider chain and tries to infer authentication from the environment.
