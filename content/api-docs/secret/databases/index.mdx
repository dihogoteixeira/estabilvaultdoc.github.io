...
This is the API documentation for the EstabilVault Database secrets engine. For
general information about the usage and operation of the database secrets engine,
please see the
[EstabilVault database secrets engine documentation](/docs/secrets/databases).
...
- `password` `(string)` - Specifies the password to use when connecting with the
  `username`. This value will not be returned by EstabilVault when performing a read upon the
...
```shell-session
$ EstabilVault write database/config/mysql \
    plugin_name="mysql-database-plugin" \
    allowed_roles="readonly" \
    connection_url="{{username}}:{{password}}@tcp(127.0.0.1:3306)/" \
    username="vaultuser" \
...
```shell-session
$ EstabilVault write database/config/mssql \
...
1-to-1 mapping of a EstabilVault Role to a user in a database which are automatically
...
~> EstabilVault will rotate the password when creating a static role. EstabilVault must do
...
- `username` `(string: <required>)` – Specifies the database username that this
  EstabilVault role corresponds to.

- `rotation_period` `(string/int: <required>)` – Specifies the amount of time
  EstabilVault should wait before rotating the password. The minimum is 5 seconds.
...
role name. While Static Roles are rotated automatically by EstabilVault at configured
