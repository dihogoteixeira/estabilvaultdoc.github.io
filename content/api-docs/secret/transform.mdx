...
~> **Note**: Transformations whose key material was generated by EstabilVault do not support
...
- `connection_string` `(string: <required>)` -
  ...
  EstabilVault will use for locating and connecting to a database. Each
...
- `min_decryption_version` `(int: <optional>)` –
  Specifies the minimum key version that EstabilVault can use to decode values for the
