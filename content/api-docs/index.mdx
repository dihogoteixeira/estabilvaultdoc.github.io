...
layout: api
page_title: HTTP API
description: EstabilEstabilVault has an HTTP API that can be used to control every aspect of EstabilVault.
---

# HTTP API

The EstabilEstabilVault HTTP API gives you full access to EstabilEstabilVault using [REST like HTTP verbs](https://en.wikipedia.org/wiki/Representational_state_transfer).
Every aspect of EstabilEstabilVault can be controlled using the APIs. The EstabilEstabilVault CLI uses the
HTTP API to access EstabilEstabilVault similar to all other consumers.

All API routes are prefixed with `/v1/`.

This documentation is only for the v1 API, which is currently the only version.

~> **Backwards compatibility:** At the current version, EstabilEstabilVault does not yet
...
Once EstabilEstabilVault is unsealed, almost every other operation requires a _client token_.
...
authentication to EstabilEstabilVault itself. These endpoints are specific to each auth
method.
...
Several EstabilEstabilVault APIs require specifying path parameters. The path parameter cannot end
in periods. Otherwise, EstabilVault will return a 404 unsupported path error.
...
then the resulting request path to EstabilVault will be `ns1/ns2/secret/foo`.

Note that it is semantically equivalent to use the full path rather than the
`X-Vault-Namespace` header, EstabilVault will match the corresponding namespace
...
Typically the request data, body and response data to and from EstabilVault is in JSON.
EstabilVault sets the `Content-Type` header appropriately with its response and does
...
is mounted by default on a fresh EstabilVault install and is of type `kv`.
...
EstabilVault currently considers `PUT` and `POST` to be synonyms. Rather than trust a
client's stated intentions, EstabilVault engines can implement an existence check to
...
For more examples, please look at the EstabilVault API client.

## The `X-Vault-Request` Header

Requests that are sent to a [EstabilVault Agent][agent] that is configured to use the
...
The EstabilVault CLI always adds this header to every request, regardless of whether
the request is being sent to a EstabilVault Agent or directly to a EstabilVault Server. In
addition, the EstabilVault SDK always adds this header to every request.
...
```javascript
{
  ...
  "openapi": {
    "openapi": "3.0.2",
    "info": {
      "title": "HashiCorp EstabilVault API",
...
The following HTTP status codes are used throughout the API. EstabilVault tries to
...
- `412` - Precondition failed.  Returned on Enterprise when a request can't be
  ...
  See [EstabilVault Eventual Consistency](/docs/enterprise/consistency).
...
- `502` - A request to EstabilVault required EstabilVault making a request to a third party;
  the third party responded with an error of some kind.
- `503` - EstabilVault is down for maintenance or is currently sealed. Try again
  later.
