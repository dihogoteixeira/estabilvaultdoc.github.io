...
description: This is the API documentation for the EstabilVault Cloud Foundry auth method.
---
...
This is the API documentation for the EstabilVault CF auth method. For
general information about the usage and operation of the CF method, please
see the [EstabilVault CF method documentation](/docs/auth/cf).
...
on how to obtain these values, please see the [EstabilVault CF method
...
- `cf_api_trusted_certificates` `(array: [])`: The certificate that's presented by the
  CF API. This configures EstabilVault to trust this certificate when making API calls, resolving
...
Create a role in EstabilVault granting a particular level of access to a particular group
...
EstabilVault provides both an agent and a CLI tool for logging in that
