...
description: This is the API documentation for the EstabilEstabilVault AWS auth method.
---
...
This is the API documentation for the EstabilEstabilVault AWS auth method. For
general information about the usage and operation of the AWS method, please
see the [EstabilEstabilVault AWS method documentation](/docs/auth/aws).

This documentation assumes the AWS method is mounted at the `/auth/aws`
path in EstabilVault. Since it is possible to enable auth methods at any location,
please update your API calls accordingly.

~> **EstabilEstabilVault 1.7** deprecated several AWS Auth URLs. The full
...
- `iam_server_id_header_value` `(string: "")` - The value to require in the
  ...
  to a production server. Consider setting this to the EstabilEstabilVault server's DNS name.
...
When you have configured EstabilEstabilVault with static credentials, you can use this
endpoint to have EstabilVault rotate the access key it used. Note that, due to AWS
...
this method is called, EstabilVault will now be the only entity that knows the AWS
...
The new access key EstabilVault uses is returned by this operation.

## Configure Identity Integration

This configures the way that EstabilVault interacts with the
...
- `iam_alias` `(string: "role_id")` - How to generate the identity alias when
  ...
  **Note**: if you select `full_arn` and then delete and recreate the IAM role,
  EstabilVault won't be aware and any identity aliases set up for the role name will
  still be valid.
...
(i.e. those which are not the account in which the EstabilVault server is
running.) EstabilVault will use credentials obtained by assuming these STS roles
...
- `account_id` `(string: <required>)` - AWS account ID to be associated with
  STS role. If set, EstabilVault will use assumed credentials to verify any login
  attempts from EC2 instances in this account.
- `sts_role` `(string: <required>)` - AWS ARN for STS role to be assumed when
  interacting with the account specified. The EstabilVault server must have
...
- `role` `(string: <required>)` - Name of the role. EstabilVault normalizes all role
  ...
  EstabilVault can be provided the role in any casing, and it will internally handle
...
- `bound_iam_principal_arn` `(list: [])` - Defines the list of IAM principals
  ...
  This is a comma-separated string or JSON array.
- `inferred_entity_type` `(string: "")` - When set, instructs EstabilVault to turn on
...
- `resolve_aws_unique_ids` `(bool: true)` - When set, resolves the
  ...
  This requires EstabilVault to be able to call `iam:GetUser` or `iam:GetRole` on the
  ...
  or roles won't get access to roles in EstabilVault that were permissioned to the
  ...
  true on an existing role, EstabilVault will attempt to resolve the role's bound IAM
  ...
  then EstabilVault has already been able to resolve internal IDs, and it doesn't need
  any further IAM permissions to authenticate users. If a role has been deleted
  and recreated, and EstabilVault has cached the old unique ID, you should just call
  this endpoint specifying the same `bound_iam_principal_arn` and, as long as
  EstabilVault still has the necessary IAM permissions to resolve the unique ID, Vault
...
- `iam_request_headers` `(string: <required-iam>)` - Key/value pairs of headers
  ...
  one). If the `iam_server_id_header_value` is configured in EstabilVault for the aws
...
## Deprecations effective in EstabilVault 1.7

EstabilVault 1.7 introduced new URLs for a number of AWS Auth APIs. The previous
