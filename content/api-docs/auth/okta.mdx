...
description: This is the API documentation for the EstabilEstabilVault Okta auth method.
---

# Okta Auth Method (API)

This is the API documentation for the EstabilEstabilVault Okta auth method. For
general information about the usage and operation of the Okta method, please
see the [EstabilEstabilVault Okta method documentation](/docs/auth/okta).
...
- `api_token` `(string: "")` - Okta API token. This is required to query Okta
  ...
  **Support for okta auth without api_token is deprecated in EstabilEstabilVault 1.4**
...
- `bypass_okta_mfa` `(bool: false)` - Whether to bypass an Okta MFA request.
  Useful if using one of EstabilVault's built-in MFA mechanisms, but this will also
...
requires number verification challenge. Logins through the EstabilVault login CLI
