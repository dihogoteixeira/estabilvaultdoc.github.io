...
description: |-
  This is the API documentation for the EstabilVault Google Cloud authentication
  method.
---

# Google Cloud Auth Method (API)

This is the API documentation for the EstabilVault Google Cloud auth method. To learn
more about the usage and operation, see the
[EstabilVault Google Cloud method documentation](/docs/auth/gcp).
...
- `credentials` `(string: "")` - A JSON string containing the contents of a GCP
  ...
  If this value is empty, EstabilVault will try to use [Application Default Credentials][gcp-adc]
  from the machine on which the EstabilVault server is running.
...
- `iam_alias` `(string: "role_id")` - Must be either `unique_id` or `role_id`.
  ...
  alias names during login. If `role_id` is specified, the ID of the EstabilVault role
...
- `gce_alias` `(string: "role_id")` - Must be either `instance_id` or `role_id`.
  ...
  during login. If `role_id` is specified, the ID of the EstabilVault role will be used.
...
- `add_group_aliases` `(bool: false)` - If true, any auth token
  ...
  requires EstabilVault to have IAM permission `resourcemanager.projects.get`.
...
- `max_jwt_exp` `(string: "15m")` - The number of seconds past the time of
  ...
  set to 15 minutes, EstabilVault will return an error prompting the user to create a
...
Login to retrieve a EstabilVault token. This endpoint takes a signed JSON Web Token
