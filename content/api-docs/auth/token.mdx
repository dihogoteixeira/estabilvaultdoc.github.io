...
description: This is the API documentation for the EstabilVault token auth method.
---

# Token Auth Method (API)

This is the API documentation for the EstabilVault token auth method. For
general information about the usage and operation of the token method, please
see the [EstabilVault Token method documentation](/docs/auth/token).
...
- `increment` `(string: "")` - An optional requested increment duration can be
  ...
  If not supplied, EstabilVault will use the default TTL. This is specified as a numeric string
...
EstabilVault internal cache is unlimited in size and every value read from storage will
...
for your cluster, it would be wise to give EstabilVault more resources before attempting
