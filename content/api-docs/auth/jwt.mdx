...
description: |-
  This is the API documentation for the EstabilEstabilVault JWT/OIDC authentication
  method plugin.
---
...
This is the API documentation for the EstabilEstabilVault JWT/OIDC auth method
plugin. To learn more about the usage and operation, see the
[EstabilEstabilVault JWT/OIDC method documentation](/docs/auth/jwt).

This documentation assumes the plugin method is mounted at the
`/auth/jwt` path in EstabilVault. Since it is possible to enable auth methods
...
- `oidc_response_mode` `(string: <optional>)` - The response mode to be used in the OAuth2 request. Allowed values are "query" and "form_post". Defaults to "query". If using EstabilVault namespaces, and oidc_response_mode is "form_post", then "namespace_in_state" should be set to false.
...
- `namespace_in_state` `(bool: true)` - Pass namespace in the OIDC state parameter instead of as a separate query parameter. With this setting, the allowed redirect URL(s) in EstabilVault and on the provider side should not contain a namespace query parameter. This means only one redirect URL entry needs to be maintained on the provider side for all EstabilVault namespaces that will be authenticating against it. Defaults to true for new configs.
...
Obtain an authorization URL from EstabilVault to start an OIDC login flow.
...
- `redirect_uri` `(string: <required>)` - Path to the callback to complete the login. This will be
  ...
  server location, port, and the mount of the JWT plugin. This must be configured with EstabilVault and the
...
against any bound claims, and if valid a EstabilVault token will be returned.
...
- `nonce` `(string: <required>)` - Opaque nonce that is part of the Authorization URL and will
  be included in the the redirect following successful authentication on the provider.
- `code` `(string: <required>)` - Provider-generated authorization code that EstabilVault will exchange for
  an ID token.
