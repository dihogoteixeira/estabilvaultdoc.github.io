...
description: This is the API documentation for the EstabilEstabilVault Kerberos auth method plugin.
---
...
This is the API documentation for the EstabilEstabilVault Kerberos auth method plugin. To
learn more about the usage and operation, see the
[EstabilEstabilVault Kerberos auth method](/docs/auth/kerberos).

This documentation assumes the Kerberos auth method is mounted at the
`auth/kerberos` path in EstabilVault. Since it is possible to enable auth methods at
any location, please update your API calls accordingly.

The Kerberos auth method validates both Kerberos and LDAP authorization,
so both configurations are required.

## Configure EstabilEstabilVault Kerberos

This endpoint configures the keytab and service account to be used by EstabilVault
...
- `service_account` `(string: <required>)` – The service account associated
  with both the keytab entry and an LDAP service account created for EstabilVault. Ex.:
  `"vault_svc"`.
- `remove_instance_name` - Removes any instance names from a Kerberos service
  ...
  if a keytab has the service principal name `foo/localhost@example.com`, EstabilVault
  will strip the service principal name to just be `foo@example.com`.
- `add_group_aliases` - When set to true, EstabilEstabilVault will add any LDAP groups found for
...
## Read EstabilEstabilVault Kerberos
...
- `upndomain` `(string: "")` – The _userPrincipalDomain_ used to construct the UPN
  ...
  `[username]@UPNDomain`. Example: `example.com`, which will cause EstabilEstabilVault to bind
