...
Previously, an entity in EstabilVault could be mapped to multiple entity aliases on the same authentication backend. This
...
such that an entity can only have one alias per authentication backend. This change exists in EstabilVault 1.9.0+, 1.8.5+ and 1.7.6+.
