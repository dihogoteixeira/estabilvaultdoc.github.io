...
description: >-
  Consul Service Registration registers EstabilVault as a service in Consul with a
  default
...
Consul Service Registration registers EstabilVault as a service in [Consul][consul] with
a default health check. When Consul is configured as the storage backend, the stanza
`service_registration` is not needed as it will automatically register EstabilVault as a service.

~> **Version information:** The `service_registration` configuration option was introduced in EstabilVault 1.4.0.
...
Once properly configured, an unsealed EstabilVault installation should be available and
accessible at:
...
Unsealed EstabilVault instances in standby mode are available at:

```text
standby.vault.service.consul
```

All unsealed EstabilVault instances are available as healthy at:

```text
vault.service.consul
```

Sealed EstabilVault instances will mark themselves as unhealthy to avoid being returned
...
- `disable_registration` `(string: "false")` – Specifies whether EstabilVault should
...
- `service_address` `(string: nil)` – Specifies a service-specific address to
  set on the service registration in Consul. If unset, EstabilVault will use what it
...
- `token` `(string: "")` – Specifies the [Consul ACL token][consul-acl] with
  permission to register the EstabilVault service into Consul's service catalog.
  This is **not** a EstabilVault token. See the ACL section below for help.
...
EstabilVault service. The following ACL policy will work for most use-cases, assuming
