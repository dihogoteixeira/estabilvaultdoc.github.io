...
description: |-
  ...
  in-memory on the same machine in which EstabilVault is running. This is useful for
...
on the same machine in which EstabilVault is running. This is useful for development
and experimentation, but use of this backend is **highly discouraged in
production**. All data is lost when EstabilVault or the machine on which it is running
is restarted.
