...
description: |-
  ...
  backend will also register EstabilVault as a service in Consul with a default health
  check.
---
...
- `path` `(string: "vault/")` – Specifies the path in Consul's key-value store
  where EstabilVault data will be stored.
...
- `token` `(string: "")` – Specifies the [Consul ACL token][consul-acl] with
  ...
  This is **not** a EstabilVault token. This can also be provided via the environment
