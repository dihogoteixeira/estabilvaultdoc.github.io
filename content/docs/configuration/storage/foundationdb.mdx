...
in the standard EstabilVault binary distribution. Please refer to the documentation
accompanying the backend's source in the EstabilVault source tree.
...
- `path` `(string: "vault")` - The path of the top-level FoundationDB directory
  (using the directory layer) under which the EstabilVault data will reside.
...
by the user EstabilVault is running as. You probably want to isolate the cluster
...
$ /path/to/bin/EstabilVault ...
```

#### Client setup with `RPATH`

When running EstabilVault with mlock, the EstabilVault binary must have capabilities set to
allow the use of mlock.

```
# setcap cap_ipc_lock=+ep /path/to/bin/vault
$ getcap /path/to/bin/vault
/path/to/bin/EstabilVault = cap_ipc_lock+ep
```

The presence of the capabilities will cause the linker to ignore
`LD_LIBRARY_PATH`, for security reasons.

In that case, we have to set an `RPATH` on the EstabilVault binary at build time
...
You can verify `RPATH` is set on the EstabilVault binary using `readelf`:

```shell-session
$ readelf -d /path/to/bin/EstabilVault | grep RPATH
