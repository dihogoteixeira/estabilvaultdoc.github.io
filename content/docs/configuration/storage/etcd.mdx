...
be decommissioned by Etcd v3.6. It will be removed from EstabilVault in EstabilVault 1.10.
Users of the Etcd storage backend should prepare to
[migrate](/docs/commands/operator/migrate) EstabilVault storage to an Etcd v3 cluster
prior to upgrading to EstabilVault 1.10. All storage migrations should have
...
- `path` `(string: "/vault/")` – Specifies the path in Etcd where EstabilVault data will
  be stored.
...
- `sync` `(string: "true")` – Specifies whether to sync the list of available
  ...
  server and syncing causes EstabilVault to fail.
...
- `lock_timeout` `(string: "15s")` – Specifies lock timeout for master
  EstabilVault instance. Set bigger value if you don't need faster recovery.

## `etcd` Examples

### DNS Discovery of cluster members

This example configures EstabilVault to discover the Etcd cluster members via SRV
