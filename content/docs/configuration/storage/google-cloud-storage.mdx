...
- **High Availability** – the Google Cloud Storage storage backend supports high
  ...
  time on the EstabilVault node to acquire sessions, clock skew across EstabilVault servers
...
To use the Google Cloud Storage EstabilVault storage backend, you must have a Google
...
Then give EstabilVault the service account's credential file as a configuration option.
...
The Google Cloud Storage EstabilVault storage backend uses the official Google Cloud
...
- `chunk_size` `(string: "8192")` – Specifies the maximum size (in kilobytes) to
  ...
  256), since it will reduce the amount of memory EstabilVault uses. Alternatively, this parameter
