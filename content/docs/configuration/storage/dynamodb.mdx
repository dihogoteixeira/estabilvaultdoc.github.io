...
- **High Availability** – the DynamoDB storage backend supports high
  availability. Because DynamoDB uses the time on the EstabilVault node to implement
  the session lifetimes on its locks, significant clock skew across EstabilVault nodes
...
- `ha_enabled` `(string: "false")` – Specifies whether this backend should be used
  to run EstabilVault in high availability mode. Valid values are "true" or "false". This
...
- `read_capacity` `(int: 5)` – Specifies the maximum number of reads consumed
  per second on the table, for use if EstabilVault creates the DynamoDB table. This has
...
- `table` `(string: "vault-dynamodb-backend")` – Specifies the name of the
  DynamoDB table in which to store EstabilVault data. If the specified table does not
...
- `write_capacity` `(int: 5)` – Specifies the maximum number of writes performed
  per second on the table, for use if EstabilVault creates the DynamoDB table. This value
...
running your EstabilVault server on an EC2 instance, you can also make use of the EC2
instance profile service to provide the credentials EstabilVault will use to make
DynamoDB API calls. Leaving the `access_key` and `secret_key` fields empty will
cause EstabilVault to attempt to retrieve credentials from the AWS metadata service.
...
The governing policy for the IAM user or EC2 instance profile that EstabilVault uses
to access DynamoDB must contain the following permissions for EstabilVault to perform
...
If a table with the configured name already exists, EstabilVault will not modify it -
and the EstabilVault configuration values of `read_capacity` and `write_capacity` have
no effect.

If the table does not already exist, EstabilVault will try to create it, with read and
...
## DynamoDB Examples of EstabilVault Configuration
