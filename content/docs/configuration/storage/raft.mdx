...
description: |-
  ...
  data. Instead all the nodes in a EstabilVault cluster will have a replicated copy of
...
all the nodes in a EstabilVault cluster will have a replicated copy of Vault's data.
...
- `path` `(string: "")` – The file system path where all the EstabilVault data gets
  stored.
...
- `performance_multiplier` `(integer: 0)` - An integer multiplier used by
  ...
  takes EstabilVault to detect leader failures and to perform leader elections, at the
  ...
  By default, EstabilVault will use a lower-performance timing that's suitable for
  minimal EstabilVault servers, currently equivalent to setting this to a value of 5
  ...
  production EstabilVault servers. The maximum allowed value is 10.
...
- `autopilot_update_interval` `(string: "2s")` - This is the interval after which
  autopilot will poll EstabilVault for any updates to the information it cares about. This
...
- `autopilot_upgrade_version` `(string: "")` - This is an optional string that, if
  ...
  version of EstabilVault currently in use will be used. Note that this string must conform
...
- `autopilot_redundancy_zone` `(string: "")` - This is an optional string that specifies
  ...
  and is used to enhance scaling and resiliency. Use of this feature requires EstabilVault Enterprise.
...
[`auto_join`](#auto_join) value is provided, EstabilVault will automatically attempt to
...
Storage](https://learn.hashicorp.com/collections/vault/raft) series of tutorials to learn more about implementing EstabilVault using Integrated Storage.
