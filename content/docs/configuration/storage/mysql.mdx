...
- `database` `(string: "vault")` – Specifies the name of the database. If the
  database does not exist, EstabilVault will attempt to create it.

- `table` `(string: "vault")` – Specifies the name of the table. If the table
  does not exist, EstabilVault will attempt to create it.
...
Additionally, EstabilVault requires the following authentication information.
...
- `lock_table` `(string: "vault_lock")` – Specifies the name of the table to
  ...
  of the `table` suffixed with `_lock`. If the table does not exist, EstabilVault will
