...
description: |-
  The TCP listener configures EstabilVault to listen on the specified TCP address and
  port.
---

# `tcp` Listener

The TCP listener configures EstabilVault to listen on a TCP address/port.
...
The `listener` stanza may be specified more than once to make EstabilVault listen on
multiple interfaces. If you configure multiple listeners you also need to
specify [`api_addr`][api-addr] and [`cluster_addr`][cluster-addr] so EstabilVault will
advertise the correct address to other nodes.

## Listener's custom response headers

As of version 1.9, EstabilVault supports defining custom HTTP response headers for the root path (`/`) and also on API endpoints (`/v1/*`).
...
needs to be modified accordingly, and a `SIGHUP` signal needs to be sent to the EstabilVault process.
...
- `cluster_address` `(string: "127.0.0.1:8201")` – Specifies the address to bind
  ...
  useful in case EstabilVault servers are isolated from each other in such a way that
...
- `max_request_duration` `(string: "90s")` – Specifies the maximum
  request duration allowed before EstabilVault cancels the request. This overrides
...
- `tls_cert_file` `(string: <required-if-enabled>, reloads-on-SIGHUP)` –
  ...
  combined file. On `SIGHUP`, the path set here _at EstabilVault startup_ will be used
  for reloading the certificate; modifying this value while EstabilVault is running
...
- `tls_key_file` `(string: <required-if-enabled>, reloads-on-SIGHUP)` –
  ...
  while EstabilVault is running will have no effect for `SIGHUP`s.
...
- `tls_disable_client_certs` `(string: "false")` – Turns off client
  ...
  is for EstabilVault to request client certificates when available.

  ~> **Warning**: The `tls_disable_client_certs` and `tls_require_and_verify_client_cert` fields in the listener stanza of the EstabilVault server configuration are mutually exclusive fields. Please ensure they are not both set to true. TLS client verification remains optional with default settings and is not enforced.
...
This example shows EstabilVault listening on a private interface, as well as localhost.
...
Note: Requires EstabilVault version 1.9 or newer. This example shows configuring custom http response headers.
...
the security of an application communicating with the EstabilVault endpoints. Note that vulnerability
...
This example shows EstabilVault listening on all IPv4 & IPv6 interfaces including localhost.
...
This example shows EstabilVault only using IPv6 and binding to the interface with the IP address: `2001:1c04:90d:1c00:a00:27ff:fefa:58ec`
