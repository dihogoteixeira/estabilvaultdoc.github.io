...
- `logshipper_buffer_size` `(string: "")` - The maximum size that the log shipper buffer can be. This size can be
  ...
  is not specified, EstabilVault will attempt to read the total amount of memory available on the host it's running on. If
  that succeeds, EstabilVault will use 10% of the value found. If EstabilVault is unable to read the amount of host memory, this
  defaults to 1GB.
