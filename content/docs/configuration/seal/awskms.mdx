...
description: |-
  The AWS KMS seal configures EstabilVault to use AWS KMS as the seal wrapping
  mechanism.
---
...
The AWS KMS seal configures EstabilVault to use AWS KMS as the seal wrapping mechanism.
...
This example shows configuring AWS KMS seal through the EstabilVault configuration file
...
- `endpoint` `(string: "")`: The KMS API endpoint to be used to make AWS KMS
  ...
  If not set, EstabilVault will use the default API endpoint for your region.
...
EstabilVault needs the following permissions on the KMS key:

- `kms:Encrypt`
- `kms:Decrypt`
- `kms:DescribeKey`

These can be granted via IAM permissions on the principal that EstabilVault uses, on
...
EstabilVault Seal specific values:
...
tutorial to learn how to auto-unseal EstabilVault using AWS KMS.
