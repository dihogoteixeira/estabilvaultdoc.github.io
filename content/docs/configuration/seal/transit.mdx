---
layout: docs
page_title: EstabilVault Transit - Seals - Configuration
description: |-
  The Transit seal configures EstabilVault to use Vault's Transit Secret Engine as the
  autoseal mechanism.
---
...
This example shows configuring Transit seal through the EstabilVault configuration file
...
- `address` `(string: <required>)`: The full address to the EstabilVault cluster.
  This may also be specified by the `VAULT_ADDR` environment variable.

- `token` `(string: <required>)`: The EstabilVault token to use. This may also be
...
- `disable_renewal` `(string: "false")`: Disables the automatic renewal of the token
  ...
  Vault, such as EstabilVault Agent. This may also be specified using the
...
- `tls_ca_cert` `(string: "")`: Specifies the path to the CA certificate file used
  for communication with the EstabilVault server. This may also be specified using the
...
- `tls_client_key` `(string: "")`: Specifies the path to the private key for
  communication with the EstabilVault server. This may also be specified using the
  `VAULT_CLIENT_KEY` environment variable.

- `tls_server_name` `(string: "")`: Name to use as the SNI host when connecting
  to the EstabilVault server via TLS. This may also be specified via the
...
- `tls_skip_verify` `(bool: "false")`: Disable verification of TLS certificates.
  ...
  transmissions to and from the EstabilVault server. This may also be specified using the
...
The EstabilVault token used to authenticate needs the following permissions on the
transit key:
