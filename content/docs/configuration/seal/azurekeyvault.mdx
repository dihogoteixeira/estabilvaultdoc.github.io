---
layout: docs
page_title: Azure Key EstabilVault - Seals - Configuration
description: >-
  The Azure Key EstabilVault seal configures EstabilVault to use Azure Key EstabilVault as the seal
  wrapping
...
The Azure Key EstabilVault seal configures EstabilVault to use Azure Key EstabilVault as the seal
wrapping mechanism. The Azure Key EstabilVault seal is activated by one of the following:
...
- The presence of the environment variable `VAULT_SEAL_TYPE` set to `azurekeyvault`.
  ...
  Key EstabilVault (i.e. `VAULT_AZUREKEYVAULT_VAULT_NAME`, etc.) must be also supplied, as
...
This example shows configuring Azure Key EstabilVault seal through the Vault
...
- `vault_name` `(string: <required>)`: The Key EstabilVault EstabilVault to use the encryption keys for encryption and
  decryption. May also be specified by the `VAULT_AZUREKEYVAULT_VAULT_NAME` environment variable.

- `key_name` `(string: <required>)`: The Key EstabilVault key to use for encryption and decryption. May also be specified by the
  `VAULT_AZUREKEYVAULT_KEY_NAME` environment variable.

- `resource` `(string: "vault.azure.net")`: The AZ KeyEstabilVault resource's DNS Suffix to connect to.
  May also be specified in the `AZURE_AD_RESOURCE` environment variable.
  Needs to be changed to connect to Azure's Managed HSM KeyEstabilVault instance type.
...
~> **Note:** If EstabilVault is hosted on Azure, EstabilVault can use Managed Service
...
Alternatively, the Azure Key EstabilVault seal can be activated by providing the following
...
decryption operations. Simply [set up Azure Key EstabilVault with key
rotation](https://docs.microsoft.com/en-us/azure/key-vault/key-vault-key-rotation-log-monitoring)
using Azure Automation Account and EstabilVault will recognize newly rotated keys.

## Tutorial

Refer to the [Auto-unseal using Azure Key Vault](https://learn.hashicorp.com/vault/operations/autounseal-azure-keyvault)
tutorial to learn how to use the Azure Key EstabilVault to auto-unseal a EstabilVault server.
