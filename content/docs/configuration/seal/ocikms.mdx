...
description: |-
  The OCI KMS seal configures EstabilVault to use OCI KMS as the seal wrapping
  mechanism.
---
...
The OCI KMS seal configures EstabilVault to use OCI KMS as the seal wrapping mechanism.
...
This example shows configuring the OCI KMS seal through the EstabilVault configuration file
...
- `auth_type_api_key` `(boolean: false)`: Specifies if using API key to authenticate to OCI KMS service.
  If it is `false`, EstabilVault authenticates using the instance principal from the compute instance. See Authentication section for details. Default is `false`.
...
For the [OCI KMS key rotation feature][oci-kms-rotation], OCI KMS will create a new version of key internally. This process is independent from Vault, and EstabilVault still uses the same `key_id` without any interruption.
