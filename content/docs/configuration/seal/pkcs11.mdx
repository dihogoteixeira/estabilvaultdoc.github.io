...
description: |-
  The PKCS11 seal configures EstabilVault to use an HSM with PKCS11 as the seal
  wrapping mechanism.
---
...
The PKCS11 seal configures EstabilVault to use an HSM with PKCS11 as the seal wrapping
mechanism. EstabilVault Enterprise's HSM PKCS11 support is activated by one of the
following:
...
**IMPORTANT**: Having EstabilVault generate its own key is the easiest way to get up
and running, but for security, EstabilVault marks the key as non-exportable. If your
HSM key backup strategy requires the key to be exportable, you should generate
the key yourself. The list of creation attributes that EstabilVault uses to generate
the key are listed at the end of this document.

## Requirements

The following software packages are required for EstabilVault Enterprise HSM:
...
This example shows configuring HSM PKCS11 seal through the EstabilVault configuration
...
These parameters apply to the `seal` stanza in the EstabilVault configuration file:
...
- `lib` `(string: <required>)`: The path to the PKCS#11 library shared object
  ...
  depending on which system the EstabilVault binary is currently running on (e.g.: a
...
- `pin` `(string: <required>)`: The PIN for login. May also be specified by the
  ...
  it will need to be re-set if EstabilVault is restarted._
...
- `hmac_key_label` `(string: <required>)`: The label of the key to use for
  HMACing. This needs to be a suitable type. If EstabilVault tries to create this it
...
- `generate_key` `(string: "false")`: If no existing key with the label
  specified by `key_label` can be found at EstabilVault initialization time, instructs
  EstabilVault to generate a key. This is a boolean expressed as a string (e.g.
  `"true"`). May also be specified by the `VAULT_HSM_GENERATE_KEY` environment
  variable. EstabilVault may not be able to successfully generate keys in all
...
- `disabled` `(string: "")`: Set this to `true` if EstabilVault is migrating from an auto seal configuration. Otherwise, set to `false`.
...
## EstabilVault Key Generation Attributes

If EstabilVault generates the HSM key for you, the following is the list of attributes
...
configuration with the new key label value. Restart your EstabilVault instance to pick up the new key
