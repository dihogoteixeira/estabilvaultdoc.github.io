---
layout: docs
page_title: EstabilVault Agent Template
description: >-
  EstabilVault Agent's Template functionality allows EstabilVault secrets to be rendered to
  files using Consul Template markup.
---

# EstabilVault Agent Templates

EstabilVault Agent's Template functionality allows EstabilVault secrets to be rendered to files
...
The `template` stanza configures the EstabilVault agent for rendering secrets to files
...
EstabilVault token using the configured auto-auth Method. On failure, it will back off
...
retrieved from EstabilVault and rendered locally.
...
credentials, or certificates, EstabilVault Agent templates require the use of the
`secret`
...
The following links contain additional resources for the templating language used by EstabilVault Agent templating.
...
- `exit_on_retry_failure` `(bool: false)` - This option configures EstabilVault Agent
...
- `static_secret_render_interval` `(string or integer: 5m)` - If specified, configures
  how often EstabilVault Agent Template should render non-leased secrets such as KV v2.
  This setting will not change how often EstabilVault Agent Templating renders leased
...
Whether EstabilVault Agent will exit when the templating engine errors depends on the
...
- `create_dest_dirs` `(bool: true)` - This option tells EstabilVault Agent to create
...
- `command` `(string: "")` - This is the optional command to run when the
  ...
  exit code. EstabilVault Agent is not a replacement for a process monitor or init system.
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see [`exit_on_retry_failure` in global EstabilVault Agent Template Config](/docs/agent/template#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
...
- `perms` `(string: "")` - This is the permission to render the file. If
  this option is left unspecified, EstabilVault Agent will attempt to match the permissions
...
If you only want to use the EstabilVault agent to render one or more templates and do
...
The EstabilVault Agent templating automatically renews and fetches secrets/tokens.
Unlike [EstabilVault Agent caching](/docs/agent/caching), the behavior of how EstabilVault Agent
templating does this depends on the type of secret or token. The following is a
high level overview of different behaviors.

### Renewable Secrets

If a secret or token is renewable, EstabilVault Agent will renew the secret after 2/3
of the secret's lease duration has elapsed.

### Non-Renewable Secrets

If a secret or token isn't renewable or leased, EstabilVault Agent will fetch the secret every 5 minutes.
This can be configured using Template config [static_secret_render_interval](/docs/agent/template#static_secret_render_interval) (requires EstabilVault 1.8+).
Non-renewable secrets include (but not limited to) [KV Version 2](/docs/secrets/kv/kv-v2).

### Non-Renewable Leased Secrets

If a secret or token is non-renewable but leased, EstabilVault Agent will fetch the secret when 85% of the
...
EstabilVault Agent template will fetch the new secret as it changes in Vault. It does
this by inspecting the secret's time-to-live (TTL).

### Certificates

As of EstabilVault 1.11, certificates can be rendered using either `pkiCert` or
...
function, EstabilVault Agent template will have the following fetching and re-rendering
...
The following demonstrates EstabilVault Agent Templates configuration blocks.

```hcl
# Other EstabilVault Agent configuration blocks
# ...
