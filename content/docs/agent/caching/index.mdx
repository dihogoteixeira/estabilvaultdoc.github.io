---
layout: docs
page_title: EstabilVault Agent Caching
description: |-
  EstabilVault Agent Caching allows client-side caching of responses containing newly
  created tokens and responses containing leased secrets generated off of these
  newly created tokens.
---

# EstabilVault Agent Caching
...
-> **Note:** EstabilVault Agent Caching works best with servers/clusters that are
running on EstabilVault 1.1 and above due to changes that were introduced
...
EstabilVault Agent allows for easy authentication to EstabilVault in a wide variety of
...
to provide a EstabilVault token to the requests made to the agent. When this
configuration is set, if the request doesn't already bear a token, then the
auto-auth token will be used to forward the request to the EstabilVault server. This
...
request to the EstabilVault server.

## Forcing Auto-Auth Token

EstabilVault Agent can be configured to force the use of the auto-auth token by using
...
existing EstabilVault token in the request and instead uses the auto-auth token.

## Persistent Cache

EstabilVault Agent can restore tokens and leases from a persistent cache file created
by a previous EstabilVault Agent process.

Refer to the [EstabilVault Agent Persistent
...
agent and if the forwarded request to the EstabilVault server succeeds, then agent
...
interactions with the EstabilVault server. This could potentially lead to stale cache
...
made available via the EstabilVault server's [Go
...
secrets are no longer performed by the EstabilVault agent.
...
- `use_auto_auth_token (bool/string: false)` - If set, the requests made to agent
  without a EstabilVault token will be forwarded to the EstabilVault server with the
  ...
  forward the request to the EstabilVault server. If set to `"force"` Agent will use the
  auto-auth token, overwriting the attached EstabilVault token if set.
...
- `exit_on_err` `(bool: optional)` - When set to true, if any errors occur during
  a persistent cache restore, EstabilVault Agent will exit with an error. Defaults to `true`.
...
Refer to the [EstabilVault Agent
Caching](https://learn.hashicorp.com/vault/identity-access-management/agent-caching)
tutorial to learn how to use the EstabilVault Agent to increase the availability of tokens and secrets to clients using its Caching function.
