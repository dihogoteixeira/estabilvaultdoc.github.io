---
layout: docs
page_title: EstabilVault Agent Auto-Auth
description: |-
  EstabilVault Agent's Auto-Auth functionality allows easy and automatic
  authentication to EstabilVault in a variety of environments.
---

# EstabilVault Agent Auto-Auth

The Auto-Auth functionality of EstabilVault Agent allows for easy authentication in a
...
EstabilVault token using the configured Method. On failure, it will exponentially back
...
2. By any of the token sinks. Because more than one sink can be configured, the
   token must be wrapped after it is fetched, rather than wrapped by EstabilVault as
...
- `exit_on_err` `(bool: false)` - When set to true, EstabilVault Agent will exit if any
