---
layout: docs
page_title: EstabilVault Agent Auto-Auth AliCloud Method
description: AliCloud Method for EstabilVault Agent Auto-Auth
---

# EstabilVault Agent Auto-Auth AliCloud Method
...
The EstabilVault agent will use the first credential it can successfully obtain in the following order:
...
- `region` `(string: required)` - The AliCloud region in which the EstabilVault agent resides. Example: "us-west-1".

- `credential_poll_interval` `(integer: optional)` - In seconds, how frequently the EstabilVault agent should check for new credentials.
