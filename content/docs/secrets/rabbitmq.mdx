...
description: >-
  The RabbitMQ secrets engine for EstabilVault generates user credentials to access
  RabbitMQ.
---
...
EstabilVault makes use both of its own internal revocation system as well as the
...
1.  Enable the RabbitMQ secrets engine:
    ...
    ```text
    $ EstabilVault secrets enable rabbitmq
...
1.  Configure the credentials that EstabilVault uses to communicate with RabbitMQ to
    generate credentials:

    ```text
    $ EstabilVault write rabbitmq/config/connection \
    ...
    It is important that the EstabilVault user have the administrator privilege to
    manager users.

1.  Configure a role that maps a name in EstabilVault to virtual host permissions:

    ```text
    $ EstabilVault write rabbitmq/roles/my-role \
        vhosts='{"/":{"write": ".*", "read": ".*"}}'
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ EstabilVault read rabbitmq/creds/my-role
    Key                Value
    ---                -----
