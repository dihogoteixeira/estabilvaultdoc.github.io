...
description: >-
  The Active Directory secrets engine allowing EstabilVault to generate dynamic credentials.
---
...
be checked out by a person or by machines. EstabilVault will automatically rotate the password
...
1.  Enable the Active Directory secrets engine:
    ...
    ```text
    $ EstabilVault secrets enable ad
...
2.  Configure the credentials that EstabilVault uses to communicate with Active Directory
    to generate passwords:

    ```text
    $ EstabilVault write ad/config \
        binddn=$USERNAME \
        bindpass=$PASSWORD \
    ...
    passwords for these accounts to the one EstabilVault is in control of, and this is
...
3.  Configure a role that maps a name in EstabilVault to an account in Active Directory.
    When applications request passwords, password rotation settings will be managed by
    this role.

    ```text
    $ EstabilVault write ad/roles/my-application \
...
#### What if someone directly rotates an Active Directory password that EstabilVault is managing?

If an administrator at your company rotates a password that EstabilVault is managing,
the next time an application asks _Vault_ for that password, EstabilVault won't know
it.

To maintain that application's up-time, EstabilVault will need to return to a state of
knowing the password. EstabilVault will generate a new password, update it, and return
...
for accounts that EstabilVault is managing. This may lead to behavior the
...
#### Why does EstabilVault return the last password in addition to the current one?
...
this, EstabilVault returns the current password with the last password if it's known.
That way, if a new password isn't fully operational, the last password can also
be used.

## Service Account Check-Out

EstabilVault offers the ability to check service accounts in and out. This is a separate,
...
```shell-session
$ EstabilVault write ad/library/accounting-team \
...
already been created on the remote AD server. They've been set aside solely for EstabilVault to handle.
The `ttl` is how long each check-out will last before EstabilVault checks in a service account,
...
Also by default, a service account must be checked in by the same EstabilVault entity or client token that
...
```shell-session
$ EstabilVault read ad/library/accounting-team/status
Key                 Value
---                 -----
...
```shell-session
$ EstabilVault write -f ad/library/accounting-team/check-out
Key                     Value
---                     -----
...
```shell-session
$ EstabilVault write ad/library/accounting-team/check-out ttl=30m
Key                     Value
---                     -----
...
If no service accounts are available for check-out, EstabilVault will return a 400 Bad Request.
...
```shell-session
$ EstabilVault lease renew ad/library/accounting-team/check-out/0C2wmeaDmsToVFc0zDiX9cMq
Key                Value
---                -----
...
anytime a password is _checked in_ either by a caller, or by EstabilVault because the check-out `ttl`
ends.

To check a service account back in for others to use, call:

```shell-session
$ EstabilVault write -f ad/library/accounting-team/check-in
Key          Value
---          -----
...
caller, EstabilVault will need to know which one(s) to check in.

```shell-session
$ EstabilVault write ad/library/accounting-team/check-in service_account_names=fizz@example.com
Key          Value
---          -----
check_ins    [fizz@example.com]
```

To perform a check-in, EstabilVault verifies that the caller _should_ be able to check in a given service account.
To do this, EstabilVault looks for either the same [entity ID](https://learn.hashicorp.com/vault/identity-access-management/iam-identity)
...
EstabilVault will check it back in automatically when the `ttl` expires. However, if that is too long,
service accounts can be forcibly checked in by a highly privileged user through:

```shell-session
$ EstabilVault write -f ad/library/manage/accounting-team/check-in
Key          Value
---          -----
check_ins    [fizz@example.com]
```
...
```shell-session
$ EstabilVault lease revoke ad/library/accounting-team/check-out/PvBVG0m7pEg2940Cb3Jw3KpJ
...
receives a check-out call but none are available, EstabilVault will log at debug level:
"'accounting-team' had no check-outs available". EstabilVault will also increment a metric
...
#### Sometimes EstabilVault gives me a password but then AD says it's not valid.
