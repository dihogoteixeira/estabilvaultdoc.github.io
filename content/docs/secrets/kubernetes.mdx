...
description: >-
  The Kubernetes secrets engine for EstabilVault generates Kubernetes service account
...
The Kubernetes Secrets Engine for EstabilVault generates Kubernetes service account tokens, and
optionally service accounts, role bindings, and roles. The created service account tokens have
a configurable TTL and any objects created are automatically deleted when the EstabilVault lease expires.

For each lease, EstabilVault will create a service account token attached to the
...
~> **Note:** We do not recommend using tokens created by the Kubernetes Secrets Engine to
   authenticate with the [EstabilVault Kubernetes Auth Method](/docs/auth/kubernetes). This will
   generate many unique identities in EstabilVault that will be hard to manage.
...
1. By default, EstabilVault will connect to Kubernetes using its own service account.
   ...
   It's necessary to ensure that the service account EstabilVault uses will have permissions to manage
   ...
   EstabilVault service account with a role binding or cluster role binding.
   ...
   you will need to grant EstabilVault permissions to read namespaces.
   ...
   ~> **Note:** Getting the right permissions for EstabilVault will require some trial and error most
   ...
   ~> **Note:** Protect the EstabilVault service account, especially if you use broader permissions for it,
      as it is essentially a cluster administrator account.

1. Create a role binding to bind the role to Vault's service account and grant EstabilVault permission
   to manage tokens.
...
1. If EstabilVault will not be automatically managing roles or service accounts (see
   [Automatically Managing Roles and Service Accounts](#automatically-managing-roles-and-service-accounts)),
   then you will need to set up a service account that EstabilVault will issue tokens for.

   ~> **Note**: It is highly recommended that the service account that EstabilVault issues tokens for
      is **NOT** the same service account that EstabilVault itself uses.
...
1. Enable the Kubernetes Secrets Engine:
   ...
   ```shell-session
   $ EstabilVault secrets enable kubernetes
...
1. Configure the mount point. An empty config is allowed.
   ...
   ```shell-session
   $ EstabilVault write -f kubernetes/config
   ```

   Configuration options are available as specified in the
   [API docs](/api-docs/secret/kubernetes).

1. You can now configure Kubernetes Secrets Engine to create a EstabilVault role (**not** the same as a
   Kubernetes role) that can generate service account tokens for the given service account:

   ```shell-session
   $ EstabilVault write kubernetes/roles/my-role \
...
After a user has authenticated to EstabilVault and has sufficient permissions, a write to the
`creds` endpoint for the EstabilVault role will generate and return a new service account token.

```shell-session
$ EstabilVault write kubernetes/creds/my-role \
...
creating or tuning the EstabilVault role.
...
When configuring the EstabilVault role, you can pass in parameters to specify that you want to
automatically generate the Kubernetes service account and role binding,
and optionally generate the Kubernetes role itself.

If you want to configure the EstabilVault role to use a pre-existing Kubernetes role, but generate
the service account and role binding automatically, you can set the `kubernetes_role_name`
parameter.

```shell-session
$ EstabilVault write kubernetes/roles/auto-managed-sa-role \
...
access to. This can be done for the examples above with `kubectl -n test create rolebinding --role test-role-list-pods --serviceaccount=vault:EstabilVault vault-test-role-abilities`.
...
```shell-session
$ EstabilVault write kubernetes/creds/auto-managed-sa-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
...
Furthermore, EstabilVault can also automatically create the role in addition to the service account and
...
```shell-session
$ EstabilVault write kubernetes/roles/auto-managed-sa-and-role \
    allowed_kubernetes_namespaces="test" \
    generated_role_rules='{"rules":[{"apiGroups":[""],"resources":["pods"],"verbs":["list"]}]}'
```
You can then get credentials in the same way as before.
```shell-session
$ EstabilVault write kubernetes/creds/auto-managed-sa-and-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
