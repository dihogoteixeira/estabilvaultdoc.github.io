...
outside of EstabilVault into the transit secrets engine.
...
Note that though this example uses SHA256, EstabilVault also supports the use of
...
This is the ciphertext that should be provided to EstabilVault when importing a
key into the transit secrets engine.

```shell-session
$ EstabilVault write transit/keys/test-key/import ciphertext=$CIPHERTEXT hash_function=SHA256 type=$KEY_TYPE
```
