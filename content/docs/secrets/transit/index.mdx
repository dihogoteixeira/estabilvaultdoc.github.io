...
description: >-
  The transit secrets engine for EstabilVault encrypts/decrypts data in-transit. It
...
EstabilVault doesn't store the data sent to the secrets engine. It can also be viewed
...
1. Create a named encryption key:
   ...
   ```text
   $ EstabilVault write -f transit/keys/my-key
...
1.  Encrypt some plaintext data using the `/encrypt` endpoint with a named key:
    ...
    **NOTE:** All plaintext data **must be base64-encoded**. The reason for this
    requirement is that EstabilVault does not require that the plaintext is "text". It
    ...
    ```text
    $ EstabilVault write transit/encrypt/my-key plaintext=$(echo "my secret data" | base64)
    ...
    keys, EstabilVault knows which version to use for decryption. The rest is a base64
    concatenation of the initialization vector (IV) and ciphertext.

    Note that EstabilVault does not _store_ any of this data. The caller is responsible
    for storing the encrypted ciphertext. When the caller wants the plaintext,
    it must provide the ciphertext back to EstabilVault to decrypt the value.

    !> EstabilVault HTTP API imposes a maximum request size of 32MB to prevent a denial
    of service attack. This can be tuned per [`listener`
    block](/docs/configuration/listener/tcp) in the EstabilVault server
    configuration.

1.  Decrypt a piece of data using the `/decrypt` endpoint with a named key:

    ```text
    $ EstabilVault write transit/decrypt/my-key ciphertext=vault:v1:8SDd3WHDOjf7mq69CyCqYjBXAiQQAVZRkFM13ok481zoCmHnSeDX9vyf7w==
    ...
    ```text
    $ EstabilVault write -field=plaintext transit/decrypt/my-key ciphertext=... | base64 --decode
    my secret data
    ```
...
1.  Rotate the underlying encryption key. This will generate a new encryption key
    ...
    ```text
    $ EstabilVault write -f transit/keys/my-key/rotate
...
1.  Upgrade already-encrypted data to a new key. EstabilVault will decrypt the value
    using the appropriate key in the keyring and then encrypted the resulting
    plaintext with the newest key in the keyring.

    ```text
    $ EstabilVault write transit/rewrap/my-key ciphertext=vault:v1:8SDd3WHDOjf7mq69CyCqYjBXAiQQAVZRkFM13ok481zoCmHnSeDX9vyf7w==
...
```text
$ EstabilVault read transit/wrapping_key
```
...
- Wrap the AES key under the EstabilVault wrapping key using RSAES-OAEP with MGF1 and
