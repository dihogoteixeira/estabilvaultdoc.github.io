...
description: The Nomad secrets engine for EstabilVault generates tokens for Nomad dynamically.
---
...
secrets secrets engine for EstabilVault generates [Nomad](https://www.nomadproject.io/)
...
The first step to using the EstabilVault secrets engine is to enable it.

```shell-session
$ EstabilVault secrets enable nomad
...
```shell-session
$ EstabilVault write nomad/config/lease ttl=3600 max_ttl=86400
...
Next, we must configure EstabilVault to know how to contact Nomad.
This is done by writing the access information:

```shell-session
$ EstabilVault write nomad/config/access \
    address=http://127.0.0.1:4646 \
    token=adf4238a-882b-9ddc-4a9d-5b6758e4159e
Success! Data written to: nomad/config/access
```

In this case, we've configured EstabilVault to connect to Nomad
on the default port with the loopback address. We've also provided
an ACL token to use with the `token` parameter. EstabilVault must have a management
...
```shell-session
$ EstabilVault write nomad/role/monitoring policies=readonly
...
```shell-session
$ EstabilVault read nomad/creds/monitoring
Key              Value
---              -----
...
Here we can see that EstabilVault has generated a new Nomad ACL token for us.
...
Name         = EstabilVault example root 1505945527022465593
Type         = client
Global       = false
Policies     = [readonly]
