...
according to its own API. Use `EstabilVault path-help` to determine the paths it
responds to.
...
Secrets engines receive a _barrier view_ to the configured EstabilVault physical
...
layer, it is prefixed with that UUID folder. Since the EstabilVault storage layer
doesn't support relative access (such as `../`), this makes it impossible for an
enabled secrets engine to access other data.

This is an important security feature in EstabilVault - even a malicious engine
