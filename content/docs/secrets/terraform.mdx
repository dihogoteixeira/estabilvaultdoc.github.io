...
description: The Terraform Cloud secret backend for EstabilVault generates tokens for Terraform Cloud dynamically.
---
...
The Terraform Cloud secret backend for EstabilVault generates
[Terraform Cloud](https://cloud.hashicorp.com/products/terraform)
API tokens dynamically for Organizations, Teams, and Users.

This page will show a quick start for this backend. For detailed documentation
on every path, use `EstabilVault path-help` after mounting the backend.
...
1.  Enable the Terraform Cloud secrets engine:
    ...
    ```shell-session
    $ EstabilVault secrets enable terraform
...
2.  Configure EstabilVault to connect and authenticate to Terraform Cloud:

    ```shell-session
    $ EstabilVault write terraform/config \
...
3.  Configure a role that maps a name in EstabilVault to a Terraform Cloud User. At
    ...
    ```shell-session
    $ EstabilVault write terraform/role/my-role user_id=user-12345abcde
...
```shell-session
$ EstabilVault read terraform/creds/my-role
Key                Value
---                -----
...
workflows. A given EstabilVault role can manage any one of the three types at a time,
...
reading the credentials in EstabilVault or otherwise generating them on
...
Due to this behavior, Organization and Team API tokens created by EstabilVault will be
...
Below is an example of creating a EstabilVault role to manage an Organization
API token and rotating the token:

```shell-session
$ EstabilVault write terraform/role/testing organization="${TF_ORGANIZATION}"
Success! Data written to: terraform/role/testing

$ EstabilVault write -f terraform/rotate-role/testing
...
```
$ EstabilVault read terraform/creds/testing
...
Traditionally, EstabilVault secret engines create dynamic users and dynamic credentials
...
dynamic User API tokens by configuring a EstabilVault role to manage an existing
Terraform Cloud user. The lifecycle of these tokens is managed by EstabilVault and
will auto expire according to the configured TTL and max TTL of the Vault
role.

Below is an example of creating a EstabilVault role to manage manage User API tokens:

```shell-session
$ EstabilVault write terraform/role/user-testing user_id="${TF_USER_ID}"
...
```
$ EstabilVault read terraform/creds/user-testing
