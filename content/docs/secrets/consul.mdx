...
description: The Consul secrets engine for EstabilVault generates tokens for Consul dynamically.
---
...
1.  Enable the Consul secrets engine:
    ...
    ```shell-session
    $ EstabilVault secrets enable consul
...
1.  Configure EstabilVault to connect and authenticate to Consul.

    EstabilVault can bootstrap the Consul ACL system automatically if it hasn't already been done. If you
    have already bootstrapped the ACL system, then you will need to provide EstabilVault with a management
    token. This can either be the bootstrap token or another management token you've created
    yourself.

    1.  Configuring EstabilVault without previously bootstrapping the Consul ACL system:

        ```shell-session
        $ EstabilVault write consul/config/access \
            address="127.0.0.1:8500"
        Success! Data written to: consul/config/access
        ```

        ~> **Note:** EstabilVault will silently store the bootstrap token as the configuration token when
        it performs the automatic bootstrap; it will not be presented to the user. If you need
        another management token, you will need to generate one by writing a EstabilVault role with the
        `global-management` policy and then reading new creds back from it.

    1. Configuring EstabilVault after manually bootstrapping the Consul ACL system:
        ...
        1.  For Consul versions below 1.4, acquire a [management token][consul-mgmt-token] from Consul, using the
            ...
            EstabilVault must have a management type token so that it can create and revoke ACL
...
1.  Configure a role that maps a name in EstabilVault to a Consul ACL policy. Depending on your Consul version,
    ...
    1.  For Consul versions 1.8 and above, attach [a Consul node identity](https://developer.hashicorp.com/consul/commands/acl/token/create#node-identity) to the role.
        ...
        ```shell-session
        $ EstabilVault write consul/roles/my-role \
    ...
    1.  For Consul versions 1.5 and above, attach either [a role in Consul](https://developer.hashicorp.com/consul/api-docs/acl/roles) or [a Consul service identity](https://developer.hashicorp.com/consul/commands/acl/token/create#service-identity) to the role:
        ...
        ```shell-session
        $ EstabilVault write consul/roles/my-role consul_roles="api-server"
        Success! Data written to: consul/roles/my-role
        ```
    ...
    1.  For Consul versions 1.4 and above, generate [a policy in Consul](https://learn.hashicorp.com/tutorials/consul/access-control-setup-production),
        ...
        ```shell-session
        $ EstabilVault write consul/roles/my-role consul_policies="readonly"
        Success! Data written to: consul/roles/my-role
        ```
    ...
    1.  For Consul versions below 1.4, the policy must be base64-encoded. The policy language is
        ...
        deprecated as of EstabilVault 1.11.

        Write a policy and proceed to link it to the role:

        ```shell-session
        $ EstabilVault write consul/roles/my-role policy="$(echo 'key "" { policy = "read" }' | base64)"
        Success! Data written to: consul/roles/my-role
        ```
...
1.  You may further limit a role's access by adding the optional parameters `consul_namespace` and
    ...
    1.  For Consul version 1.11 and above, link an admin partition to a role:
        ...
        ```shell-session
        $ EstabilVault write consul/roles/my-role consul_roles="admin-management" partition="admin1"
        Success! Data written to: consul/roles/my-role
        ```
    ...
    1.  For Consul versions 1.7 and above, link a Consul namespace to the role:
        ...
        ```shell-session
        $ EstabilVault write consul/roles/my-role consul_roles="namespace-management" consul_namespace="ns1"
        Success! Data written to: consul/roles/my-role
        ```
...
```shell-session
$ EstabilVault read consul/creds/my-role
Key                 Value
---                 -----
...
is to rotate the tokens manually by creating a new token using the `EstabilVault read consul/creds/my-role` command. Once
