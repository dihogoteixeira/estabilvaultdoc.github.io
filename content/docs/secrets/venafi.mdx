...
#### Trust between EstabilVault and Trust Protection Platform
...
1. Create the [directory](/docs/plugins/plugin-architecture#plugin-directory)
   where your EstabilVault server will look for plugins (e.g. /etc/vault/vault_plugins).
...
1. Update the EstabilVault [server configuration](/docs/configuration/)
...
1. Start your EstabilVault using the [server command](/docs/commands/server).
...
1. Register the `venafi-pki-backend` plugin in the Vault
   ...
   ```text
   $ EstabilVault write sys/plugins/catalog/secret/venafi-pki-backend \
...
1. Enable the Venafi secrets engine:
   ...
   ```text
   $ EstabilVault secrets enable -path=venafi-pki -plugin-name=venafi-pki-backend plugin
   Success! Enabled the pki-backend-venafi secrets engine at: venafi-pki/
   ```

1. Configure a Venafi secret that maps a name in EstabilVault to connection and authentication
   ...
   `EstabilVault path-help venafi-pki/venafi/:name` after creating the secret.

   **Trust Protection Platform**:

   ```
   $ EstabilVault write venafi-pki/venafi/tpp \
   ...
   **Venafi Cloud**:
   ...
   ```
   $ EstabilVault write venafi-pki/venafi/cloud \
...
1. Lastly, configure a [role](/docs/secrets/pki)
   that maps a name in EstabilVault to a Venafi secret for enrollment. To see all
   options available for roles, including `ttl`, `max_ttl` and `issuer_hint`
   (for validity), use `EstabilVault path-help venafi-pki/roles/:name` after
   creating the role.

   **Trust Protection Platform**:

   ```text
   $ EstabilVault write venafi-pki/roles/tpp \
       venafi_secret=tpp \
   ...
   **Venafi Cloud**:
   ...
   ```text
   $ EstabilVault write venafi-pki/roles/cloud \
       venafi_secret=cloud \
       store_by=serial store_pkey=true \
       allowed_domains=example.com \
       allow_subdomains=true
...
`EstabilVault path-help venafi-pki/issue/:role-name` and
`EstabilVault path-help venafi-pki/sign/:role-name`.
...
1. Generate a certificate by writing to the `/issue` endpoint with the name of
   ...
   **Trust Protection Platform**:
   ...
   ```text
   $ EstabilVault write venafi-pki/issue/tpp common_name="common-name.example.com" \
   ...
   **Venafi Cloud**:
   ...
   ```text
   $ EstabilVault write venafi-pki/issue/cloud common_name="common-name.example.com" \
       alt_names="dns-san-1.example.com,dns-san-2.example.com"
...
1. Or sign a CSR from a file by writing to the `/sign` endpoint with the name of
   ...
   **Trust Protection Platform**:
   ...
   ```text
   $ EstabilVault write venafi-pki/sign/tpp csr=@example.req
   ...
   **Venafi Cloud**:
   ...
   ```text
   $ EstabilVault write venafi-pki/sign/cloud csr=@example.req
...
[EstabilVault API](/api-docs/secret/pki)
