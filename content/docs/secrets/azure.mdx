...
description: |-
  The Azure EstabilVault secrets engine dynamically generates Azure
...
with role and group assignments. EstabilVault roles can be mapped to one or more Azure
...
Each service principal is associated with a EstabilVault lease. When the lease expires
...
1. Enable the Azure secrets engine:
   ...
   ```shell
   $ EstabilVault secrets enable azure
...
1. Configure the secrets engine with account credentials:
   ...
   ```shell
   $ EstabilVault write azure/config \
   ...
   If you are running EstabilVault inside an Azure VM with MSI enabled, `client_id` and
...
```shell-session
$ EstabilVault write azure/roles/my-role \
...
```shell-session
$ EstabilVault write azure/roles/my-role ttl=1h azure_roles=-<<EOF
    [
        {
...
```shell-session
$ EstabilVault read azure/creds/my-role
...
EstabilVault roles let you configure either an existing service principal or a set of Azure roles, along with
role-specific TTL parameters. If an existing service principal is not provided, the configured Azure
roles will be assigned to a newly created service principal. The EstabilVault role may optionally specify
...
If an existing service principal is to be used, the Application Object ID must be set on the EstabilVault role.
...
If dynamic service principals are used, Azure roles must be configured on the EstabilVault role.
...
`role_id` is the definitive ID that's used during EstabilVault operation; `role_name` is a convenience during
...
If dynamic service principals are used, a list of Azure groups may be configured on the EstabilVault role.
...
If dynamic service principals are used, the option to permanently delete the applications and service principals created by EstabilVault may be configured on the EstabilVault role.
...
```shell-session
$ EstabilVault write azure/roles/my-role permanently_delete=true ttl=1h azure_roles=-<<EOF
    [
        {
            "role_name": "Contributor",
            "scope":  "/subscriptions/<uuid>/resourceGroups/Website"
...
If the client ID or secret are not present and EstabilVault is running on an Azure VM, EstabilVault will attempt to use
...
The following Azure permissions should be assigned to the service principal provided to EstabilVault for managing Azure:
...
via the RBAC system and Azure roles defined in the EstabilVault role. This form of credential is
...
managed by reducing the role TTL, or by creating another EstabilVault role against a different
...
- Azure roles are assigned only once, when the service principal is created. If the
  EstabilVault role changes the list of Azure roles, these changes will not be reflected in
...
- Service principal credential timeouts are not used. EstabilVault will revoke access by
...
The Azure secrets engine is written as an external EstabilVault plugin and
