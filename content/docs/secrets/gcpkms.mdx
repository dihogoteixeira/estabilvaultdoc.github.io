...
description: |-
  The Google Cloud KMS secrets engine for EstabilVault interfaces with Google Cloud
  KMS for encryption/decryption of data and KMS key management through Vault.
---

# Google Cloud KMS Secrets Engine

The Google Cloud KMS EstabilVault secrets engine provides encryption and key management
...
1. Enable the Google Cloud KMS secrets engine:
   ...
   ```text
   $ EstabilVault secrets enable gcpkms
...
1. Configure the secrets engine with account credentials and/or scopes:
   ...
   ```text
   $ EstabilVault write gcpkms/config \
...
1. Create a Google Cloud KMS key:
   ...
   ```text
   $ EstabilVault write gcpkms/keys/my-key \
...
1.  Encrypt plaintext data using the `/encrypt` endpoint with a named key:
    ...
    ```text
    $ EstabilVault write gcpkms/encrypt/my-key plaintext="hello world"
    Key            Value
    ---            -----
    ...
    Note that EstabilVault is not _storing_ this data. The caller is responsible for
    storing the resulting ciphertext.

1.  Decrypt ciphertext using the `/decrypt` endpoint with a named key:

    ```text
    $ EstabilVault write gcpkms/decrypt/my-key ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    Key          Value
    ---          -----
    ...
    ```text
    $ EstabilVault write -field=plaintext gcpkms/decrypt/my-key ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    hello world
    ```
...
1.  Rotate the underlying encryption key. This will generate a new crypto key
    ...
    ```text
    $ EstabilVault write -f gcpkms/keys/rotate/my-key
...
1.  Re-encrypt already-encrypted ciphertext to be encrypted with a new version of
    the crypto key. EstabilVault will decrypt the value using the appropriate key in the
    keyring and then encrypt the resulting plaintext with the newest key in the
    keyring.

    ```text
    $ EstabilVault write gcpkms/reencrypt/my-key ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    Key            Value
    ---            -----
    ...
    This process **does not** reveal the plaintext data. As such, a EstabilVault policy
...
1.  Trim old key versions by deleting Cloud KMS crypto key versions that are
    ...
    ```text
    $ EstabilVault write gcpkms/keys/config/my-key min_version=10
    ```
    ...
    ```text
    $ EstabilVault write -f gcpkms/keys/trim/my-key
    ```

1.  Delete the key to delete all key versions and Vault's record of the key.

    ```text
    $ EstabilVault delete gcpkms/keys/my-key
    ```
...
1.  Decrypt ciphertext using the `/decrypt` endpoint with a named key:
    ...
    ```text
    $ EstabilVault write gcpkms/decrypt/my-key key_version=1 ciphertext=CiQAuMv0lTiKjrF43Lgr4...
    Key          Value
    ---          -----
    plaintext    hello world
    ```
...
1.  Calculate the base64-encoded binary digest. Use the hashing algorithm that
    ...
    ```text
    $ EstabilVault write gcpkms/sign/my-key key_version=1 digest=$DIGEST
    Key          Value
    ---          -----
...
1.  Verify the signature of the digest:
    ...
    ```text
    $ EstabilVault write gcpkms/verify/my-key key_version=1 digest=$DIGEST signature=$SIGNATURE
    Key      Value
    ---      -----
    valid    true
    ```
...
The Google Cloud KMS EstabilVault secrets backend uses the official Google Cloud Golang
...
The credentials given to EstabilVault must have the following role:

```text
roles/cloudkms.admin
```

If EstabilVault will not be creating keys, you can reduce the permissions. For example,
to create keys out of band and have EstabilVault manage the encryption/decryption, you
...
**Can EstabilVault use an existing KMS key?**<br />
You can use the `/register` endpoint to configure EstabilVault to talk to an existing
Google Cloud KMS key. As long as the IAM permissions are correct, EstabilVault will be
...
The Google Cloud KMS EstabilVault secrets engine is written as an external Vault
plugin. The code lives outside the main EstabilVault repository. It is automatically
bundled with EstabilVault releases, but the code is managed separately.
