...
non-string values by writing the key/value pairs to EstabilVault from a JSON file or
using the HTTP API.
...
```shell-session
$ EstabilVault secrets enable -version=2 kv
```

Or, you can pass `kv-v2` as the secrets engine type:

```shell-session
$ EstabilVault secrets enable kv-v2
```
...
```shell-session
$ EstabilVault kv enable-versioning secret/
...
1. Write arbitrary data:
   ...
   ```shell-session
   $ EstabilVault kv put -mount=secret my-secret foo=a bar=b
   Key              Value
   ---              -----
...
1. Read arbitrary data:
   ...
   ```shell-session
   $ EstabilVault kv get -mount=secret my-secret
   ====== Metadata ======
   Key              Value
   ---              -----
   created_time     2019-06-19T17:20:22.985303Z
...
1. Write another version, the previous version will still be accessible. The
  ...
  current version.
   ...
   ```shell-session
   $ EstabilVault kv put -mount=secret -cas=1 my-secret foo=aa bar=bb
   Key              Value
   ---              -----
...
1. Partial updates can be accomplished using the `EstabilVault kv patch` command. A
   ...
   ```shell-session
   $ EstabilVault kv patch -mount=secret -cas=2 my-secret bar=bbb
   Key              Value
   ---              -----
...
1. The `EstabilVault kv patch` command also supports a `-method` flag which can be
   ...
   ```shell-session
   $ EstabilVault kv patch -mount=secret -method=patch -cas=2 my-secret bar=bbb
   Key              Value
   ---              -----
   created_time     2019-06-19T17:23:49.199802Z
   custom_metadata  <nil>
   ...
   ```shell-session
   $ EstabilVault kv patch -mount=secret -method=rw my-secret bar=bbb
   Key              Value
   ---              -----
   created_time     2019-06-19T17:23:49.199802Z
   custom_metadata  <nil>
...
3. Previous versions can be accessed with the `-version` flag:
   ...
   ```shell-session
   $ EstabilVault kv get -mount=secret -version=1 my-secret
   ====== Metadata ======
   Key              Value
   ---              -----
   created_time     2019-06-19T17:20:22.985303Z
...
When deleting data the standard `EstabilVault kv delete` command will perform a
...
which allows the version to be undeleted. The `EstabilVault kv undelete` command
handles undeleting versions.

A version's data is permanently deleted only when the key has more versions than
are allowed by the max-versions setting, or when using `EstabilVault kv destroy`. When
...
1. The latest version of a key can be deleted with the delete command, this also
   ...
   ```shell-session
   $ EstabilVault kv delete -mount=secret my-secret
...
1. Versions can be undeleted:
   ...
   ```shell-session
   $ EstabilVault kv undelete -mount=secret -versions=2 my-secret
...
1. Destroying a version permanently deletes the underlying data:
   ...
   ```shell-session
   $ EstabilVault kv destroy -mount=secret -versions=2 my-secret
...
1. All metadata and versions for a key can be viewed:
   ...
   ```shell-session
   $ EstabilVault kv metadata get -mount=secret my-secret
   ========== Metadata ==========
   Key                     Value
   ---                     -----
   cas_required            false
...
1. The metadata settings for a key can be configured:
   ...
   ```shell-session
   $ EstabilVault kv metadata put -mount=secret -max-versions 2 -delete-version-after="3h25m19s" my-secret
   ...
   ```shell-session
   $ EstabilVault kv put -mount=secret my-secret my-value=newer-s3cr3t
   Key              Value
   ---              -----
   ...
   The `EstabilVault kv metadata put` command can be used to fully overwrite the value of `custom_metadata`:

   ```shell-session
   $ EstabilVault kv metadata put -mount=secret -custom-metadata=foo=abc -custom-metadata=bar=123 my-secret
   Success! Data written to: secret/metadata/my-secret
   ...
   The `EstabilVault kv metadata patch` command can be used to partially overwrite the value of `custom_metadata`.
   The following invocation will update `custom_metadata` sub-field `foo` but leave `bar` untouched:

   ```shell-session
   $ EstabilVault kv metadata patch -mount=secret -custom-metadata=foo=def my-secret
   Success! Data written to: secret/metadata/my-secret
...
1. Permanently delete all metadata and versions for a key:
   ...
   ```shell-session
   $ EstabilVault kv metadata delete -mount=secret my-secret
