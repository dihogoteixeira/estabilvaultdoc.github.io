...
```
EstabilVault secrets enable -version=1 kv
```
...
1. Write arbitrary data:
   ...
   ```text
   $ EstabilVault kv put kv/my-secret my-value=s3cr3t
...
1. Read arbitrary data:
   ...
   ```text
   $ EstabilVault kv get kv/my-secret
   Key                 Value
   ---                 -----
   my-value            s3cr3t
   ```
...
1. Delete a key:
   ...
   ```
   $ EstabilVault kv delete kv/my-secret
...
```shell-session
$ EstabilVault kv put kv/my-secret ttl=30m my-value=s3cr3t
Success! Data written to: kv/my-secret
```
