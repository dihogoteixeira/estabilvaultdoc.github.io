...
1.  Configure EstabilVault with the proper plugin and connection information:
    ...
    In this case, we've configured EstabilVault with the user "vaultuser" and password
    "yourStrong(!)Password", connecting to an instance at "localhost" on port 1433. It is
    not necessary that EstabilVault has the vaultuser login, but the user must have privileges
    ...
    users and grant permissions in the databases that EstabilVault manages. The fixed
...
EstabilVault will execute the default revocation procedure.
...
First, we mount a database backend at the azuresql path with `EstabilVault secrets enable -path=azuresql database`. Then we configure a connection called "testvault" to connect to a database called "test-vault", using "azuresql" at the beginning of our path:

```shell-session
$ EstabilVault write azuresql/config/testEstabilVault \
    plugin_name=mssql-database-plugin \
...
```shell-session
$ EstabilVault write azuresql/roles/test \
    db_name=testEstabilVault \
...
```shell-session
$ EstabilVault read azuresql/creds/test
Key            	Value
---            	-----
...
When we no longer need the backend, we can unmount it with `EstabilVault unmount azuresql`. Now, you can use the MSSQL Database Plugin with your Azure SQL databases.
...
```shell
EstabilVault write database/roles/my-role revocation_statements="\
   USE my_database; \
   IF EXISTS \
     (SELECT name \
