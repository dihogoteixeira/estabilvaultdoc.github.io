...
1.  Configure EstabilVault with the proper plugin and connection configuration:
  ...
  ```shell-session
  $ EstabilVault write database/config/my-redis-elasticache-cluster \
...
1.  Configure a static role that maps a name in EstabilVault to an existing Redis ElastiCache user.
