...
1.  Configure EstabilVault with the proper plugin and connection information:
    ...
    ```text
    $ EstabilVault write database/config/my-mongodb-database \
...
1.  Configure a role that maps a name in EstabilVault to a MongoDB command that executes and
...
from MongoDB with the exception that the EstabilVault parameters are the contents of those files, not filenames. As such,
