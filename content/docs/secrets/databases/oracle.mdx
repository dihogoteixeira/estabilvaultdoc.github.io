...
~> The Oracle database plugin is not bundled in the core EstabilVault code tree and can be
...
The Oracle Database Plugin does not live in the core EstabilVault code tree and can be found
...
If you are running EstabilVault with [mlock enabled](/docs/configuration#disable_mlock),
you will need to enable ipc_lock capabilities for the plugin binary.

1.  Enable the database secrets engine if it is not already enabled:

    ```shell
    $ EstabilVault secrets enable database
...
1.  Download and register the plugin:
    ...
    ```shell
    $ EstabilVault write sys/plugins/catalog/database/oracle-database-plugin \
        sha256="..." \
        command=vault-plugin-database-oracle
    ```

1.  Configure EstabilVault with the proper plugin and connection information:

    ```shell
    $ EstabilVault write database/config/my-oracle-database \
...
1. It is highly recommended that you immediately rotate the "root" user's password, see
   ...
   This will ensure that only EstabilVault is able to access the "root" user that EstabilVault uses to
   ...
   recommended that you create a user for EstabilVault to utilize rather than using the actual root user.

1. Configure a role that maps a name in EstabilVault to an SQL statement to execute to
   create the database credential:

   ```shell
   $ EstabilVault write database/roles/my-role \
   ...
   Note: The `creation_statements` may be specified in a file and interpreted by the EstabilVault CLI using the `@` symbol:
...
If the Oracle server EstabilVault is trying to connect to uses an SSL listener, the database
plugin will require additional configuration using the `connection_url` parameter:

```shell
EstabilVault write database/config/oracle \
...
The wallet used by EstabilVault should be in a well known location with the proper filesystem permissions. For example, if EstabilVault is running as the `vault` user,
the wallet directory may be setup as follows:

```shell
mkdir -p /etc/vault/wallets
cp cwallet.sso /etc/vault/wallets/cwallet.sso
chown -R vault:EstabilVault /etc/vault
...
be available on every EstabilVault server when using high availability clusters.

EstabilVault can optionally use TNS Names in the connection string when connecting to Oracle databases using a `tnsnames.ora` file. An example
...
To configure EstabilVault to use TNS names, set the following environment variable on the EstabilVault server:

```shell
TNS_ADMIN=/path/to/tnsnames/directory
```

~> **Note**: If EstabilVault returns a "could not open file" error, double check that
the `TNS_ADMIN` environment variable is available to the EstabilVault server.

Finally, use the alias in the `connection_url` parameter on the database configuration:

```
EstabilVault write database/config/oracle-east \
    plugin_name=vault-plugin-database-oracle \
...
EstabilVault write database/config/oracle-west \
    plugin_name=vault-plugin-database-oracle \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ EstabilVault read database/creds/my-role
    Key                Value
    ---                -----
