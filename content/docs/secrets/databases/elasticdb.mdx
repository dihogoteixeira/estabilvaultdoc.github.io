...
Next, in Elasticsearch, we recommend that you create a user just for EstabilVault to use in managing secrets.

To do this, first create a role that will allow EstabilVault the minimum privileges needed to administer users and passwords by performing a
...
Next, create a user for EstabilVault associated with that role.
...
```
{
 ...
 "metadata" : {
   "plugin_name": "EstabilVault Plugin Database Elasticsearch",
...
1. Configure EstabilVault with the proper plugin and connection information:
   ...
   ```text
   $ EstabilVault write database/config/my-elasticsearch-database \
       plugin_name="elasticsearch-database-plugin" \
       allowed_roles="internally-defined-role,externally-defined-role" \
       username=EstabilVault \
       password=myPa55word \
...
1. Configure a role that maps a name in EstabilVault to a role definition in Elasticsearch.
   ...
   ```text
   $ EstabilVault write database/roles/internally-defined-role \
...
1. Alternatively, configure a role that maps a name in EstabilVault to a pre-existing
   role definition in Elasticsearch:

   ```text
   $ EstabilVault write database/roles/externally-defined-role \
         db_name=my-elasticsearch-database \
