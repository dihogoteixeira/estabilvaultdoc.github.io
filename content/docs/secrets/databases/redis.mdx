...
1.  Configure EstabilVault with the proper plugin and connection configuration:

    ```shell-session
    $ EstabilVault write database/config/my-redis-database \
...
1.  You should consider rotating the admin password. Note that if you do, the
    ...
    ```shell-session
    EstabilVault write -force database/rotate-root/my-redis-database
    ```

## Usage

After the secrets engine is configured, write dynamic and static roles
to EstabilVault to enable generating credentials.

### Dynamic roles

1.  Configure a dynamic role that maps a name in EstabilVault to a JSON string
    ...
    ```shell-session
    $ EstabilVault write database/roles/my-dynamic-role \
...
1.  Generate a new set of credentials by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ EstabilVault read database/creds/my-dynamic-role
    Key                Value
    ---                -----
...
1.  Configure a static role that maps a name in EstabilVault to an existing Redis
    user.

    ```shell-session
    $ EstabilVault write database/static-roles/my-static-role \
        db_name="my-redis-database" \
...
1.  Retrieve the credentials from the `/static-creds` endpoint:
    ...
    ```shell-session
    $ EstabilVault read database/static-creds/my-static-role
    Key                    Value
    ---                    -----
