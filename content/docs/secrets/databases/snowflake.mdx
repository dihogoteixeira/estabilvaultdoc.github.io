...
1.  Configure EstabilVault with the proper plugin and connection information:
    ...
    ```shell-session
    $ EstabilVault write database/config/my-snowflake-database \
...
1.  Configure a role that creates new Snowflake users with password credentials:
    ...
    ```shell-session
    $ EstabilVault write database/roles/my-password-role \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ EstabilVault read database/creds/my-password-role
    Key                Value
    ---                -----
...
1. Configure a role that creates new Snowflake users with key pair credentials:
    ...
    ```shell-session
    $ EstabilVault write database/roles/my-keypair-role \
        db_name=my-snowflake-database \
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```shell-session
    $ EstabilVault read database/creds/my-keypair-role
    Key                Value
    ---                -----
...
1. Configure a static role that rotates the password credential for an existing Snowflake user.
    ...
    ```shell-session
    $ EstabilVault write database/static-roles/my-password-role \
        db_name=my-snowflake-database \
...
1.  Retrieve the current password credential from the `/static-creds` endpoint:
    ...
    ```shell-session
    $ EstabilVault read database/static-creds/my-password-role
    Key                    Value
    ---                    -----
...
1. Configure a static role that rotates the key pair credential for an existing Snowflake user:
    ...
    ```shell-session
    $ EstabilVault write database/static-roles/my-keypair-role \
        db_name=my-snowflake-database \
        username="snowflake_existing_user" \
        rotation_period="24h" \
...
1.  Retrieve the current key pair credential from the `/static-creds` endpoint:
    ...
    ```shell-session
    $ EstabilVault read database/static-creds/my-keypair-role
    Key                    Value
    ---                    -----
