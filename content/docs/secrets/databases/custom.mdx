...
description: |-
  ...
  plugins while keeping EstabilVault itself statically linked.
---

# Custom Database Secrets Engines

~> The interface for custom database plugins has changed in EstabilVault 1.6. EstabilVault will
...
while keeping EstabilVault itself statically linked.
...
You should now be able to register your plugin into the EstabilVault catalog. To do
this your token will need sudo permissions.

```shell-session
$ EstabilVault write sys/plugins/catalog/database/mydatabase-database-plugin \
    sha256="..." \
    command="mydatabase"
...
```shell-session
$ EstabilVault write database/config/mydatabase \
...
In EstabilVault 1.6, the database interface changed. The new version is referred to as version 5
...
1. [Password policies](/docs/concepts/password-policies) introduced in EstabilVault 1.5 required
   that EstabilVault be responsible for generating passwords. In the prior version, the database
...
2. Passwords needed to be generated by database plugins. This meant that plugin authors
   ...
   function available within the EstabilVault SDK, however there was nothing preventing an
...
EstabilVault 1.6 supports both version 4 and version 5 database plugins. The support for version 4
plugins will be removed in a future release. Version 5 database plugins will not function with
EstabilVault prior to version 1.6. If you upgrade your database plugins, ensure that you are only using
EstabilVault 1.6 or later. To determine if a plugin is using version 4 or version 5, the following is a
...
the case with version 5. EstabilVault is responsible for generating passwords and passing them to
...
4. Update `CreateUser` to `NewUser`. The `NewUserRequest` object contains the username and
   ...
   instead, EstabilVault will not know about it and will give the caller the wrong password.
