...
EstabilVault makes use of its own internal revocation system to ensure that users
become invalid within a reasonable time of the lease expiring.

### Static Roles

The database secrets engine supports the concept of "static roles", which are
a 1-to-1 mapping of EstabilVault Roles to usernames in a database. The current password
for the database user is stored and automatically rotated by EstabilVault on a
...
When credentials are requested for the Role, EstabilVault returns the current
password for the configured database user, allowing anyone with the proper
EstabilVault policies to have access to the user account in the database.
...
1. Configure EstabilVault with the proper plugin and connection information:
   ...
   ```shell-session
   $ EstabilVault write database/config/my-database \
       plugin_name="..." \
       connection_url="..." \
       allowed_roles="..." \
       username="..." \
   ...
   specifically for EstabilVault to use. This user will be used to manipulate
   dynamic and static users within the database. This user is called the
   "root" user within the documentation.

   EstabilVault will use the user specified here to create/update/revoke database
...
1. After configuring the root user, it is highly recommended you rotate that user's
   password such that the EstabilVault user is not accessible by any users other than
   EstabilVault itself:

   ```shell-session
   $ EstabilVault write -force database/rotate-root/my-database
   ```
   ...
   user is created specifically for EstabilVault to use to manage database
   users.

1. Configure a role that maps a name in EstabilVault to a set of creation statements to
...
As of EstabilVault 1.6, all databases support dynamic roles and static roles. All plugins except MongoDB Atlas support rotating
...
of dynamic and static roles configure the credential that EstabilVault will generate and
...
`EstabilVault write database/config/my-database` you can specify a password policy for all
...
As of EstabilVault 1.10, you can now specify the option `disable_escaping` with a value of `true ` in
some secrets engines to prevent EstabilVault from escaping special characters in the username and password
...
```shell-session
$ EstabilVault write database/config/my-mssql-database \
