...
1.  Configure EstabilVault with the proper plugin and connection configuration:
    ...
    ```bash
    $ EstabilVault write database/config/my-couchbase-database \
...
1.  You should consider rotating the admin password. Note that if you do, the
    ...
    ```bash
    EstabilVault write -force database/rotate-root/my-couchbase-database
    ```
...
1.  Configure a static role that maps a name in EstabilVault to an existing couchbase
    user.
