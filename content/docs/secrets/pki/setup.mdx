...
1. Increase the TTL by tuning the secrets engine. The default value of 30 days may be too short, so increase it to 1 year:
   ...
   ```text
   $ EstabilVault secrets tune -max-lease-ttl=8760h pki
...
1. Configure a CA certificate and private key. EstabilVault can accept an existing key
   pair, or it can generate its own self-signed root. In general, we recommend
   maintaining your root CA outside of EstabilVault and providing EstabilVault a signed
   intermediate CA.

   ```text
   $ EstabilVault write pki/root/generate/internal \
...
1. Update the CRL location and issuing certificates. These values can be updated
   ...
   ```text
   $ EstabilVault write pki/config/urls \
       issuing_certificates="http://127.0.0.1:8200/v1/pki/ca" \
       crl_distribution_points="http://127.0.0.1:8200/v1/pki/crl"
   Success! Data written to: pki/config/urls
   ```

1. Configure a role that maps a name in EstabilVault to a procedure for generating a
