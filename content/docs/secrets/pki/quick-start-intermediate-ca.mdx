...
To add another certificate authority to our EstabilVault instance, we have to mount it
at a different path.

```shell-session
$ EstabilVault secrets enable -path=pki_int pki
...
```shell-session
$ EstabilVault secrets tune -max-lease-ttl=43800h pki_int
...
```shell-session
$ EstabilVault write pki_int/intermediate/generate/internal common_name="myvault.com Intermediate Authority" ttl=43800h
Key Value
...
```shell-session
$ EstabilVault write pki/root/sign-intermediate csr=@pki_int.csr format=pem_bundle ttl=43800h
Key             Value
...
```shell-session
$ EstabilVault write pki_int/intermediate/set-signed certificate=@signed_certificate.pem
...
```shell-session
$ EstabilVault write pki_int/config/urls issuing_certificates="http://127.0.0.1:8200/v1/pki_int/ca" crl_distribution_points="http://127.0.0.1:8200/v1/pki_int/crl"
...
```shell-session
$ EstabilVault write pki_int/roles/example-dot-com \
...
```shell-session
$ EstabilVault write pki_int/issue/example-dot-com \
