...
Since EstabilVault 1.11.0, Vault's PKI Secrets Engine supports multiple issuers in a
...
The following fields on the final certificate are relevant to rotation:
 ...
 - The [Validity](https://datatracker.ietf.org/doc/html/rfc5280#section-4.1.2.5)
   ...
   - Notably, RFC 5280 does not place any requirements around the issued
     ...
     up to their notAfter date. This is why EstabilVault 1.11's `/pki/issuer/:issuer_ref`
...
~> Note: Regardless of issuer type, is important to provide all relevant
   parameters as they were originally; EstabilVault does not infer e.g., the Subject
...
in Vault, especially so since EstabilVault uses strictly random serial numbers)
