...
on every path, use `EstabilVault path-help` after mounting the secrets engine.

## Client Key Signing

Before a client can request their SSH key be signed, the EstabilVault SSH secrets engine must
be configured. Usually a EstabilVault administrator or security team performs these
...
The following steps are performed in advance by a EstabilVault administrator, security
...
1.  Mount the secrets engine. Like all secrets engines in Vault, the SSH secrets engine
    ...
    ```text
    $ EstabilVault secrets enable -path=ssh-client-signer ssh
...
1.  Configure EstabilVault with a CA for signing client keys using the `/config/ca`
    endpoint. If you do not have an internal CA, EstabilVault can generate a keypair for
    you.

    ```text
    $ EstabilVault write ssh-client-signer/config/ca generate_signing_key=true
    Key             Value
    ---             -----
    ...
    ```text
    $ EstabilVault write ssh-client-signer/config/ca \
        private_key="..." \
        public_key="..."
    ```
...
1.  Add the public key to all target host's SSH configuration. This process can
    ...
    ```text
    $ EstabilVault read -field=public_key ssh-client-signer/config/ca > /etc/ssh/trusted-user-ca-keys.pem
    ```
...
1.  Create a named EstabilVault role for signing client keys.

    ~> **IMPORTANT NOTE:** Prior to Vault-1.9, if `"allowed_extensions"` is either empty or not specified in the role,
    EstabilVault will assume permissive defaults: any user assigned to the role may specify any arbitrary
    extension values as part of the certificate request to the EstabilVault server.
    ...
    ```text
    $ EstabilVault write ssh-client-signer/roles/my-role -<<"EOH"
    {
...
1.  Ask EstabilVault to sign your **public key**. This file usually ends in `.pub` and
    the contents begin with `ssh-rsa ...`.

    ```text
    $ EstabilVault write ssh-client-signer/sign/my-role \
    ...
    ```text
    $ EstabilVault write ssh-client-signer/sign/my-role -<<"EOH"
    {
...
1.  Save the resulting signed, public key to disk. Limit permissions as needed.
    ...
    ```text
    $ EstabilVault write -field=signed_key ssh-client-signer/sign/my-role \
...
1.  SSH into the host machine using the signed key. You must supply both the
    signed public key from EstabilVault **and** the corresponding private key as
...
1.  Mount the secrets engine. For the most security, mount at a different path from the
    ...
    ```text
    $ EstabilVault secrets enable -path=ssh-host-signer ssh
    Successfully mounted 'ssh' at 'ssh-host-signer'!
    ```

1.  Configure EstabilVault with a CA for signing host keys using the `/config/ca`
    endpoint. If you do not have an internal CA, EstabilVault can generate a keypair for
    you.

    ```text
    $ EstabilVault write ssh-host-signer/config/ca generate_signing_key=true
    Key             Value
    ---             -----
    public_key      ssh-rsa AAAAB3NzaC1yc2EA...
    ```
    ...
    ```text
    $ EstabilVault write ssh-host-signer/config/ca \
        private_key="..." \
        public_key="..."
    ```
...
1.  Extend host key certificate TTLs.
    ...
    ```text
    $ EstabilVault secrets tune -max-lease-ttl=87600h ssh-host-signer
    ```
...
1.  Create a role for signing host keys. Be sure to fill in the list of allowed
    ...
    ```text
    $ EstabilVault write ssh-host-signer/roles/hostrole \
        key_type=ca \
        algorithm_signer=rsa-sha2-256 \
        ttl=87600h \
...
1.  Sign the host's SSH public key.
    ...
    ```text
    $ EstabilVault write ssh-host-signer/sign/hostrole \
        cert_type=host \
...
1.  Set the resulting signed certificate as `HostCertificate` in the SSH
    ...
    ```text
    $ EstabilVault write -field=signed_key ssh-host-signer/sign/hostrole \
        cert_type=host \
...
1.  Retrieve the host signing CA public key to validate the host signature of
    ...
    ```text
    $ EstabilVault read -field=public_key ssh-host-signer/config/ca
    ```
...
1.  Set `default_user` in the role. If you are always authenticating as the same
    ...
    ```text
    $ EstabilVault write ssh/roles/my-role -<<"EOH"
    {
...
parameters as per the EstabilVault CLI and API steps demonstrated below.

```shell-extension
# Using CLI:
EstabilVault secrets enable -path=hosts-ca ssh
...
~> **IMPORTANT:** Do NOT add a private key password since EstabilVault can't decrypt it.
