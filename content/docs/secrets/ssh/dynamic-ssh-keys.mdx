...
description: |-
  ...
  credential request, EstabilVault creates a new SSH key pair and appends the
  newly-generated public key to the authorized_keys file for the configured
  username on the remote host. EstabilVault uses a configurable install script to
  achieve this.
---
...
request, EstabilVault creates a new SSH key pair and appends the newly-generated public
key to the `authorized_keys` file for the configured username on the remote
host. EstabilVault uses a configurable install script to achieve this.

The secrets engine does not prompt for `sudo` passwords; the `NOPASSWD` option for
sudoers should be enabled at all remote hosts for the EstabilVault administrative
user.

The private key returned to the user will be leased and can be renewed if
desired. Once the key is given to the user, EstabilVault will not know when it gets
used or how many time it gets used. Therefore, EstabilVault **WILL NOT** and cannot
audit the SSH session establishments.

When the credential lease expires, EstabilVault removes the secret key from the remote
machine.
...
1. _Audit logs are unreliable_: EstabilVault can only log when users request
...
Next, infrastructure configuration must be registered with EstabilVault via roles.
First, however, the shared secret key must be specified.

### Mount the secrets engine

```shell-session
$ EstabilVault secrets enable ssh
...
```shell-session
$ EstabilVault write ssh/keys/dev_key \
...
```shell-session
$ EstabilVault write ssh/roles/dynamic_key_role \
    key_type=dynamic \
    key=dev_key \
    admin_user=username \
...
into the EstabilVault binary, but it is straight forward to specify an alternate. The
...
```shell-session
$ EstabilVault write ssh/creds/dynamic_key_role ip=x.x.x.x
Key            	Value
...
can all be done with a single EstabilVault CLI command.

```shell-session
$ EstabilVault ssh -role dynamic_key_role username@<IP of remote host>
username@<IP of remote host>:~$
```
