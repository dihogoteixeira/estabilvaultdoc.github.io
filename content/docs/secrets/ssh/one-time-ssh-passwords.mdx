...
description: |-
  The One-Time SSH Password (OTP) SSH secrets engine type allows a EstabilVault server
...
The One-Time SSH Password (OTP) SSH secrets engine type allows a EstabilVault server to
...
An authenticated client requests credentials from the EstabilVault server and, if
...
the EstabilVault helper, which then validates the OTP with the EstabilVault server. The Vault
server then deletes this OTP, ensuring that it is only used once.

Since the EstabilVault server is contacted during SSH connection establishment, every
login attempt and the correlating EstabilVault lease information is logged to the audit
secrets engine.
...
documentation on every path, use `EstabilVault path-help` after mounting the secrets
engine.

### Drawbacks

The main concern with the OTP secrets engine type is the remote host's
connection to Vault; if compromised, an attacker could spoof the EstabilVault server
returning a successful request. This risk can be mitigated by using TLS for the
connection to EstabilVault and checking certificate validity; future enhancements to
...
```shell-session
$ EstabilVault write ssh/roles/otp_key_role \
    key_type=otp \
...
```shell-session
$ EstabilVault write ssh/creds/otp_key_role ip=x.x.x.x
Key            	Value
...
```shell-session
$ EstabilVault ssh -role otp_key_role -mode otp username@x.x.x.x
...
```shell-session
$ EstabilVault ssh -role otp_key_role -mode otp -strict-host-key-checking=no username@x.x.x.x
...
to learn how to use the EstabilVault SSH secrets engine to secure authentication and authorization for access to machines.
