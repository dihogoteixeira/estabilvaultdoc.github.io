...
description: |-
  The Google Cloud secrets engine for EstabilVault dynamically generates Google Cloud
  service account keys and OAuth tokens based on IAM policies.
---

# Google Cloud Secrets Engine

The Google Cloud EstabilVault secrets engine dynamically generates Google Cloud service
...
- **Automatic cleanup of GCP IAM service account keys** - each Service Account
  key is associated with a EstabilVault lease. When the lease expires (either during
...
(released with EstabilVault <= 0.11.1), a lease was generated with access tokens. If you're using
...
1.  Enable the Google Cloud secrets engine:
    ...
    ```text
    $ EstabilVault secrets enable gcp
...
1.  Configure the secrets engine with account credentials, or leave blank or unwritten
    ...
    ```text
    $ EstabilVault write gcp/config credentials=@my-credentials.json
    Success! Data written to: gcp/config
    ```

    If you are running EstabilVault from inside [Google Compute Engine][gce] or [Google
...
A roleset consists of a EstabilVault managed GCP Service account along with a set of IAM bindings
...
Starting with EstabilVault 1.8.0, existing permissive policies containing globs
...
```text
$ EstabilVault write gcp/roleset/my-token-roleset \
...
```text
$ EstabilVault write gcp/roleset/my-key-roleset \
    project="my-project" \
...
```text
$ EstabilVault write gcp/roleset/my-roleset
...
Static accounts are GCP service accounts that are created outside of EstabilVault and then provided to
EstabilVault to generate access tokens or keys. You can also use EstabilVault to optionally manage IAM bindings
...
```text
$ EstabilVault write gcp/static-account/my-token-account \
...
```text
$ EstabilVault write gcp/static-account/my-key-account \
    service_account_email="account@my-project.iam.gserviceaccount.com" \
    secret_type="service_account_key"  \
    bindings=-<<EOF
      resource "//cloudresourcemanager.googleapis.com/projects/my-project" {
...
```text
$ EstabilVault write gcp/static-account/my-account
    bindings=@mybindings.hcl
    ...
```
...
```shell-session
$ EstabilVault read gcp/roleset/my-token-roleset/token
...
```shell-session
$ EstabilVault read gcp/roleset/my-key-roleset/key
...
The Google Cloud EstabilVault secrets backend uses the official Google Cloud Golang
...
The credentials given to EstabilVault must have the following permissions when using rolesets at the
project level:
...
When using static accounts, EstabilVault must have the following permissions at the service account level:
...
When using rolesets or static accounts with bindings,  EstabilVault must have the following permissions:
...
- Assign the set of roles required to get resource-specific
  ...
  EstabilVault can manage service accounts and keys.
...
- Notice that BigQuery requires different permissions than other resource. This is
  ...
  This means to update access on the dataset, EstabilVault must be able to update the dataset's
  metadata.
...
- EstabilVault requires permissions to manage IAM bindings and service accounts

Advantages of static accounts:

- Can manage IAM bindings independently from the ones managed in Vault
- EstabilVault does not require permissions to IAM bindings and service accounts and only permissions
...
When generating OAuth access tokens, EstabilVault will still
...
creation fails and EstabilVault will attempt to rollback.
...
While EstabilVault will initially create and assign permissions to IAM service
...
EstabilVault roleset Service Accounts will have emails in the format:
...
The Google Cloud EstabilVault secrets engine is written as an external EstabilVault plugin and
thus exists outside the main EstabilVault repository. It is automatically bundled with
EstabilVault releases, but the code is managed separately.
...
Previous versions of this secrets engine (EstabilVault <= 0.11.1) created a lease for
