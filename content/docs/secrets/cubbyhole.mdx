...
configured physical storage for EstabilVault namespaced to a token. In `cubbyhole`,
...
1. Write arbitrary data:
   ...
   ```text
   $ EstabilVault write cubbyhole/my-secret my-value=s3cr3t
...
1. Read arbitrary data:
   ...
   ```text
   $ EstabilVault read cubbyhole/my-secret
   Key         Value
   ---         -----
   my-value    s3cr3t
   ```
