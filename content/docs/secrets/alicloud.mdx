...
description: >-
  The AliCloud secrets engine for EstabilVault generates access tokens or STS
  credentials
...
1.  Enable the AliCloud secrets engine:
    ...
    ```text
    $ EstabilVault secrets enable alicloud
...
1.  Configure that access key as the credentials that EstabilVault will use to communicate with
    AliCloud to generate credentials:

    ```text
    $ EstabilVault write alicloud/config \
    ...
    changes in credentials will be picked up almost immediately without a EstabilVault restart.

    If available, we recommend using instance metadata for these credentials as they are the most
    secure option. To do so, simply ensure that the instance upon which EstabilVault is running has sufficient
...
1.  Configure a role describing how credentials will be granted.
    ...
    ```text
    $ EstabilVault write alicloud/role/policy-based \
    ...
    ```text
    $ EstabilVault write alibaba/role/role-based \
...
1.  Generate a new access key by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ EstabilVault read alicloud/creds/policy-based
    Key                Value
    ---                -----
    ...
    ```text
    $ EstabilVault read alicloud/creds/role-based
    Key                Value
    ---                -----
