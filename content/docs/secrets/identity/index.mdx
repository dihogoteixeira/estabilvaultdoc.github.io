...
description: The Identity secrets engine for EstabilVault manages client identities.
---
...
to a single entity in EstabilVault that has 2 aliases, one of type GitHub and one of
type LDAP. When a client authenticates via any of the credential backends
(except the Token backend), EstabilVault creates a new entity and attaches a new
...
The EstabilVault Identity secrets engine supports several different features. Each
...
Additionally, EstabilVault can be configured as an OIDC identity provider. Please see
