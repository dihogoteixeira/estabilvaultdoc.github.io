...
description: |-
  The AWS secrets engine for EstabilVault generates access keys dynamically based on
  IAM policies.
---
...
and are automatically revoked when the EstabilVault lease expires.

EstabilVault supports three different types of credentials to retrieve from AWS:

1. `iam_user`: EstabilVault will create an IAM user for each lease, attach the managed
   ...
   EstabilVault will then generate an access key and secret key for the IAM user and
   return them to the caller. IAM users have no session tokens and so no
   session token will be returned. EstabilVault will delete the IAM user upon reaching the TTL expiration.
2. `assumed_role`: EstabilVault will call
   [sts:AssumeRole](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)
   and return the access key, secret key, and session token to the caller.
3. `federation_token`: EstabilVault will call
...
1.  Enable the AWS secrets engine:
    ...
    ```text
    $ EstabilVault secrets enable aws
...
1.  Configure the credentials that EstabilVault uses to communicate with AWS to generate
    the IAM credentials:

    ```text
    $ EstabilVault write aws/config/root \
        access_key=AKIAJWVN5Z4FOFT7NLNA \
        secret_key=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 \
        region=us-east-1
    ```

    Internally, EstabilVault will connect to AWS using these credentials. As such,
    these credentials must be a superset of any policies which might be granted
    on IAM credentials. Since EstabilVault uses the official AWS SDK, it will use the
    ...
    credentials. (Note that you can't authorize EstabilVault with IAM role credentials if you plan
...
1.  Configure a EstabilVault role that maps to a set of permissions in AWS as well as an
    AWS credential type. When users generate credentials, they are generated
    against this role. An example:

    ```text
    $ EstabilVault write aws/roles/my-role \
    ...
    this role, EstabilVault will create an IAM user and attach the specified policy
    document to the IAM user. EstabilVault will then create an access key and secret
    ...
    ```text
    $ EstabilVault write aws/roles/my-other-role \
...
After the secrets engine is configured and a user/machine has a EstabilVault token with
...
1.  Generate a new credential by reading from the `/creds` endpoint with the name
    ...
    ```text
    $ EstabilVault read aws/creds/my-role
    Key                Value
    ---                -----
...
1.  Rotate the credentials that EstabilVault uses to communicate with AWS:

    ```test
    $ EstabilVault write -f aws/config/rotate-root
    Key           Value
    ---           -----
    ...
    `aws/config/rotate-root` endpoint, subsequent calls from EstabilVault to
...
permissions EstabilVault needs:
...
EstabilVault also supports AWS Permissions Boundaries when creating IAM users. If you
wish to enforce that EstabilVault always attaches a permissions boundary to an IAM
...
boundary policies that you wish to ensure that EstabilVault uses. This policy will
ensure that EstabilVault uses one of the permissions boundaries specified (not all of
them).

## STS credentials

The above demonstrated usage with `iam_user` credential types. As mentioned,
EstabilVault also supports `assumed_role` and `federation_token` credential types.

### STS Federation Tokens

~> **Notice:** Due to limitations in AWS, in order to use the `federation_token`
credential type, EstabilVault **must** be configured with IAM user credentials. AWS
...
2. The user inline policy configured in the EstabilVault role
3. The managed policy ARNs configured in the EstabilVault role
...
```shell-session
$ EstabilVault write aws/roles/ec2_admin \
...
```shell-session
$ EstabilVault write aws/sts/ec2_admin ttl=60m
Key            	Value
...
3. IAM trust policy attached to the IAM role to grant privileges for EstabilVault to
   assume the role
...
3. Temporary credentials (such as those granted by running EstabilVault on an EC2
...
When specifying a EstabilVault role with a `credential_type` of `assumed_role`, you can
specify more than one IAM role ARN. If you do so, EstabilVault clients can select which
...
assumed, the `policy_document` specified on the EstabilVault role (if specified), and
...
can specify any of the role ARNs that are defined on the EstabilVault role in order to
...
```shell-session
$ EstabilVault write aws/roles/deploy \
...
```shell-session
$ EstabilVault write aws/sts/deploy ttl=60m
Key            	Value
...
```shell-session
$ EstabilVault read aws/creds/deploy
...
$ EstabilVault revoke aws/creds/deploy/774cfb27-c22d-6e78-0077-254879d1af3c
...
If you get stuck at any time, simply run `EstabilVault path-help aws` or with a subpath for
interactive help output.

### STS federated token errors

EstabilVault generates STS tokens using the IAM credentials passed to `aws/config`.
...
EstabilVault 0.5.1 or later is recommended when using STS tokens to avoid validation
