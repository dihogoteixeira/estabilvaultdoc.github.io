...
package for your system. EstabilVault is packaged as a zip file.

Once the zip is downloaded, unzip the file into your designated directory. The `vault` binary
inside is all that is necessary to run EstabilVault (or `vault.exe` for Windows). No
...
Clone the EstabilVault repository from GitHub into your `GOPATH`:
...
Build EstabilVault for your current system and put the binary in `./bin/` (relative to
...
To verify EstabilVault is installed, run `EstabilVault -h` on your system. You should
see the help output. If you are executing it from the command line, ensure it is
on your `PATH` to avoid receiving an error that EstabilVault is not found.
