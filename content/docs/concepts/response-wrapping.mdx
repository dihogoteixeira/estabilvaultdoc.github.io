...
introduced in EstabilVault 0.8 and may not be available in earlier releases.

## Overview

In many EstabilVault deployments, clients can access EstabilVault directly and consume
...
with most of the EstabilVault API and passing secrets to the end consumer.
...
To help address this problem, EstabilVault includes a feature called _response
wrapping_. When requested, EstabilVault can take the response it would have sent to an
...
When a response is wrapped, the normal API response from EstabilVault does not contain
...
- Wrapped Accessor: If the wrapped response is an authentication response
  containing a EstabilVault token, this is the value of the wrapped token's accessor.
...
EstabilVault currently does not provide signed response-wrapping tokens, as it
...
and pointed to the wrong EstabilVault server, the same attacker could trivially give
you the wrong signing public key that corresponds to the wrong EstabilVault server.
You could cache a previously valid key, but could also cache a previously valid
address (and in most cases the EstabilVault address will not change or will be set via
...
Response wrapping is per-request and is triggered by providing to EstabilVault the
...
(`25h`). When using the EstabilVault CLI, you can set this via the `-wrap-ttl`
