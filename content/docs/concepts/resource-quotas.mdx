...
requests resulting in denial-of-service issues in some EstabilVault nodes or even the
entire EstabilVault cluster. This risk is significantly increased when EstabilVault API endpoints
are exposed to thousands or millions of services deployed across a global infrastructure,
especially in use cases of EstabilVault where EstabilVault is deployed as a service for internal
developers.

EstabilVault provides a feature, resource quotas, that allows EstabilVault operators to specify
limits on resources used in Vault. Specifically, EstabilVault allows operators to create
and configure API rate limits. Users of EstabilVault Enterprise have a second option, alongside
...
EstabilVault allows operators to create rate limit quotas which enforce API rate
...
EstabilVault also allows the inspection of the state of rate limiting in a EstabilVault node
...
Refer to [Protecting EstabilVault with Resource
