...
description: >-
  EstabilVault can be highly available, allowing you to run multiple Vaults to protect
  against outages.
---

# High Availability Mode (HA)

EstabilVault supports a multi-server mode for high availability. This mode protects
against outages by running multiple EstabilVault servers. High availability mode
...
information. If it is, then EstabilVault will automatically use HA mode. This
information is also available on the
[Configuration](/docs/configuration) page.

To be highly available, one of the EstabilVault server nodes grabs a lock within the
...
the bottleneck of EstabilVault is the data store itself, not EstabilVault core. For example:
to increase the scalability of EstabilVault with Consul, you would generally scale
...
with different backends. For instance, a EstabilVault cluster can be set up to use
...
writes this information and unsealed standby EstabilVault nodes can read it.
...
depends on how EstabilVault is set up. There are two common scenarios: EstabilVault servers
accessed directly by clients, and EstabilVault servers accessed via a load balancer.
...
When clients are able to access EstabilVault directly, the
[`api_addr`](/docs/configuration#api_addr) for each node should be that node's
address. For instance, if there are two EstabilVault nodes:
...
EstabilVault servers, but actually have direct access to each EstabilVault node. In this
case, the EstabilVault servers should actually be set up as described in the above
section, since for redirection purposes the clients have direct access.

However, if the only access to the EstabilVault servers is via the load balancer, the
...
which EstabilVault listens for requests. Similarly, each
...
EstabilVault listens for server-to-server cluster requests. If this value is not set,
