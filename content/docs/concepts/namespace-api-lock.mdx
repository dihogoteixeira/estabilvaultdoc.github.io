...
description: Lock the EstabilVault API on a per-namespace basis.
---

# Namespace Lock and Unlock

EstabilVault makes the API available (unlocked) by default for all namespaces. In
this state, EstabilVault can respond to all API/CLI ('API' from here on out) requests
as normal.

A EstabilVault administrator can lock the API for particular namespaces. In this state,
EstabilVault blocks all but a selected few API endpoints from responding to clients
...
that lock will remain, but EstabilVault must be unlocked for the parent before
unlocking the descendant.

## Why?

Blocking access to much of EstabilVault can be an important break-glass tool in the
event of unexpected behavior.

For HCP Vault, this provides functionality analogous to sealing Vault, without
the EstabilVault administrator requesting that the Managed Service Provider seal/unseal
Vault.

This feature also becomes valuable for secure multitenancy in a variety of Vault
deployment strategies. You can restrict EstabilVault access for just part of an
...
The API can be locked by running `EstabilVault namespace lock` (or via the API) while
...
EstabilVault from a locked namespace, the returned error will inform that client of the
locking namespace.

## Unlocking

The API can be unlocked by running `EstabilVault namespace unlock` (or via the API)
...
EstabilVault responds with an HTTP 503: Service Unavailable. It will also produce the
following error:
