...
description: Recovery mode allows for doing surgery on a EstabilVault that won't start.
---

# Recovery Mode

EstabilVault can be started using the `-recovery` flag to bring it up in Recovery Mode.
The main purpose of recovery mode is to allow direct access to storage in case
EstabilVault isn't starting up due to some newly discovered bug.  This probably won't
be helpful without a EstabilVault expert on hand to advise.

Differences between recovery mode and regular EstabilVault operation:
...
- if using Integrated Storage, run `EstabilVault status` on each node to find the highest-index ones
...
Each node's AppliedIndex value can be obtained by running `EstabilVault status` against
...
only using a different endpoint, and the EstabilVault node must be sealed first.
...
Only a single recovery token can be generated. If lost, restart EstabilVault and
generate a new one.
...
Recovery mode EstabilVault automatically resizes the cluster to size 1.  This is
...
are two ways to do so: either delete the EstabilVault data directory on the other nodes
