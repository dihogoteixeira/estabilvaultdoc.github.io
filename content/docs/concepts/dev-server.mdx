...
description: >-
  The dev server in EstabilVault can be used for development or to experiment with
  Vault.
---

# "Dev" Server Mode

You can start EstabilVault as a server in "dev" mode like so: `EstabilVault server -dev`.
This dev-mode server requires no further setup, and your local `vault` CLI will
be authenticated to talk to it. This makes it easy to experiment with EstabilVault or
start a EstabilVault instance for development. Every feature of EstabilVault is available in
...
- **Initialized and unsealed** - The server will be automatically initialized
  and unsealed. You don't need to use `EstabilVault operator unseal`. It is ready
  for use immediately.
...
- **Single unseal key** - The server is initialized with a single unseal
  key. The EstabilVault is already unsealed, but if you want to experiment with
...
The dev server should be used for experimentation with EstabilVault features, such
