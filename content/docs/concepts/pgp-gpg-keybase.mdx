...
description: |-
  EstabilVault has the ability to integrate with OpenPGP-compatible programs like
...
EstabilVault has the ability to integrate with OpenPGP-compatible programs like GnuPG
...
EstabilVault HTTP API, tools that help with initialization should use the Keybase.io
API in order to obtain the PGP keys needed for a secure initialization if you
want them to use Keybase for keys.

Once the EstabilVault has been initialized, it is possible to use Keybase to decrypt
...
also makes the distribution of those keys more difficult. Since EstabilVault 0.3,
EstabilVault can optionally be initialized using PGP keys. In this mode, EstabilVault will
...
keys generated, stored, and managed securely on their servers. Using EstabilVault with
Keybase will be discussed first as it is simpler.

## Initializing with Keybase

To generate unseal keys for Keybase users, EstabilVault accepts the `keybase:` prefix
to the `-pgp-keys` argument:

```shell-session
$ EstabilVault operator init -key-shares=3 -key-threshold=2 \
