...
EstabilVault supports moving secrets and auth mounts, both within and across namespaces. The operation
...
There are two primary cleanup operations that EstabilVault operators will need to keep in mind after performing a remount.
...
the EstabilVault operator should find and remove those aliases from the source namespace, and create equivalent aliases
...
EstabilVault will dynamically generate them upon login operations.
...
which does not have a secrets engine called my-secret/, the EstabilVault operator would either create an equivalent my-secret/
