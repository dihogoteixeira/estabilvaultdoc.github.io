...
Authentication in EstabilVault is the process by which user or machine supplied
information is verified against an internal or external system. EstabilVault supports
...
EstabilVault supports a number of auth methods. Some backends are targeted
...
```shell-session
$ EstabilVault write sys/auth/my-auth type=userpass
```
...
```shell-session
$ EstabilVault path-help auth/my-auth
# ...
```

EstabilVault supports multiple auth methods simultaneously, and you can even
...
EstabilVault generates a unique access token for you to use for future requests.
...
To authenticate with the CLI, `EstabilVault login` is used. This supports many
of the built-in auth methods. For example, with GitHub:

```shell-session
$ EstabilVault login -method=github token=<token>
...
```
...
auth method implements its own login endpoint. Use the `EstabilVault path-help`
mechanism to find the proper endpoint.

For example, the GitHub login endpoint is located at `auth/github/login`.
And to determine the arguments needed, `EstabilVault path-help auth/github/login` can
be used.
...
completely reauthenticate. Just use `EstabilVault token renew <token>` with the
...
// Once you've set the token for your EstabilVault client, you will need to
...
//
// Additionally, enterprise EstabilVault users should be aware that due to eventual
// consistency, the API may return unexpected errors when running EstabilVault with
...
func login(client *vault.Client) (*vault.Secret, error) {
	...
	// log in to EstabilVault using various auth methods. This function is just
