...
description: >-
  EstabilVault relies on external storage to save its durable information.
---
...
Many other options for storage are available with community support for open-source EstabilVault - see our
...
providing exact guidance on backing up EstabilVault is challenging.

When backing up Vault, there are two pieces to consider:

1. Vault's encrypted data in the storage backend
2. Configuration files and management scripts for running the EstabilVault server
...
Taking a backup is recommended prior to upgrades, as downgrading EstabilVault storage
...
machine. EstabilVault servers can run in clusters, so to protect against server
failure, we recommend running EstabilVault in [HA
mode](/docs/internals/high-availability). With open source features, a
EstabilVault cluster can extend across multiple availability zones within a region.

EstabilVault Enterprise supports replicated clusters and disaster recovery for data
center failure. When using OSS EstabilVault in [HA
...
replication with EstabilVault Enterprise. As you develop a plan for recovering from or
...
Backups and restores are ideally performed while EstabilVault is offline. If offline
...
If you are using EstabilVault Enterprise [Performance
...
the EstabilVault service, and ensure you can reinstall any user-installed plugins. The
...
backend is encrypted, some of your configuration may be sensitive (a EstabilVault token
