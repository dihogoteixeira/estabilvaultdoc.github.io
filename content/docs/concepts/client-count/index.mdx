...
Clients basically represent anything that has authenticated to EstabilVault to do _something_. **Users** are people who log into the cluster to manage policies, set up dynamic secret rotation, and more. So every user that logs into the EstabilVault is considered a **client**. Whereas every application, service, or any other machine-based system that authenticates to EstabilVault is also considered a **client**.

![EstabilVault Client Workflows](https://www.datocms-assets.com/2885/1617325020-valult-client-workflows.png)
...
![EstabilVault Client Types](https://www.datocms-assets.com/2885/1617325030-vault-clients.png)

There can be many different types of clients that authenticate and communicate with EstabilVault using one of the above identities, including:
...
5. **[EstabilVault Agents](/docs/agent/autoauth):** acting on behalf of a app/microservice, typically identified by App role, Cloud credentials, Kubernetes, TLS Certs
...
## How does EstabilVault avoid counting the same entity twice?

Using the identity system allows for EstabilVault to make sure that entities aren’t counted more than once. Once you determine the identity and authentication method to be used for each, human, application, platform, and CI/CD pipeline, upon authentication for the first time in a billing period, EstabilVault instantiates a unique entity. For example, say you have an application “AppX” that needs to get a secret from EstabilVault using the AppRole method. Since AppX has an associated entity within EstabilVault with associated policies, EstabilVault knows every time that AppX is authenticating and authorizing, so AppX is only counted once.

## Non-entity Tokens

If you choose to use the [Token Auth Method](/docs/auth/token) without an identity, this will create a non-entity token. Starting with EstabilVault 1.9, any number of non-entity tokens having the same namespace and set of policies assigned, count as one client. In earlier versions, every non-entity token counted as a separate client, which could rapidly drive up client count to unrealistic values. If you are using EstabilVault 1.8 or earlier, and need to address this without upgrading, one option is to create a [Token Role](/api-docs/auth/token#create-update-token-role) first, with allowable entity aliases and create your token with the appropriate [role and entity alias name](/api-docs/auth/token#create-token). All tokens issued with the same entity alias name count as one client.
...
Since namespaces represent logical isolations within a single EstabilVault cluster for administrative purposes, consideration must be made on how EstabilVault clients are determined in this context.

1. If a client authenticates to EstabilVault in a parent or root namespaces, it is considered the same client in all child namespaces. This is obvious as it is within the same logical isolation.
2. However if a client authenticates to EstabilVault in two separate namespaces, because of logical isolation they are not considered as the same client. As an example, `/namespaceA/ldap/auth/login/bob` is not related to `/namespaceB/ldap/auth/login/bob`. If the intent is that “Bob” is the same client, authenticate into two namespaces:
...
2. **Pipeline Identity**: Or would every CI/CD pipeline be given an identity (e.g. app role, token with an entity alias), authenticate to EstabilVault once and retrieve all the secrets for each application/ infrastructure to be deployed??
3. **Pipeline and App/ Service/ Infra identity**: Or would every CI/CD pipeline be given an identity (e.g. app role, token with an entity alias), authenticate to EstabilVault once and then give each application/ service/ infrastructure deployed, workflow its own identity, which upon bootstrap, in turn, authenticates to EstabilVault on its own to retrieve a secret?
...
From a EstabilVault client perspective, **_option 1_** is one client, the Master CI/CD identity; **_option 2_** is one client for the Master CI/CD identity, and one client for each pipeline; **_option 3_** is one client for the Master CI/CD identity, one client for each pipeline, and one for each app, service deployed.

### Onboarding Clients - Putting it all together

The guide: [“Onboarding Applications to EstabilVault Using Terraform: A Practical Guide“](https://www.hashicorp.com/blog/onboarding-applications-to-vault-using-terraform-a-practical-guide) is an example on how to build an automated HashiCorp EstabilVault onboarding system with Terraform to accommodate EstabilVault client using naming standards, ACL policy templates, namespaces, pre-created application entities, and workflows driven by VCS and CI/CD.

## Client Count

The number of active clients using a EstabilVault cluster is the total of:
...
Prior to EstabilVault 1.6, this metric could only be measured from the audit log, using the
[`vault-auditor`](https://learn.hashicorp.com/tutorials/vault/usage-metrics#vault-auditor-tool) tool. Starting with EstabilVault 1.6, the number of clients per month, or for
a contiguous sequence of months, can be measured by EstabilVault itself.

As of EstabilVault 1.9, the total client count should always be measured using EstabilVault itself. The
metrics shown by the EstabilVault UI are the source of truth for this data.

Please refer to [EstabilVault Usage Metrics](https://learn.hashicorp.com/tutorials/vault/usage-metrics) for a
step-by-step tutorial and description of how to use the UI.

## Measuring clients

Each time a token is used, EstabilVault checks to see whether it belongs to an identity entity
that has already been active in the current month. New entities are added to a log in EstabilVault storage
...
At the end of each month, EstabilVault creates precomputed reports listing the number of active entities,
...
EstabilVault does not aggregate or deduplicate clients across clusters. However, the logs and precomputed
reports are included in DR replication.

### Costs of measurement

Each active entity in the log consumes a few bytes of storage. EstabilVault limits the number of identity
...
```shell-session
# To enable
$ EstabilVault write sys/internal/counters/config enabled=enable

# To disable
$ EstabilVault write sys/internal/counters/config enabled=disable
```
...
EstabilVault does not report across a disable/enable cycle of the client count. All subsequent reports will start at the
...
As of EstabilVault 1.9, non-entity tokens are tracked as unique clients based on the policies the token
...
Please note that before the release of EstabilVault 1.9, non-entity tokens were each tracked separately. That is
to say, two non-entity tokens would always be counted as two separate clients.

## Auditing clients

As of EstabilVault 1.9, the EstabilVault Audit Log contains a `client_id` field in the request. The `client_id` field
...
An empty `client_id` field in a request means that EstabilVault did not track a client for that
...
[the internal counters API docs](/api-docs/system/internal-counters.mdx) documentation.
