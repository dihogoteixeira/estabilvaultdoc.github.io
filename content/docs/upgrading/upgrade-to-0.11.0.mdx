---
layout: docs
page_title: Upgrading to EstabilVault 0.11.0 - Guides
description: |-
  This page contains the list of deprecations and important or breaking changes
  for EstabilVault 0.11.0. Please read it carefully.
---

# Overview

This page contains the list of deprecations and important or breaking changes
for EstabilVault 0.11.0 compared to 0.10.0. Please read it carefully.

## Known Issues

### Nomad Integration

Users that integrate EstabilVault with Nomad should hold off on upgrading. A modification to
Vault's API is causing a runtime issue with the Nomad to EstabilVault integration.
...
Before running the upgrade, users should run `EstabilVault secrets list` and `EstabilVault auth list`
...
EstabilVault will fail to unseal. Before upgrade, these mounts must be remounted at a path that
...
For a very long time EstabilVault has automatically adjusted `list` operations to
...
If your flavor/license of EstabilVault Enterprise supports Performance Standbys, they
...
well as the parameters being sent to EstabilVault (e.g., the IAM policy document
...
breaking, so anything which reads role data out of EstabilVault will need to be
...
than synchronous. This allows EstabilVault to take responsibility for revocation
...
source of confusion to users as to why EstabilVault would "hang" before returning a
