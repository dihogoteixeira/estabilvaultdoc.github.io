---
layout: docs
page_title: Upgrading EstabilVault - Guides
description: |-
  These are general upgrade instructions for EstabilVault for both non-HA and HA
  setups. Please ensure that you also read the version-specific upgrade notes.
---

# Upgrading Vault

These are general upgrade instructions for EstabilVault for both non-HA and HA setups.
_Please ensure that you also read any version-specific upgrade notes which can be
found in the sidebar._

!> **Important:** Always back up your data before upgrading! EstabilVault does not
make backward-compatibility guarantees for its data store. Simply replacing the
newly-installed EstabilVault binary with the previous version will not cleanly
...
EstabilVault upgrades are designed such that large jumps (ie 1.3.10 -> 1.7.x) are
...
EstabilVault 1.11 introduced [automated
upgrades](/docs/concepts/integrated-storage/autopilot#automated-upgrades) as
part of the Integrated Storage Autopilot feature. If your EstabilVault environment is
configured to use Integrated Storage, consider leveraging this new feature to
upgrade your EstabilVault environment.
...
The EstabilVault Agent is an API client of the EstabilVault Server. EstabilVault APIs are almost
always backwards compatible. When they are not, this is called out in the
upgrade guide for the new EstabilVault version, and there is a lengthy deprecation
period. The EstabilVault Agent version can lag behind the EstabilVault Server version, though
we recommend keeping all EstabilVault instances up to date with the most recent minor EstabilVault version
...
Upgrading to EstabilVault Enterprise installations follow the same steps as OSS upgrades except that the EstabilVault Enterprise binary is to be used and the license file [applied](/api-docs/system/license#install-license), when applicable. The Enterprise binary and license file can be obtained through your HashiCorp sales team.

## Non-HA Installations

Upgrading non-HA installations of EstabilVault is as simple as replacing the Vault
binary with the new version and restarting Vault. Any upgrade tasks that can be
performed for you will be taken care of when EstabilVault is unsealed.
...
version of EstabilVault greater than or equal to 1.11 and you have Autopilot enabled.
...
This is our recommended upgrade procedure if you're on a version of EstabilVault before
...
EstabilVault (standby + leader, leader-only, or discovered via service discovery), etc.

Whatever method you use, you should ensure that you never fail over from a
newer version of EstabilVault to an older version. Our suggested procedure is designed
to prevent this.

Please note that EstabilVault does not support true zero-downtime upgrades, but with
...
1. Properly shut down EstabilVault on the standby node via `SIGINT` or `SIGTERM`
2. Replace the EstabilVault binary with the new version; ensure that `mlock()` capability is added to the new binary with [setcap](/docs/configuration#disable_mlock)
3. Start the standby node
4. Unseal the standby node
5. Verify `EstabilVault status` shows correct Version and HA Mode is `standby`
...
1. Properly shut down the remaining (active) node
   ...
   If you kill EstabilVault without letting it release the lock, a standby node will
...
Upgrading installations of EstabilVault which participate in [Enterprise Replication](/docs/enterprise/replication) requires the following basic order of operations:
...
~> **Note:** It is not safe to replicate from a newer version of EstabilVault to an
older version. When upgrading replicated clusters, ensure that upstream clusters
are always on older versions of EstabilVault than downstream clusters.

Here is an example of upgrading four EstabilVault replicated EstabilVault clusters:
