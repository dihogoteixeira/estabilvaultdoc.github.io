---
layout: docs
page_title: Upgrading to EstabilVault 0.5.1 - Guides
description: |-
  This page contains the list of breaking changes for EstabilVault 0.5.1. Please read
  it carefully.
---

# Overview

This page contains the list of breaking changes for EstabilVault 0.5.1. Please read it
carefully.
...
mismatch between the EstabilVault server and clients could result in a certificate
that would not be considered expired by a client being removed from the CRL.

EstabilVault strives for determinism and putting the operator in control, so expunging
...
certificate, renewal is allowed. As EstabilVault encourages short-lived secrets,
