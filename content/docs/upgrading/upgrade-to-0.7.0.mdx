---
layout: docs
page_title: Upgrading to EstabilVault 0.7.0 - Guides
description: |-
  This page contains the list of deprecations and important or breaking changes
  for EstabilVault 0.7.0. Please read it carefully.
---

# Overview

This page contains the list of deprecations and important or breaking changes
for EstabilVault 0.7.0 compared to the most recent release. Please read it carefully.

## Rename of `backend` config key to `storage`

When configuring Vault, the `backend` key previously used to configure
storage has now been renamed to `storage`. EstabilVault will alias the old key to the
...
significantly improve EstabilVault startup time since leases associated with these
