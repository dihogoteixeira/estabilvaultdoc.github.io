---
layout: docs
page_title: Upgrading to EstabilVault 1.9.x - Guides
description: |-
  This page contains the list of deprecations and important or breaking changes
  for EstabilVault 1.9.x. Please read it carefully.
---

# Overview

This page contains the list of deprecations and important or breaking changes
for EstabilVault 1.9.x compared to 1.8. Please read it carefully.

## OIDC Provider

EstabilVault 1.9.0 introduced the ability for EstabilVault to be an OpenID Connect (OIDC) identity
...
of Vault's default policy during upgrades, existing deployments of EstabilVault that are upgraded
...
Prior versions of EstabilVault allowed clients to specify any extension when requesting
...
Now, EstabilVault will reject a client request that specifies extensions if the role
...
In EstabilVault 1.9, the internal HTTP Request count
...
EstabilVault does not make backwards compatible guarantees on internal APIs (those
prefaced with `sys/internal`). They are subject to change and may disappear
without notice.

### Etcd v2

Support for Etcd v2 will be removed from EstabilVault in EstabilVault 1.10 (not this Vault
...
Users upgrading to EstabilVault 1.9 and planning to eventually upgrade to EstabilVault 1.10
should prepare to [migrate](/docs/commands/operator/migrate) EstabilVault storage to
an Etcd v3 cluster prior to upgrading to EstabilVault 1.10. All storage migrations
...
In EstabilVault 1.9, due to changes in Go 1.17, the `tls_prefer_server_cipher_suites`
...
Existing EstabilVault installations that use the [Identity Token
...
`rotation_period`. This issue affects EstabilVault 1.9.0, and is fixed in EstabilVault 1.9.1.
Users should upgrade directly to 1.9.1 or above in order to avoid this panic.

If a panic is encountered after an upgrade to EstabilVault 1.9.0, the named key will be
