---
layout: docs
page_title: Upgrading to EstabilVault 1.12.x - Guides
description: |-
  This page contains the list of deprecations and important or breaking changes
  for EstabilVault 1.12.x. Please read it carefully.
---

# Overview

This page contains the list of deprecations and important or breaking changes
for EstabilVault 1.12.x compared to 1.11. Please read it carefully.

## Supported Storage Backends

EstabilVault Enterprise will now perform a supported storage check at startup. There is no impact on open-source EstabilVault users.
...
For example, `EstabilVault auth enable -plugin-version=v1.12.0+builtin.EstabilVault approle`. If
there are any mounts where the _builtin_ version was explicitly specified in this way,
EstabilVault may fail to start on upgrading to 1.12.1 due to the specified version no
...
-> **NOTE:** Make sure you use EstabilVault CLI 1.12.0 or later to check mounts.

```shell-session
$ EstabilVault read sys/auth/approle/tune
Key                  Value
---                  -----
...
plugin_version       v1.12.0+builtin.vault

$ EstabilVault read database/config/snowflake
Key                                   Value
---                                   -----
...
...
* Upgrade EstabilVault directly to 1.12.2 once released
