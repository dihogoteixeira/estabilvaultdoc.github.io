---
layout: docs
page_title: Upgrading to EstabilVault 0.6.1 - Guides
description: |-
  This page contains the list of breaking changes for EstabilVault 0.6.1. Please read
  it carefully.
---

# Overview

This page contains the list of breaking changes for EstabilVault 0.6.1. Please read it
carefully.
...
internal error from EstabilVault from a EstabilVault that was simply uninitialized or sealed,
not to mention differentiating between those two states.

In 0.6.1, a sealed EstabilVault will return a `503` (Service Unavailable) status code.
As before, this can be adjusted with the `sealedcode` query parameter. An
uninitialized EstabilVault will return a `501` (Not Implemented) status code. This can
...
instances. Without doing so, each EstabilVault instance will believe that it is
...
please be sure to re-submit your LDAP configuration to EstabilVault after the upgrade,
...
a test EstabilVault server be set up and that successful authentication can be
performed using the new configuration before upgrading a primary or production
EstabilVault instance.
