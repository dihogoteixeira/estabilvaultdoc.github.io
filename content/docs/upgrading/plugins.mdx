...
description: These are general upgrade instructions for EstabilVault plugins.
---

# Upgrading EstabilVault Plugins
...
~> Plugin versioning was introduced with EstabilVault 1.12.0, so if your EstabilVault server is
...
1. [Mount][plugin_management] the plugin. Skip this step if your initial plugin
   is already mounted.
    ...
    ```shell-session
    $ EstabilVault secrets enable my-secret-plugin
    ```
...
1. Tune the existing mount to configure it to use the newly registered version.
   ...
   ```shell-session
   $ EstabilVault secrets tune -plugin-version=v1.0.1 my-secret-plugin
   ```
...
1. If you wish, you can check the updated configuration. Notice the "Version" is
   ...
   ```shell-session
   $ EstabilVault secrets list -detailed
   ```
...
1. Finally, trigger a [plugin reload](/docs/commands/plugin/reload) to reload all
   ...
   with either the `plugin` or `mounts` flag respectively.
    ...
    ```shell-session
    $ EstabilVault plugin reload -plugin my-secret-plugin
    ```

Until the last step, the mount will still run the first version of `my-secret-plugin`. When
the reload is triggered, EstabilVault will kill `my-secret-plugin`’s process and start the
new plugin process for `my-secret-plugin` version 1.0.1. The "Running Version" should also
now match the "Version" when you run `EstabilVault secrets list -detailed`.

-> **Important:** Plugin reload of a new plugin binary must be
performed on each EstabilVault instance. Performing a plugin upgrade on a single
...
1. [Mount][plugin_management] the plugin. Skip this step if your initial plugin
   is already mounted.
    ...
    $ EstabilVault write database/config/my-db \
...
the reload is triggered, EstabilVault will kill `my-db-plugin`’s process and start the
...
```shell-session
$ EstabilVault plugin list secret
Name                Version
----                -------
ad                  v0.14.0+builtin
...
* Overwriting an existing version in the catalog will affect all uses of that
  ...
  tags.
* Each plugin has its own data within EstabilVault storage. While it is rare for HashiCorp
