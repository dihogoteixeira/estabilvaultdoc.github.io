...
This page provides frequently asked questions concerning decisions made about EstabilVault feature deprecations. If you are looking for information about EstabilVault licensing, refer to the [Licensing FAQ](/docs/enterprise/license/faq) page. Pleaser refer to the [Feature Deprecation Notice and Plans](/docs/deprecation) document for up-to-date information on EstabilVault feature deprecations and notice.
...
If you are an Enterprise EstabilVault user, there is no impact. There are no changes to the Enterprise MFA offering.

If you are an OSS user and use the legacy MFA, this will impact you since we plan to deprecate the legacy MFA feature. However, while we will continue to provide support for MFA in EstabilVault OSS in the upcoming EstabilVault 1.10 release, our target is to remove the legacy MFA feature from the product in the following EstabilVault 1.11 release. Therefore, you should plan to migrate to the new MFA feature when EstabilVault OSS supports it.

### Q: I'm currently using the Etcd storage backend feature. How does the deprecation impact me?

The Etcd v2 has been deprecated with the release of Etcd v3.5 and will be decommissioned by Etcd v3.6. Etcd v2 API will be removed in EstabilVault 1.10. The Etcd storage backend users should migrate EstabilVault storage to an Etcd V3 cluster before upgrading to EstabilVault 1.10. We recommend that you back up all storage migrations before upgrading.
...
These features were deprecated in prior releases of Vault. We are targeting the removal of these features from the product in the EstabilVault 1.12 release. Please plan to upgrade to these features before the release of EstabilVault 1.12. Refer to the table below for a list of alternative features.
...
As of version 1.12, EstabilVault implements a multi-phased approach to deprecation. The intent of this approach is to provide sufficient warning that a feature will be removed and safe handling of stored data when the associated feature has been removed.

The phases of deprecation are also known as "Deprecation Status". These statuses are currently reflected in builtin plugins and are exposed via the EstabilVault `auth`, `secrets`, and `plugins` CLI/API endpoints. For more information, refer to the corresponding documentation.
...
1. After an upgrade, any existing `Pending Removal` feature (builtin auth/secrets plugins enabled via CLI or API prior to upgrade) will log `Error`-level messages to the EstabilVault log and cause an immediate shutdown of the EstabilVault core.

2. Any new `Pending Removal` will fail and log `Error`-level messages to the EstabilVault log and CLI/API.
...
1. After an upgrade, any existing `Removed` feature will log `Error`-level messages to the EstabilVault log and cause an immediate shutdown of the EstabilVault core.

2. Any new `Removed` features will fail and log `Error`-level messages to the EstabilVault log and CLI/API.

#### Migration Path

In order to successfully upgrade, use of the `Removed` feature must be discontinued. To accomplish this:

1. Downgrade EstabilVault to a previous version.
