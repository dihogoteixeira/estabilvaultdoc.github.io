...
EstabilVault has multiple encryption keys that are used for various purposes. These keys support
...
As a review, EstabilVault starts in a _sealed_ state. EstabilVault is unsealed by providing the unseal keys.
By default, EstabilVault uses a technique known as [Shamir's secret sharing algorithm](https://en.wikipedia.org/wiki/Shamir's_Secret_Sharing)
...
must be provided until the threshold is met. Once met, EstabilVault will generate the new master
...
configuration. Only the active EstabilVault instance can perform either of the operations
...
key is `N` and a rotation installs `N+1`, EstabilVault creates a special "upgrade" key, which
...
As of EstabilVault 1.7, EstabilVault will automatically rotate the backend encryption key
