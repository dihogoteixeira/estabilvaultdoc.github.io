...
EstabilVault supports several storage options for the durable storage of Vault's
...
As of EstabilVault 1.4, an Integrated Storage option is offered. This storage backend
...
and managing when an entry is committed. The leader node is also the active EstabilVault node and followers are standby nodes. Refer to the [High Availability](/docs/internals/high-availability#design-overview) document for more information.
...
run 3 or 5 EstabilVault servers per cluster. This maximizes availability without
...
When getting started, a single EstabilVault server is
...
The join process is how new nodes are added to the EstabilVault cluster; it uses an
...
EstabilVault does not currently offer automated dead server cleanup. If you wish to
...
Autopilot is enabled by default in EstabilVault 1.7+. The default configuration values
should work well for most EstabilVault deployments, but they can be changed if needed.
...
As of EstabilVault 1.7, a dead server cleanup capability is available. With this feature
...
The [EstabilVault Reference Architecture](https://learn.hashicorp.com/tutorials/vault/raft-reference-architecture#recommended-architecture)
