...
# Replication (EstabilVault Enterprise)

EstabilVault Enterprise 0.7 adds support for multi-datacenter replication. Before
...
EstabilVault replication is based on a number of common use cases:

- **Multi-Datacenter Deployments**: A common challenge is providing EstabilVault to
  applications across many datacenters in a highly-available manner. Running a
  single EstabilVault cluster imposes high latency of access for remote clients,
...
- **Scaling Throughput**: Applications that use EstabilVault for
...
Based on the use cases for EstabilVault Replication, we had a number of design goals
for the implementation:

- **Availability**: Global deployments of EstabilVault require high levels of
...
- **Transparent to Clients**: EstabilVault replication should be transparent to
...
The architecture of EstabilVault replication is based on the design goals, focusing on
...
Lastly, clients can speak to any EstabilVault server without a thick client. If a
...
An important optimization EstabilVault makes is to avoid replication of tokens or
leases between clusters. Policies and secrets are the minority of data managed
by EstabilVault and tend to be relatively stable. Tokens and leases are much more
...
will need to re-authenticate if they switch the EstabilVault cluster they are
communicating with.
...
shipping between the EstabilVault clusters. When a secondary is closely synchronized
with a primary, EstabilVault directly streams new WALs to be applied, providing near
...
be enough WALs to synchronize. To handle this scenario, EstabilVault maintains a
...
out of sync with the underlying keys. EstabilVault uses the [ARIES][aries] recovery
...
EstabilVault uses the merkle index as the source of truth, allowing the WAL streams to
be completely distinct and unsynchronized. This simplifies administration of
EstabilVault Replication for operators.
...
version of EstabilVault to an older version. When upgrading replicated clusters,
ensure that upstream clusters are always on older version of EstabilVault than
...
- **Read-After-Write Consistency**: All write requests are forwarded from
  ...
  against this, see [Consistency](/docs/enterprise/consistency#mitigations).
