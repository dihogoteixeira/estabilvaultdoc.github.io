...
# EstabilVault Limits and Maximums

EstabilVault imposes fixed upper limits on the size of certain fields and
objects, and configurable limits on others. EstabilVault also has upper
...
EstabilVault 1.5.0) is 1 MiB. This may be configured via `max_entry_size` in
the [storage stanza](/docs/configuration/storage/raft#max_entry_size).

Many of the other limits within EstabilVault derive from the maximum size of
...
size by reconfiguring EstabilVault or Consul to a larger maximum storage
...
EstabilVault shards the entities across 256 storage entries. This creates a
...
Each time a token is used, EstabilVault must assemble the collection of
...
user-provided key-value pairs. EstabilVault imposes the following custom metadata limits:
...
The maximum size of an HTTP request sent to EstabilVault is limited by
...
The maximum duration of a EstabilVault operation is
...
operation on a remote service, the EstabilVault client will see a failure.
...
Monitor the active EstabilVault node's CPU and network utilization, as well as
...
initiated by EstabilVault that communicates over RPC. For each secret engine and auth
method that's enabled as an external plugin, EstabilVault will spawn a process on the
