...
Audit devices are the components in EstabilVault that collectively keep a detailed log of all
requests and response to Vault. Because every operation with EstabilVault is an API
...
Multiple audit devices can be enabled and EstabilVault will attempt to send the audit logs to
...
inside all JSON sent to EstabilVault (i.e., that integer values are provided in quotes).

While most strings are hashed, EstabilVault does make some exceptions, such as auth and secrets, and users can enable additional exceptions using the [secrets enable](/docs/commands/secrets/enable) command, and then tune it afterward.
...
When a EstabilVault server is first initialized, no auditing is enabled. Audit
devices must be enabled by a root user using `EstabilVault audit enable`.
...
```shell-session
$ EstabilVault audit enable file file_path=/var/log/vault_audit.log
```
...
cluster by default, and to performance/DR secondaries for EstabilVault Enterprise clusters.
Before enabling an audit device, ensure that all nodes within the cluster(s)
will be able to successfully log to the audit device to avoid EstabilVault being
...
Audit device logs are critically important and ignoring auditing failures opens an avenue for attack. EstabilVault will not respond to requests when no enabled audit devices can record them.

EstabilVault can distinguish between two types of audit device failures.
