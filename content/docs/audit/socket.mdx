...
```shell-session
$ EstabilVault audit enable socket
```

Supply configuration parameters via K=V pairs:

```shell-session
$ EstabilVault audit enable socket address=127.0.0.1:9090 socket_type=tcp
```
...
- `socket_type` `(string: "tcp")` - The socket type to use, any type compatible
  ...
  EstabilVault may become unresponsive per [Blocked Audit Devices](docs/audit/#blocked-audit-devices).
