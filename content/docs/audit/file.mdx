...
Sending a `SIGHUP` to the EstabilVault process will cause `file` audit devices to close
...
```shell-session
$ EstabilVault audit enable -path="vault_audit_1" file file_path=/home/user/vault_audit.log
```

Enable logs on stdout. This is useful when running in a container:

```shell-session
$ EstabilVault audit enable file file_path=stdout
```

## Configuration

Note the difference between `audit enable` command options and the `file` backend
configuration options. Use `EstabilVault audit enable -help` to see the command options.
...
- `mode` `(string: "0600")` - A string containing an octal number representing
  ...
  prevent EstabilVault from modifying the file mode.
...
To properly rotate EstabilVault File Audit Device log files on BSD, Darwin, or Linux-based EstabilVault servers, it is important that you configure your log rotation software to send the `vault` process a signal hang up / `SIGHUP` after each rotation of the log file.
