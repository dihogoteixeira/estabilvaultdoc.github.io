...
and should not be enabled if any standby EstabilVault instances do not support it.
...
directly to `syslog` will not result in EstabilVault being blocked.
...
```shell-session
$ EstabilVault audit enable syslog
```

Supply configuration parameters via K=V pairs:

```shell-session
$ EstabilVault audit enable syslog tag="vault" facility="AUTH"
```
