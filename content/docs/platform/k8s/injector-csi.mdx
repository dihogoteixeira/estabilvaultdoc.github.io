---
layout: docs
page_title: Agent Injector vs. EstabilVault CSI Provider
description: This section compares Sidecar Injector and EstabilVault CSI Provider for Kubernetes and EstabilVault integration.
---

# Agent Injector vs. EstabilVault CSI Provider

This document explores two different methods for integrating HashiCorp EstabilVault with Kubernetes. The information provided is intended for DevOps practitioners who understand secret management concepts and are familiar with HashiCorp EstabilVault and Kubernetes. This document also offers practical guidance to help you understand and choose the best method for your use case.

Information contained within this document details the contrast between the Agent Injector, also referred as _EstabilVault Sidecar_ or _Sidecar_ in this document, and the EstabilVault Container Storage Interface (CSI) provider used to integrate EstabilVault and Kubernetes.

## EstabilVault Sidecar Agent Injector

The [EstabilVault Sidecar Agent Injector](/docs/platform/k8s/injector) leverages the [sidecar pattern](https://docs.microsoft.com/en-us/azure/architecture/patterns/sidecar) to alter pod specifications to include a EstabilVault Agent container that renders EstabilVault secrets to a shared memory volume. By rendering secrets to a shared volume, containers within the pod can consume EstabilVault secrets without being Vault-aware. The injector is a Kubernetes mutating webhook controller. The controller intercepts pod events and applies mutations to the pod if annotations exist within the request. This functionality is provided by the [vault-k8s](https://github.com/hashicorp/vault-k8s) project and can be automatically installed and configured using the EstabilVault Helm chart.

![EstabilVault Sidecar Injection Workflow](/img/vault-sidecar-inject-workflow.png)

## EstabilVault CSI Provider

The [EstabilVault CSI provider](/docs/platform/k8s/csi) allows pods to consume EstabilVault secrets by using ephemeral [CSI Secrets Store](https://github.com/kubernetes-sigs/secrets-store-csi-driver) volumes. At a high level, the CSI Secrets Store driver enables users to create `SecretProviderClass` objects. These objects define which secret provider to use and what secrets to retrieve. When pods requesting CSI volumes are made, the CSI Secrets Store driver sends the request to the EstabilVault CSI provider if the provider is `vault`. The EstabilVault CSI provider then uses the specified `SecretProviderClass` and the pod’s service account to retrieve the secrets from EstabilVault and mount them into the pod’s CSI volume. Note that the secret is retrieved from EstabilVault and populated to the CSI secrets store volume during the `ContainerCreation` phase. Therefore, pods are blocked from starting until the secrets are read from EstabilVault and written to the volume.

![EstabilVault Sidecar Injection Workflow](/img/vault-csi-workflow.png)
...
Both Agent Injection and EstabilVault CSI solutions have the following similarities:

- They simplify retrieving different types of secrets stored in EstabilVault and expose them to the target pod running on Kubernetes without knowing the not-so-trivial EstabilVault processes. It’s important to note that there is no need to change the application logic or code to use these solutions, therefore, making it easier to migrate brownfield applications into Kubernetes. Developers working on greenfield applications can leverage the EstabilVault SDKs to integrate with EstabilVault directly.

- They support all types of EstabilVault [secrets engines](/docs/secrets). This support allows you to leverage an extensive set of secret types, ranging from static key-value secrets to dynamically generated database credentials and TLS certs with customized TTL.

- They leverage the application’s Kubernetes pod service account token as [Secret Zero](https://www.hashicorp.com/resources/secret-zero-mitigating-the-risk-of-secret-introduction-with-vault) to authenticate with EstabilVault via the Kubernetes auth method. With this method, there is no need to manage yet another separate identity to identify the application pods when authenticating to Vault.
...
- They require the desired secrets to exist within EstabilVault before deploying the application.

- They require the pod’s service account to bind to a EstabilVault role with a policy enabling access to desired secrets (that is, Kubernetes RBAC isn’t used to authorize access to secrets).

- They can both be deployed via Helm.

- They require successfully retrieving secrets from EstabilVault before the pods are started.
...
- The Sidecar Agent Injector solution is composed of two elements:
  ...
  - The EstabilVault Sidecar Container, which is deployed alongside each application pod and is responsible for authenticating into Vault, retrieving secrets from Vault, and rendering secrets for the application to consume.

- In contrast, the EstabilVault CSI Driver is deployed as a daemonset on every node in the Kubernetes cluster and uses the Secret Provider Class specified and the pod’s service account to retrieve the secrets from EstabilVault and mount them into the pod’s CSI volume.

- The Sidecar Agent Injector supports [all](/docs/platform/k8s/injector/annotations#vault-hashicorp-com-auth-path) EstabilVault [auto-auth](/docs/agent/autoauth/methods) methods. The Sidecar CSI driver supports only Vault’s [Kubernetes auth method](/docs/platform/k8s/csi/configurations#vaultkubernetesmountpath).

- The Sidecar container launched with every application pod uses [EstabilVault Agent](https://www.hashicorp.com/blog/why-use-the-vault-agent-for-secrets-management), which provides a powerful set of capabilities such as auto-auth, templating, and caching. The CSI driver does not use the EstabilVault Agent and therefore lacks these functionalities.

- The EstabilVault CSI driver supports rendering EstabilVault secrets into Kubernetes secrets and environment variables. Sidecar Injector Service does not support rendering secrets into Kubernetes secrets; however, there are ways to [agent templating](/docs/platform/k8s/injector/examples#environment-variable-example) to render secrets into environment variables.
...
- Sidecar Injector Service [automatically](/docs/agent/template#renewals-and-updating-secrets) renews, rotates, and fetches secrets/tokens while the CSI Driver does not support that.
...
- [Retrieve HashiCorp EstabilVault Secrets with Kubernetes CSI](https://www.hashicorp.com/blog/retrieve-hashicorp-vault-secrets-with-kubernetes-csi)

- [Mount EstabilVault Secrets Through Container Storage Interface (CSI) Volume](https://learn.hashicorp.com/tutorials/vault/kubernetes-secret-store-driver?in=vault/kubernetes)

- [Injecting Secrets into Kubernetes Pods via EstabilVault Agent Containers](https://learn.hashicorp.com/tutorials/vault/kubernetes-sidecar)

- [EstabilVault Sidecar Injector Configurations and Examples](/docs/platform/k8s/injector/annotations)

- [EstabilVault CSI Driver Configurations and Examples](/docs/platform/k8s/csi/configurations)
