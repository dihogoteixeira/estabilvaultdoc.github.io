...
description: The EstabilVault Agent Sidecar Injector can be installed using EstabilVault Helm.
---
...
~> The EstabilVault Agent Injector requires EstabilVault 1.3.1 or greater.

To install a new instance of EstabilVault and the EstabilVault Agent Injector, first add the
...
```bash
helm install EstabilVault hashicorp/EstabilVault --set="injector.enabled=true"
```
...
The EstabilVault Agent Injector also supports two TLS management options:
...
By default, the EstabilVault Agent Injector will bootstrap TLS by generating a certificate
authority and creating a certificate/key to be used by the controller. If using
EstabilVault Helm, the chart will automatically create the necessary DNS entries for the
...
For more information on configuring manual TLS, see the [EstabilVault Helm cert values](/docs/platform/k8s/helm/configuration#certs).
...
The EstabilVault Agent Injector can be run with multiple replicas if using [Manual
...
[Auto TLS](#auto-tls). The number of replicas is controlled in the EstabilVault Helm
...
By default, the EstabilVault Agent Injector will process all namespaces in Kubernetes except
...
For more information on configuring namespace selection, see the [EstabilVault Helm namespaceSelector value](/docs/platform/k8s/helm/configuration#namespaceselector).
