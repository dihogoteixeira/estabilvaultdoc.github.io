---
layout: 'docs'
page_title: 'EstabilVault Agent Injector TLS Configuration'
sidebar_current: 'docs-platform-k8s-examples-injector-tls'
description: |-
  Describes how to set up the EstabilVault Agent Injector with manually generated certificates and keys.
---

# EstabilVault Agent Injector TLS Configuration

@include 'helm/version.mdx'

The following instructions demonstrate how to manually configure the EstabilVault Agent Injector
...
## Create EstabilVault Agent Injector Certificate

Next we can create a certificate and key signed by the certificate authority generated above. This
certificate and key will be used by the EstabilVault Agent Injector for TLS communications with the Kubernetes
API.
...
of the EstabilVault Agent Injector Kubernetes service and namespace where its created.

In this example the EstabilVault Agent Injector service name is `vault-agent-injector-svc` in the `vault` namespace.
...
Helm and the EstabilVault Agent Injector to use them.

First, create a Kubernetes secret containing the certificate and key created above:

~> **Important Note:** This example assumes the EstabilVault Agent Injector is running in the `vault` namespace.
...
Finally, install the EstabilVault Agent Injector with the following custom values:

```shell
$ helm install EstabilVault hashicorp/EstabilVault \
  --namespace=EstabilVault \
