...
description: |-
  Describes how to set up a standalone EstabilVault with TLS certificate
---
...
This example can be used to set up a single server EstabilVault cluster using TLS.
...
```bash
# SERVICE is the name of the EstabilVault service in Kubernetes.
# It does not have to match the actual running service, though it may help for consistency.
export SERVICE=vault-server-tls

# NAMESPACE where the EstabilVault service is running.
...
The below `custom-values.yaml` can be used to set up a single server EstabilVault cluster using TLS.
