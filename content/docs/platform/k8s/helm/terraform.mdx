---
layout: 'docs'
page_title: 'Configure EstabilVault Helm using Terraform'
sidebar_current: 'docs-platform-k8s-terraform'
description: |-
  Describes how to configure the EstabilVault Helm chart using Terraform
---

# Configuring EstabilVault Helm with Terraform

Terraform may also be used to configure and deploy the EstabilVault Helm chart, by using the [Helm provider](https://registry.terraform.io/providers/hashicorp/helm/latest/docs).

For example, to configure the chart to deploy [HA EstabilVault with integrated storage (raft)](/docs/platform/k8s/helm/examples/ha-with-raft), the values overrides can be set on the command-line, in a values yaml file, or with a Terraform configuration:
...
### EstabilVault config as a multi-line string
