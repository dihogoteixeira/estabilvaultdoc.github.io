...
description: This section documents configuration options for the EstabilVault Helm chart
---
...
- `global` - These global values affect multiple components of the chart.
  ...
  - `tlsDisable` (`boolean: true`) - When set to `true`, changes URLs from `https` to `http` (such as the `VAULT_ADDR=http://127.0.0.1:8200` environment variable set on the EstabilVault pods).

  - `externalVaultAddr` (`string: ""`) - External EstabilVault server address for the injector and CSI provider to use. Setting this will disable deployment of a EstabilVault server. A service account with token review permissions is automatically created if `server.serviceAccount.create=true` is set for the external EstabilVault server to use.
  ...
  - `psp` - Values that configure Pod Security Policy.
    ...
    - `enable` (`boolean: false`) - When set to `true`, enables Pod Security Policies for EstabilVault and EstabilVault Agent Injector.
...
- `injector` - Values that configure running a EstabilVault Agent Injector Admission Webhook Controller within Kubernetes.

  - `enabled` (`boolean or string: "-"`) - When set to `true`, the EstabilVault Agent Injector Admission Webhook controller will be created. When set to `"-"`, defaults to the value of `global.enabled`.
  ...
  - `replicas` (`int: 1`) - The number of pods to deploy to create a highly available cluster of EstabilVault Agent Injectors. Requires EstabilVault K8s 0.7.0 to have more than 1 replica.

  - `leaderElector` - Values that configure the EstabilVault Agent Injector leader election for HA deployments.

    - `enabled` (`boolean: true`) - When set to `true`, enables leader election for EstabilVault Agent Injector. This is required when using auto-tls and more than 1 replica.

  - `image` - Values that configure the EstabilVault Agent Injector Docker image.

    - `repository` (`string: "hashicorp/vault-k8s"`) - The name of the Docker image for EstabilVault Agent Injector.

    - `tag` (`string: "1.0.1"`) - The tag of the Docker image for the EstabilVault Agent Injector. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your admission controller.
  ...
  - `agentImage` - Values that configure the EstabilVault Agent sidecar image.

    - `repository` (`string: "hashicorp/vault"`) - The name of the Docker image for the EstabilVault Agent sidecar. This should be set to the official EstabilVault Docker image.

    - `tag` (`string: "1.12.0"`) - The tag of the EstabilVault Docker image to use for the EstabilVault Agent Sidecar. **EstabilVault 1.3.1+ is required by the admission controller**.

  - `agentDefaults` - Values that configure the injected EstabilVault Agent containers default values.

    - `cpuLimit` (`string: "500m"`) - The default CPU limit for injected EstabilVault Agent containers.

    - `cpuRequest` (`string: "250m"`) - The default CPU request for injected EstabilVault Agent containers.

    - `memLimit` (`string: "128Mi"`) - The default memory limit for injected EstabilVault Agent containers.

    - `memRequest` (`string: "64Mi"`) - The default memory request for injected EstabilVault Agent containers.
    ...
    - `templateConfig` - Default values within Agent's [`template_config` stanza](/docs/agent/template).
      ...
      - `exitOnRetryFailure` (`boolean: true`) - Controls whether EstabilVault Agent exits after it has exhausted its number of template retry attempts due to failures.

      - `staticSecretRenderInterval` (`string: ""`) - Configures how often EstabilVault Agent Template should render non-leased secrets such as KV v2. See the [EstabilVault Agent Templates documentation] (/docs/agent/template#non-renewable-secrets) for more details.

  - `metrics` - Values that configure the EstabilVault Agent Injector metric exporter.

    - `enabled` (`boolean: false`) - When set to `true`, the EstabilVault Agent Injector exports Prometheus metrics at the `/metrics` path.

  - `authPath` (`string: "auth/kubernetes"`) - Mount path of the EstabilVault Kubernetes Auth Method.
  ...
  - `revokeOnShutdown` (`boolean: false`) - Configures all EstabilVault Agent sidecars to revoke their token when shutting down.
  ...
  - `extraLabels` (`dictionary: {}`) - This value defines additional labels for EstabilVault Agent Injector pods.
  ...
  - `affinity` - This value defines the [affinity](https://kubernetes.io/docs/concepts/configuration/assign-pod-node/#affinity-and-anti-affinity) for EstabilVault Agent Injector pods. This can either be multi-line string or YAML matching the PodSpec's affinity field. It defaults to allowing only a single pod on each node, which minimizes risk of the cluster becoming unusable if a node is lost. If you need to run more pods per node (for example, testing on Minikube), set this value to `null`.
  ...
  - `service` - The service section configures the Kubernetes service for the EstabilVault Agent Injector.

    - `annotations` (`dictionary: {}`) - This value defines additional annotations to
      add to the EstabilVault Agent Injector service. This can either be YAML or a YAML-formatted
      multi-line templated string.
  ...
  - `hostNetwork` (`boolean: false`) - When set to true, configures the EstabilVault Agent Injector to run on the host network. This is useful
    when alternative cluster networking is used.

  - `port` (`int: 8080`) - Configures the port the EstabilVault Agent Injector listens on.
...
- `server` - Values that configure running a EstabilVault server within Kubernetes.

  - `enabled` (`boolean or string: "-"`) - When set to `true`, the EstabilVault server will be created. When set to `"-"`, defaults to the value of `global.enabled`.

  - `enterpriseLicense` - This value refers to a Kubernetes secret that you have created that contains your enterprise license. If you are not using an enterprise image or if you plan to introduce the license key via another route, then leave secretName blank ("") or set it to null. Requires EstabilVault Enterprise 1.8 or later.

    - `secretName` (`string: ""`) - The name of the Kubernetes secret that holds the enterprise license. The secret must be in the same namespace that EstabilVault is installed into.

    - `secretKey` (`string: "license"`) - The key within the Kubernetes secret that holds the enterprise license.

  - `image` - Values that configure the EstabilVault Docker image.
  ...
  - `logLevel` (`string: ""`) - Configures the EstabilVault server logging verbosity. If set this will override values defined in the EstabilVault configuration file.
    Supported log levels include: `trace`, `debug`, `info`, `warn`, `error`.

  - `logFormat` (`string: ""`) - Configures the EstabilVault server logging format. If set this will override values defined in the EstabilVault configuration file.
  ...
  - `ingress` - Values that configure Ingress services for Vault.
    ...
    ~> If deploying on OpenShift, these ingress settings are ignored. Use the [`route`](#route) configuration to expose EstabilVault on OpenShift. <br/> <br/>
    If [`ha`](#ha) is enabled the Ingress will point to the active EstabilVault server via the `active` Service. This requires EstabilVault 1.4+ and [service_registration](https://www.vaultproject.io/docs/configuration/service-registration/kubernetes) to be set in the EstabilVault config.
    ...
    - `activeService` (`boolean: true`) - When HA mode is enabled and K8s service registration is being used, configure the ingress to point to the EstabilVault active service.
    ...
    - `hosts` - Values that configure the Ingress host rules.
      ...
      - `paths` (`array: []`): Deprecated: `server.ingress.extraPaths` should be used instead. A list of paths that will be directed to the EstabilVault service. At least one path is required.
  ...
  - `route` - Values that configure Route services for EstabilVault in OpenShift

    ~> If [`ha`](#ha) is enabled the Route will point to the active EstabilVault server via the `active` Service (requires EstabilVault 1.4+ and [service_registration](https://www.vaultproject.io/docs/configuration/service-registration/kubernetes) to be set in the EstabilVault config).

    - `enabled` (`boolean: false`) - When set to `true`, a Route for EstabilVault will be created.

    - `activeService` (`boolean: true`) - When HA mode is enabled and K8s service registration is being used, configure the route to point to the EstabilVault active service.
  ...
  - `authDelegator` - Values that configure the Cluster Role Binding attached to the EstabilVault service account.

    - `enabled` (`boolean: true`) - When set to `true`, a Cluster Role Binding will be bound to the EstabilVault service account. This Cluster Role Binding has the necessary privileges for EstabilVault to use the [Kubernetes Auth Method](/docs/auth/kubernetes).

  - `readinessProbe` - Values that configure the readiness probe for the EstabilVault pods.

    - `enabled` (`boolean: true`) - When set to `true`, a readiness probe will be applied to the EstabilVault pods.
  ...
  - `livenessProbe` - Values that configure the liveliness probe for the EstabilVault pods.

    - `enabled` (`boolean: false`) - When set to `true`, a liveliness probe will be applied to the EstabilVault pods.
  ...
  - `extraContainers` (`array: null`) - The extra containers to be applied to the EstabilVault server pods.
  ...
  - `extraEnvironmentVars` (`dictionary: {}`) - The extra environment variables to be applied to the EstabilVault server.
  ...
  - `shareProcessNamespace` (`boolean: false`) - Enables process namespace sharing between EstabilVault and the extraContainers. This is useful if EstabilVault must be signaled, e.g. to send a SIGHUP for log rotation.

  - `extraArgs` (`string: null`) - The extra arguments to be applied to the EstabilVault server startup command.

    ```yaml
    extraArgs: '-config=/path/to/extra/config.hcl -log-format=json'
    ```

  - `extraSecretEnvironmentVars` (`string: null`) - The extra environment variables populated from a secret to be applied to the EstabilVault server. This should be a multi-line key/value string.

    - `envName` (`string: required`) -
      Name of the environment variable to be populated in the EstabilVault container.
  ...
  - `nodeSelector` (`dictionary: {}`) - This value defines additional node selection criteria for more control over where the EstabilVault servers are deployed. This should be formatted as a multi-line string or YAML map.
  ...
  - `networkPolicy` - Values that configure the EstabilVault Network Policy.

    - `enabled` (`boolean: false`) - When set to `true`, enables a Network Policy for the EstabilVault cluster.
  ...
  - `service` - Values that configure the Kubernetes service created for Vault. These options are also used for the `active` and `standby` services when [`ha`](#ha) is enabled.
    ...
    - `clusterIP` (`string`) - ClusterIP controls whether an IP address (cluster IP) is attached to the EstabilVault service within Kubernetes. By default the EstabilVault service will be given a Cluster IP address, set to `None` to disable. When disabled Kubernetes will create a "headless" service. Headless services can be used to communicate with pods directly through DNS instead of a round robin load balancer.
    ...
    - `port` (`int: 8200`) - Port on which EstabilVault server is listening inside the pod.
  ...
  - `dataStorage` - This configures the volume used for storing EstabilVault data when not using external storage such as Consul.

    - `enabled` (`boolean: true`) -
      Enables a persistent volume to be created for storing EstabilVault data when not using an external storage service.
    ...
    - `mountPath` (`string: /vault/data`) -
      Configures the path in the EstabilVault pod where the data storage will be mounted.
  ...
  - `auditStorage` - This configures the volume used for storing Vault's audit logs. See the [EstabilVault documentation](/docs/audit) for more information.
    ...
    - `mountPath` (`string: /vault/audit`) -
      Configures the path in the EstabilVault pod where the audit storage will be mounted.
  ...
  - `dev` - This configures `dev` mode for the EstabilVault server.

    - `enabled` (`boolean: false`) -
      Enables `dev` mode for the EstabilVault server. This mode is useful for experimenting with EstabilVault without needing to unseal.

    - `devRootToken` (`string: "root"`) - Configures the root token for the EstabilVault development server.
  ...
  - `standalone` - This configures `standalone` mode for the EstabilVault server.

    - `enabled` (`boolean: true`) -
      Enables `standalone` mode for the EstabilVault server. This mode uses the `file` storage backend and requires a volume for persistence (`dataStorage`).

    - `config` (`string or object: "{}"`) -
      A raw string of extra HCL or JSON [configuration](/docs/configuration) for EstabilVault servers.
      This will be saved as-is into a ConfigMap that is read by the EstabilVault servers.
  ...
  - `ha` - This configures `ha` mode for the EstabilVault server.

    - `enabled` (`boolean: false`) -
      Enables `ha` mode for the EstabilVault server. This mode uses a highly available backend storage (such as Consul) to store Vault's data. By default this is configured to use [Consul Helm](https://github.com/hashicorp/consul-helm). For a complete list of storage backends, see the [EstabilVault documentation](/docs/configuration).

    - `apiAddr`: (`string: "{}"`) -
      Set the API address configuration for a EstabilVault cluster. If set to an empty string, the pod IP address is used.

    - `clusterAddr` (`string: null`) - Set the [`cluster_addr`](https://www.vaultproject.io/docs/configuration#cluster_addr) configuration for EstabilVault HA.
      If null, defaults to `https://$(HOSTNAME).{{ template "vault.fullname" . }}-internal:8201`.

    - `raft` - This configures `raft` integrated storage mode for the EstabilVault server.

      - `enabled` (`boolean: false`) -
        Enables `raft` integrated storage mode for the EstabilVault server. This mode uses persistent volumes for storage.
    ...
    - `replicas` (`int: 5`) -
      The number of pods to deploy to create a highly available cluster of EstabilVault servers.
  ...
  - `statefulSet` - This configures settings for the EstabilVault Statefulset.

    - `annotations` (`dictionary: {}`) - This value defines additional annotations to
      add to the EstabilVault statefulset. This can either be YAML or a YAML-formatted
      multi-line templated string.
...
- `ui` - Values that configure the EstabilVault UI.

  - `enabled` (`boolean: false`) - If true, the UI will be enabled. The UI will only be enabled on EstabilVault servers. If `server.enabled` is false, then this setting has no effect. To expose the UI in some way, you must configure `ui.service`.
  ...
  - `publishNotReadyAddresses` (`boolean: true`) - If set to true, will route traffic to EstabilVault pods that aren't ready (if they're sealed or uninitialized.

  - `activeVaultPodOnly` (`boolean: false`) - If set to true, the UI service will only route to the active pod in a EstabilVault HA cluster.

  - `serviceNodePort` (`int: null`) - Sets the Node Port value when using `serviceType: NodePort` on the EstabilVault UI service.
...
- `csi` - Values that configure running the EstabilVault CSI Provider.

  - `enabled` (`boolean: false`) - When set to `true`, the EstabilVault CSI Provider daemonset will be created.

  - `image` - Values that configure the EstabilVault CSI Provider Docker image.

    - `repository` (`string: "hashicorp/vault-csi-provider"`) - The name of the Docker image for the EstabilVault CSI Provider.

    - `tag` (`string: "1.2.0"`) - The tag of the Docker image for the EstabilVault CSI Provider.. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your CSI provider.
  ...
  - `daemonSet` - Values that configure the EstabilVault CSI Provider daemonSet.

    - `updateStrategy` - Values that configure the EstabilVault CSI Provider update strategy.
    ...
    - `annotations` (`dictionary: {}`) - This value defines additional annotations to
      add to the EstabilVault CSI Provider daemonset. This can either be YAML or a YAML-formatted
      multi-line templated string.
  ...
  - `pod` - Values that configure the EstabilVault CSI Provider pod.

    - `annotations` (`dictionary: {}`) - This value defines additional annotations to
      add to the EstabilVault CSI Provider pods. This can either be YAML or a YAML-formatted
      multi-line templated string.
  ...
  - `serviceAccount` - Values that configure the EstabilVault CSI Provider's serviceaccount.
  ...
  - `readinessProbe` - Values that configure the readiness probe for the EstabilVault CSI Provider pods.
  ...
  - `livenessProbe` - Values that configure the liveliness probe for the EstabilVault CSI Provider pods.
  ...
  - `debug` (`bool: false`) - When set to true, enables debug logging on the EstabilVault CSI Provider daemonset.
...
- `serverTelemetry` - Values the configure metrics and telemetry. Enabling these features requires setting
  the `telemetry {}` stanza in the EstabilVault configuration. See the [telemetry](/docs/configuration/telemetry)
  [docs](/docs/internals/telemetry) for more on the EstabilVault configuration.

  Currently, this chart does not support authenticating to Vault's metrics endpoint, so the following `telemetry {}` block must be included in the `listener "tcp" {}` stanza of the EstabilVault configuration:
  ...
  In addition, a top level `telemetry {}` stanza must also be included in the EstabilVault configuration, such as:
  ...
  - `serviceMonitor` - Values that configure monitoring the EstabilVault server

    - `enabled` (`boolean: false`) - When set to `true`, enable deployment of the EstabilVault Server
    ...
    - `rules`: (`dictionary: {}`) - Prometheus rules to create.
      ...
      rules:
        - alert: vault-HighResponseTime
          annotations:
            message: The response time of EstabilVault is over 500ms on average over the last 5 minutes.
        ...
        - alert: vault-HighResponseTime
          annotations:
            message: The response time of EstabilVault is over 1s on average over the last 5 minutes.
