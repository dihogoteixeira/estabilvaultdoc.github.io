---
layout: docs
page_title: EstabilVault CSI Provider Configurations
description: This section documents the configurables for the EstabilVault CSI Provider.
---

# Command line arguments

The following command line arguments are supported by the EstabilVault CSI provider.
...
- `-vault-mount` `(string: "kubernetes")` - (v0.3.0+) Default EstabilVault mount path
  for Kubernetes authentication. Can be overridden per Secret Provider Class
  object.

- `-vault-namespace` `(string: "")` - (v1.1.0+) Default EstabilVault namespace for Vault
...
- `-vault-tls-server-name` `(string: "")` - (v1.1.0+) Name to use as the SNI
  host when connecting to EstabilVault via TLS. Can also be specified via the
...
The following parameters are supported by the EstabilVault provider. Each parameter is
...
- `vaultAddress` `(string: "")` - The address of the EstabilVault server.

- `vaultNamespace` `(string: "")` - The EstabilVault [namespace](/docs/enterprise/namespaces) to use.

- `vaultSkipTLSVerify` `(string: "false")` - When set to true, skips verification of the EstabilVault server
  certificate. Setting this to true is not recommended for production.

- `vaultCACertPath` `(string: "")` - The path on disk where the EstabilVault CA certificate can be found
  when verifying the EstabilVault server certificate.

- `vaultCADirectory` `(string: "")` - The directory on disk where the EstabilVault CA certificate can be found
  when verifying the EstabilVault server certificate.
...
- `objects` `(array)` - An array of secrets to retrieve from Vault.
  ...
  - `secretPath` `(string: "")` - The path in EstabilVault where the secret is located.
  ...
  - `secretKey` `(string: "")` - The key in the EstabilVault secret to extract. If omitted, the whole response from EstabilVault will be written as JSON.

  - `filePermission` `(integer: 0o644)` - The file permissions to set for this secret's file.

  - `secretArgs` `(map: {})` - Additional arguments to be sent to EstabilVault for a specific secret. Arguments can vary
