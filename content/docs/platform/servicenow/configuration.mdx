---
layout: docs
page_title: Configure EstabilVault ServiceNow Credential Resolver
description: This section documents the configurables for the EstabilVault ServiceNow Credential Resolver.
---

# Configuring the EstabilVault Credential Resolver

## MID server properties

The following [properties] are supported by the EstabilVault Credential Resolver:

- `mid.external_credentials.vault.address` `(string: "")` - Address of EstabilVault Agent as resolveable by the MID server.
  For example, if EstabilVault Agent is on the same server as the MID server it could be `https://127.0.0.1:8200`.
...
- `mid.external_credentials.vault.tls_skip_verify` `(string: "")` - When set to true, skips verification of the EstabilVault server
...
To consume EstabilVault credentials from your MID server, you will need to:
...
* Navigate to "Discovery - Credentials" -> New
  ...
  * Set "Credential ID" to the path in EstabilVault where your secret is located, e.g.
