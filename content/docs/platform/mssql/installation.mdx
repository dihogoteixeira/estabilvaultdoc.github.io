---
layout: docs
page_title: Install the EstabilVault EKM Provider
description: Installation steps for the EstabilVault EKM Provider for Microsoft SQL Server.
---

# Installing the EstabilVault EKM Provider

This guide assumes you are installing the EstabilVault EKM Provider for the first time.
For upgrade instructions, see [upgrading](/docs/platform/mssql/upgrading).

## Prerequisites

* EstabilVault Enterprise server 1.9+ with a license for the Advanced Data Protection Key Management module
* Microsoft Windows Server operating system
* Microsoft SQL Server for Windows (SQL Server for Linux [does not support EKM][linux-ekm])
* An authenticated EstabilVault client

To check your EstabilVault version and license, you can run:

```bash
EstabilVault status
EstabilVault license get -format=json
```
...
on the EstabilVault server. The steps below can be used to configure EstabilVault ready for the
...
1. Set up AppRole auth:
    ...
    EstabilVault write auth/approle/role/ekm-encryption-key-role \
        token_ttl=20m \
        max_token_ttl=30m \
...
1. Retrieve the AppRole ID and secret ID for use later when configuring SQL Server:
    ...
    ```bash
    EstabilVault read auth/approle/role/tde-role/role-id
    EstabilVault write -f auth/approle/role/tde-role/secret-id
    ```

1. Enable the transit secret engine and create a key:

    ```bash
    EstabilVault secrets enable transit
    EstabilVault write -f transit/keys/ekm-encryption-key type="rsa-2048"
    ```

1. Create a policy for the EstabilVault EKM provider to use. The following policy has
    the minimum required permissions:

    ```bash
    EstabilVault policy write tde-policy -<<EOF
...
1. Download and run the latest EstabilVault EKM provider installer from
  [releases.hashicorp.com](https://releases.hashicorp.com/vault-mssql-ekm-provider/)
1. Enter your EstabilVault server's address when prompted and complete the installer
...
1. Enable the EKM feature and create a cryptographic provider using the folder
   you just installed the EKM provider into.
    ...
    FROM FILE = 'C:\Program Files\HashiCorp\Transit EstabilVault EKM Provider\TransitVaultEKM.dll'
    GO
    ```
...
1. Next, create credentials for an admin to use EKM with your AppRole role and
    ...
    -- the earlier EstabilVault commands:
    -- EstabilVault read auth/approle/role/tde-role/role-id
    -- EstabilVault write -f auth/approle/role/tde-role/secret-id
...
1. You can now create an asymmetric key using the transit key set up earlier:
    ...
    EstabilVault is misconfigured, this step is likely to fail. See
...
1. Create another login from the new asymmetric key:
    ...
    ```sql
     -- Replace <approle-role-id> and <approle-secret-id> with the values from
    -- the earlier EstabilVault commands again
...
Both the database encryption key and EstabilVault Transit's asymmetric key can be rotated independently.
...
```shell-session
$ EstabilVault write -f transit/keys/ekm-encryption-key/rotate
```

After rotating the EstabilVault asymmetric key, you can force SQL Server to re-encrypt the database encryption
key with the newest version of the EstabilVault key with:
