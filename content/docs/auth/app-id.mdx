...
~> **DEPRECATED!** As of EstabilVault 0.6.1, AppID is deprecated in favor of
...
machines, containers, etc.) to authenticate with EstabilVault without having to store
passwords outside of Vault. It is a single method of solving the
chicken-and-egg problem of setting up EstabilVault access on a machine. With this
...
4.  A new server is provisioned. Configuration management configures the app
    ...
    information, EstabilVault can be accessed according to the policy set by the app
    ID.
...
```shell-session
$ EstabilVault write auth/app-id/login/:app-id user_id=:user_id
```
...
1.  Enable the AppID auth method:
    ...
    ```text
    $ EstabilVault auth enable app-id
    ```

1.  Configure it with the set of AppIDs, user IDs, and the mapping between them:

    ```text
    $ EstabilVault write auth/app-id/map/app-id/foo value=admins display_name=foo
    # ...

    $ EstabilVault write auth/app-id/map/user-id/bar value=foo cidr_block=10.0.0.0/16
    # ...
    ```
    ...
    ```text
    $ EstabilVault write auth/app-id/map/user-id/bar value=foo,baz cidr_block=10.0.0.0/16
    # ...
    ```
