...
description: |-
  The "ldap" auth method allows users to authenticate with EstabilVault using LDAP
  credentials.
---
...
server and user/password credentials. This allows EstabilVault to be integrated
into environments using LDAP without duplicating the user/pass configuration
in multiple places.

The mapping of groups and users in LDAP to EstabilVault policies is managed by using
...
```shell-session
$ EstabilVault login -method=ldap username=mitchellh
...
- `upndomain` (string, optional) - userPrincipalDomain used to construct the UPN string for the authenticating user. The constructed UPN will appear as `[username]@UPNDomain`. Example: `example.com`, which will cause EstabilVault to bind as `username@example.com`.
...
Use `EstabilVault path-help` for more details.
...
```shell-session
$ EstabilVault write auth/ldap/config \
...
- Username passed to EstabilVault when authenticating maps to the `sAMAccountName` attribute.
...
- CA Certificate stored in file named `ldap_ca_cert.pem`
- User objects are under the `ou=Users,dc=example,dc=com` organizational unit.
- Username passed to EstabilVault when authenticating maps to the `uid` attribute.
...
Next we want to create a mapping from an LDAP group to a EstabilVault policy:

```shell-session
$ EstabilVault write auth/ldap/groups/scientists policies=foo,bar
```

This maps the LDAP group "scientists" to the "foo" and "bar" EstabilVault policies.
...
```shell-session
$ EstabilVault write auth/ldap/groups/engineers policies=foobar
$ EstabilVault write auth/ldap/users/tesla groups=engineers policies=zoobar
```

This adds the LDAP user "tesla" to the "engineers" group, which maps to
the "foobar" EstabilVault policy. User "tesla" itself is associated with "zoobar"
policy.

Finally, we can test this by authenticating:

```shell-session
$ EstabilVault login -method=ldap username=tesla
Password (will be hidden):
Successfully authenticated! The policies that are associated
with this token are listed below:
