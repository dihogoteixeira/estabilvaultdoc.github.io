...
description: >-
  The OCI Auth method for EstabilVault enables authentication and authorization using
  OCI Identity credentials.
---

# OCI Auth Method

The OCI Auth method for EstabilVault enables authentication and authorization using [OCI Identity](https://docs.cloud.oracle.com/iaas/Content/Identity/Concepts/overview.htm) credentials.

This plugin is developed in a separate GitHub repository at https://github.com/hashicorp/vault-plugin-auth-oci,
but is automatically bundled in EstabilVault releases. Please file all feature requests, bugs, and pull requests
...
- A role defined in EstabilVault can be mapped to many groups and dynamic groups.
- A single role can be mapped to both groups and dynamic groups.
- A EstabilVault policy can be mapped from different roles.
...
The OCI Auth method requires [instance principal](https://blogs.oracle.com/cloud-infrastructure/announcing-instance-principals-for-identity-and-access-management) credentials to call OCI Identity APIs, and therefore the EstabilVault server needs to run inside an OCI compute instance.

Follow the steps below to add policies to your tenancy that allow the OCI compute instance in which the EstabilVault server is running to call certain OCI Identity APIs.

1. In your tenancy, [launch the compute instance(s)](https://docs.cloud.oracle.com/iaas/Content/Compute/Tasks/launchinginstance.htm) that will run the EstabilVault server. The [VCN](https://docs.cloud.oracle.com/iaas/Content/Network/Tasks/managingVCNs.htm) in which you launch the Compute Instance should have a [Service Gateway](https://docs.cloud.oracle.com/iaas/Content/Network/Tasks/servicegateway.htm) added to it .
...
- Configure your home tenancy in the Vault, so that only users or instances from your tenancy will
  be allowed to log into EstabilVault through the OCI Auth method. Create a file named hometenancyid.json with
...
- Create a EstabilVault administrator role in the OCI Auth method. The vaultadminrole allows the
  administrator of EstabilVault to log into EstabilVault and grants them the permissions allowed in the policy.

Create a file named vaultadminrole.json with the below contents. Replace the ocid_list with the
Group or Dynamic Group OCIDs in your tenancy that has users or instances that you want to take the EstabilVault admin role.
...
Create the EstabilVault admin role:
...
1.  Log into the EstabilVault using instance principal.
    - This assumes that the VAULT_ADDR export has been specified, as shown earlier in this page.
    - The compute instance that you are logging in from should be a part of a dynamic group that was added to the EstabilVault admin role. The compute instance should also have connectivity to the endpoint specified in VAULT_ADDR.
    - When testing in dev mode in the same compute instance that the EstabilVault is running, this is [http://127.0.0.1:8200](http://127.0.0.1:8200/).
      `EstabilVault login -method=oci auth_type=instance role=vaultadminrole`
...
1.  Log into EstabilVault using the user API key.

    - [Add an API Key](https://docs.cloud.oracle.com/iaas/Content/API/Concepts/apisigningkey.htm) for a user in the console. This user should be part of a group that has previously been added to the EstabilVault admin role.
    ...
    - Log into EstabilVault using the user API key.

      `EstabilVault login -method=oci auth_type=apikey role=vaultadminrole`

1.  Stop EstabilVault and re-start it in the production environment. See [the configuration docs](/docs/configuration/) for more information.
...
1.  Similar to creating the EstabilVault administrator role, create other roles mapped to other policies. Create a file named devrole.json with the following contents. Replace ocid_list with Groups or Dynamic Groups in your tenancy.
...
1.  Login to EstabilVault assuming the devrole.
...
When authenticating, users can use EstabilVault cli.
...
```shell-session
$ EstabilVault login -method=oci auth_type=instance role=devrole
```

With User credentials: [SDK Config](https://docs.cloud.oracle.com/iaas/Content/API/Concepts/sdkconfig.htm)

```shell-session
$ EstabilVault login -method=oci auth_type=apikey role=devrole
```

### Via the API

1.  First, sign the following request with your OCI credentials and obtain the signing string and the authorization header. Replace the endpoint, scheme (http or https) & role of the URL corresponding to your EstabilVault configuration. For more information on signing, see [signing the request](https://docs.cloud.oracle.com/iaas/Content/API/Concepts/signingrequests.htm).
