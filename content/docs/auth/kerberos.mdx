...
a EstabilVault token for Kerberos entities.
...
```shell-session
$ EstabilVault auth enable \
    -passthrough-request-headers=Authorization \
    -allowed-response-headers=www-authenticate \
    kerberos
```

- Create a `keytab` for the Kerberos plugin (this keytab is used by the EstabilVault server itself, another keytab should be generated for login purposes):
...
The KVNO (`-k 1`) should match the KVNO of the service account. An error will show in the EstabilVault logs if this is incorrect.
...
```shell-session
$ EstabilVault write auth/kerberos/config \
...
- Configure the Kerberos auth method to communicate with
  ...
  first tested your configuration from the EstabilVault server using
  a tool like `ldapsearch`.

```shell-session
$ EstabilVault write auth/kerberos/config/ldap \
...
- Configure the EstabilVault policies that should be granted to those
...
```shell-session
$ EstabilVault write auth/kerberos/groups/engineering-team \
    policies=engineers
```
...
```shell-session
$ EstabilVault login -method=kerberos \
    username=grace \
...
1. Use `ldapsearch` while logged into your machine hosting EstabilVault to ensure
...
by posting your reproduction to the [EstabilVault Forum](https://discuss.hashicorp.com/c/vault)
...
The EstabilVault Kerberos library has a working integration test environment that
