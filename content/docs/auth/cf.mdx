...
The `cf` auth method provides an automated mechanism to retrieve a EstabilVault token
...
1. You construct a request to EstabilVault including your `CF_INSTANCE_CERT`, signed by your `CF_INSTANCE_KEY`.
2. EstabilVault validates that the signature is no more than 300 seconds old, or 60 seconds in the future.
3. EstabilVault validates that the cert was issued by the CA certificate you've pre-configured.
4. EstabilVault validates that the request was signed by the private key for the `CF_INSTANCE_CERT`.
5. EstabilVault validates that the `CF_INSTANCE_CERT` application ID, space ID, and org ID presently exist.
6. If all checks pass, EstabilVault issues an appropriately-scoped token.
...
someone with access to them to self-issue identity certificates that meet the criteria for a EstabilVault role,
...
run `$ cf api` to view the API endpoint that EstabilVault will use.

Next, configure a user for EstabilVault to use. This plugin was tested with Org Manager level
permissions, but lower level permissions _may_ be usable.

```shell-session
$ cf create-user EstabilVault pa55w0rd
$ cf orgs
$ cf org-users my-example-org
$ cf set-org-role EstabilVault my-example-org OrgManager
```
...
After obtaining the information described above, a EstabilVault operator will configure the CF auth method
like so:

```shell-session
$ EstabilVault auth enable cf

$ EstabilVault write auth/cf/config \
      identity_ca_certificates=@ca.crt \
      cf_api_addr=https://api.dev.cfdev.sh \
      cf_username=EstabilVault \
      cf_password=pa55w0rd \
      cf_api_trusted_certificates=@cfapi.crt

$ EstabilVault write auth/cf/roles/my-role \
...
```shell-session
$ EstabilVault login -method=cf role=test-role
```

For CF, we do also offer an agent that, once configured, can be used to obtain a EstabilVault token on
your behalf.
...
$ EstabilVault write auth/vault-plugin-auth-cf/config identity_ca_certificates="$CURRENT" identity_ca_certificates="$FUTURE"
```

If EstabilVault receives a `CF_INSTANCE_CERT` matching _any_ of the `identity_ca_certificates`,
...
an expired certificate, an incorrect `ca.crt`, or a EstabilVault configuration that doesn't
