...
An "AppRole" represents a set of EstabilVault policies and login constraints that must
...
```shell-session
$ EstabilVault write auth/approle/login \
...
for a code snippet demonstrating the authentication with EstabilVault using the
AppRole auth method.
...
1. Create a named role:
   ...
   ```shell-session
   $ EstabilVault write auth/approle/role/my-role \
       secret_id_ttl=10m \
       token_num_uses=10 \
       token_ttl=20m \
       token_max_ttl=30m \
...
1. Fetch the RoleID of the AppRole:
   ...
   ```shell-session
   $ EstabilVault read auth/approle/role/my-role/role-id
   role_id     db02de05-fa39-4855-059b-67221c5c2f63
   ```
...
1. Get a SecretID issued against the AppRole:
   ...
   ```shell-session
   $ EstabilVault write -f auth/approle/role/my-role/secret-id
   secret_id               6a174c20-f6de-a53c-74d2-6018fcceff64
...
func getSecretWithAppRole() (string, error) {
	...
	// with an AppRole.
	// First, let's get the role ID given to us by our EstabilVault administrator.
...
namespace Examples
{
    public class ApproleAuthExample
    {
        ...
        /// <summary>
        /// Fetches a key-value secret (kv-v2) after authenticating to EstabilVault via AppRole authentication
        /// </summary>
        public string GetSecretWithAppRole()
        {
            // A combination of a Role ID and Secret ID is required to log in to EstabilVault with an AppRole.
