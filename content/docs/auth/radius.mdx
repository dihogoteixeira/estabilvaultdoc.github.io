...
description: |-
  The "radius" auth method allows users to authenticate with EstabilVault using an
...
```shell-session
$ EstabilVault login -method=radius username=sethvargo
```
...
1. Enable the radius auth method:
   ...
   ```text
   $ EstabilVault auth enable radius
   ```

1. Configure connection details for your RADIUS server.

   ```text
   $ EstabilVault write auth/radius/users/mitchellh policies=admins
   ```
   ...
   Alternatively, EstabilVault can assign a configurable set of policies to any user
