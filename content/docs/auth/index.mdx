...
Auth methods are the components in EstabilVault that perform authentication and are
responsible for assigning identity and a set of policies to a user. In all cases,
EstabilVault will enforce authentication as part of the request processing. In most cases,
EstabilVault will delegate the authentication administration and decision to the relevant configured
...
most sense for your use case of EstabilVault and your organization.
...
they are mounted within the EstabilVault mount table and can be accessed
...
```shell-session
$ EstabilVault auth enable -path=my-login userpass
```
...
When using an external auth method (e.g., GitHub), EstabilVault will call the external service
