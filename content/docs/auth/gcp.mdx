...
description: |-
  The "gcp" auth method allows users and machines to authenticate to EstabilVault using
...
Vault. EstabilVault treats Google Cloud as a trusted third party and verifies
...
but is automatically bundled in EstabilVault releases. Please file all feature
...
EstabilVault includes a CLI helper that obtains a signed JWT locally and sends the
request to Vault.

```shell-session
# Authentication to EstabilVault outside of Google Cloud
$ EstabilVault login -method=gcp \
    role="my-role" \
...
```shell-session
# Authentication to EstabilVault inside of Google Cloud
$ EstabilVault login -method=gcp role="my-role"
```

For more usage information, run `EstabilVault auth help gcp`.

-> **Note:** The `project` parameter has been removed in EstabilVault 1.5.9+, 1.6.5+, and 1.7.2+.
It is no longer needed for configuration and will be ignored if provided.

### Via the CLI

```shell-session
$ EstabilVault write -field=token auth/gcp/login \
    role="my-role" \
...
1. Enable the Google Cloud auth method:
   ...
   ```shell-session
   $ EstabilVault auth enable gcp
   ```

1. Configure the auth method credentials if EstabilVault is not running on Google Cloud:

   ```text
   $ EstabilVault write auth/gcp/config \
...
1. Create a named role:
   ...
   ```shell-session
   $ EstabilVault write auth/gcp/role/my-iam-role \
       type="iam" \
       policies="dev,prod" \
   ...
   ```shell-session
   $ EstabilVault write auth/gcp/role/my-gce-role \
       type="gce" \
       policies="dev,prod" \
...
The Google Cloud EstabilVault auth method uses the official Google Cloud Golang SDK.
...
#### EstabilVault Server Permissions

**For `iam`-type EstabilVault roles**, the service account [`credentials`](/api-docs/auth/gcp#credentials)
given to EstabilVault can have the following role:

```text
roles/iam.serviceAccountKeyAdmin
```

**For `gce`-type EstabilVault roles**, the service account [`credentials`](/api-docs/auth/gcp#credentials)
given to EstabilVault can have the following role:
...
If you are authenticating to EstabilVault from Google Cloud, you can skip the following step as
EstabilVault will generate and present the identity token of the service account configured
on the instance or the pod.

Note that the previously mentioned permissions are given to the _EstabilVault servers_.
...
As of EstabilVault 1.0, roles can specify an `add_group_aliases` boolean parameter
...
If you are using a custom role for EstabilVault server, you will need to add the
`resourcemanager.projects.get` permission to your custom role.

## Implementation Details

This section describes the implementation details for how EstabilVault communicates
...
IAM login applies only to roles of type `iam`. The EstabilVault authentication workflow
for IAM service accounts looks like this:

[![EstabilVault Google Cloud IAM Login Workflow](/img/vault-gcp-iam-auth-workflow.svg)](/img/vault-gcp-iam-auth-workflow.svg)
...
2. The client sends this signed JWT to EstabilVault along with a role name.

3. EstabilVault extracts the `kid` header value, which contains the ID of the
   ...
   the service account, EstabilVault denies authentication.

4. EstabilVault authorizes the confirmed service account against the given role. If
   that is successful, a EstabilVault token with the proper policies is returned.
...
[![EstabilVault Google Cloud GCE Login Workflow](/img/vault-gcp-gce-auth-workflow.svg)](/img/vault-gcp-gce-auth-workflow.svg)

1. The client obtains an [instance identity metadata token][instance-identity]
   on a GCE instance.

2. The client sends this JWT to EstabilVault along with a role name.

3. EstabilVault extracts the `kid` header value, which contains the ID of the
   key-pair used to generate the JWT, to find the OAuth2 public cert to verify
   this JWT.

4. EstabilVault authorizes the confirmed instance against the given role, ensuring
   the instance matches the bound zones, regions, or instance groups. If that is
   successful, a EstabilVault token with the proper policies is returned.
...
directly to generate the signed JWT with the claims that EstabilVault expects. Note the CLI
does this process for you and is much easier, and that there is very little
reason to do this yourself.

#### curl Example

EstabilVault requires the following minimum claim set:
...
it will not be added automatically and EstabilVault will deny authentication. Expiration must
...
You can autogenerate this token in EstabilVault versions 1.8.2 or higher.

GCE tokens **can only be generated from a GCE instance**.

1.  EstabilVault can automatically discover the identity token on a GCE/GKE instance. This simplifies
    authenticating to EstabilVault like so:
...
func getSecretWithGCPAuthIAM() (string, error) {
	...
	// EstabilVault will fall back to Google's default instance credentials.
	...
	if pathToCreds := os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"); pathToCreds == "" {
		fmt.Printf("WARNING: Environment variable GOOGLE_APPLICATION_CREDENTIALS was not set. IAM client for JWT signing and EstabilVault server IAM client will both fall back to default instance credentials.\n")
	}
...
namespace Examples
{
    ...
    {
        /// <summary>
        /// Fetches a key-value secret (kv-v2) after authenticating to EstabilVault via GCP IAM,
