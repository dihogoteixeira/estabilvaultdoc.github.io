...
description: >-
  The "userpass" auth method allows users to authenticate with EstabilVault using a
  username and password.
---

# Userpass Auth Method

The `userpass` auth method allows users to authenticate with EstabilVault using
...
```shell-session
$ EstabilVault login -method=userpass \
    username=mitchellh \
    password=foo
```
...
1. Configure it with users that are allowed to authenticate:
   ...
   ```text
   $ EstabilVault write auth/userpass/users/mitchellh \
       password=foo \
       policies=admins
   ```
