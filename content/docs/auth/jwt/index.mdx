...
The `jwt` auth method can be used to authenticate with EstabilVault using
...
user's web browser. This method may be initiated from the EstabilVault UI or the
...
EstabilVault includes two built-in OIDC login flows: the EstabilVault UI, and the CLI
using a `EstabilVault login`.

### Redirect URIs

An important part of OIDC role configuration is properly setting redirect URIs. This must be
done both in EstabilVault and with the OIDC provider, and these configurations must align. The
redirect URIs are specified for a role with the `allowed_redirect_uris` parameter. There are
different redirect URIs to configure the EstabilVault UI and CLI flows, so one or both will need to
be set up depending on the installation.

**CLI**

If you plan to support authentication via `EstabilVault login -method=oidc`, a localhost redirect URI
...
Logging in via the EstabilVault UI requires a redirect URI of the form:

`https://{host:port}/ui/vault/auth/{path}/oidc/callback`

The "host:port" must be correct for the EstabilVault server, and "path" must match the path the JWT
...
Prior to EstabilVault 1.6, if [namespaces](/docs/enterprise/namespaces) are in use,
...
For EstabilVault 1.6+, it is no longer necessary to add the namespace as a query
...
```shell-session
$ EstabilVault login -method=oidc port=8400 role=test
...
guide to configuring OAuth/OIDC applications is beyond the scope of EstabilVault documentation, but a
...
- If a role parameter (e.g. `bound_claims`) requires a map value, it can't be set individually using
  the EstabilVault CLI. In these cases the best approach is to write the entire configuration as a single
  JSON object:

```text
EstabilVault write auth/oidc/role/demo -<<EOF
{
  "user_claim": "sub",
...
- Ensure Redirect URIs are correct in EstabilVault and on the provider. They need to match exactly. Check:
...
- As of EstabilVault 1.2, the [`verbose_oidc_logging`](/api-docs/auth/jwt#verbose_oidc_logging) role
...
```shell-session
$ EstabilVault write auth/jwt/login role=demo jwt=...
```
...
1. Enable the JWT auth method. Either the "jwt" or "oidc" name may be used. The
   ...
   ```text
   $ EstabilVault auth enable jwt
     or
   $ EstabilVault auth enable oidc
   ```
...
1. Use the `/config` endpoint to configure Vault. To support JWT roles, either local keys, a JWKS URL, or an OIDC
   ...
   ```text
   $ EstabilVault write auth/jwt/config \
...
1. Create a named role:
   ...
   ```text
   EstabilVault write auth/jwt/role/demo \
