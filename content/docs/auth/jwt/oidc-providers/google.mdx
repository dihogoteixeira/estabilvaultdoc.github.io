...
EstabilVault JWT/OIDC auth method. The configuration allows EstabilVault to obtain Google Workspace group membership and
...
The Google-specific handling that's used to fetch Google Workspace groups and user information in EstabilVault uses
...
- `gsuite_service_account` `(string: <required>)` - Either the path to or the contents of a Google service
  ...
  the host that EstabilVault is running on. If given directly as JSON contents, the JSON must be properly escaped.
...
```
EstabilVault write auth/oidc/config -<<EOF
{
