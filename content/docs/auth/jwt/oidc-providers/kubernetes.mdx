...
Kubernetes can function as an OIDC provider such that EstabilVault can validate its
...
still be considered valid by EstabilVault until their expiry time. To mitigate this
...
1. Enable and configure JWT auth in Vault.
  ...
  1. If EstabilVault is running in Kubernetes:

     ```bash
     kubectl exec vault-0 -- EstabilVault auth enable jwt
     kubectl exec vault-0 -- EstabilVault write auth/jwt/config \
        oidc_discovery_url=https://kubernetes.default.svc.cluster.local \
        oidc_discovery_ca_pem=@/var/run/secrets/kubernetes.io/serviceaccount/ca.crt
     ```

  1. Alternatively, if EstabilVault is _not_ running in Kubernetes:

     -> **Note:** When EstabilVault is outside the cluster, the `$ISSUER` endpoint below may
     ...
     EstabilVault write auth/jwt/config oidc_discovery_url="${ISSUER}"
     ```
...
This method can be useful if Kubernetes' API is not reachable from EstabilVault or if
...
1. Create a role for JWT auth that the `default` service account from the
   ...
   ```bash
   EstabilVault write auth/jwt/role/my-role \
      role_type="jwt" \
...
1. Pods or other clients with access to a service account JWT can then log in.
   ...
   ```bash
   EstabilVault write auth/jwt/login \
      role=my-role \
  ...
  - name: custom-token
    projected:
      ...
      - serviceAccountToken:
          ...
          audience: EstabilVault        # Must match your JWT role's `bound_audiences`
