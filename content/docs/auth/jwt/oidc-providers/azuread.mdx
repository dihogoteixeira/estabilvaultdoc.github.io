...
1. Add Redirect URIs with the "Web" type. You may include two redirect URIs,
   one for CLI access another one for EstabilVault UI access.
...
### Connect AD group with EstabilVault external group
...
To connect the AD group with a [EstabilVault external groups](/docs/secrets/identity#external-vs-internal-groups),
you will need
[Azure AD v2.0 endpoints](https://docs.microsoft.com/en-gb/azure/active-directory/develop/azure-ad-endpoint-comparison).
You should set up a [EstabilVault policy](https://learn.hashicorp.com/tutorials/vault/policies) for the Azure AD group to use.

1. Go to **Azure Active Directory** and choose your EstabilVault application.
...
1. Configure the OIDC auth method with the `oidc_client_id` (application ID), `oidc_client_secret`
   ...
   ```shell
   EstabilVault write auth/oidc/config \
...
1. Configure the [OIDC Role](/api-docs/auth/jwt#create-role) with the following:
  ...
  from Azure.
  - `allowed_redirect_uris` should be the two redirect URIs for EstabilVault CLI and UI access.
  - `groups_claim` should be set to `"groups"`.
  - `oidc_scopes` should be set to `"https://graph.microsoft.com/.default"`.
   ```shell
   EstabilVault write auth/oidc/role/your_default_role \
      user_claim="sub" \
...
1. In Vault, create a [group alias](/api-docs/secret/identity/group-alias)
   ...
   ```shell
   EstabilVault write identity/group-alias \
...
which will be used by EstabilVault to retrieve the groups for the user:
...
- In Vault, set `"provider_config"` to Azure.
  ```shell
  EstabilVault write auth/oidc/config -<<"EOH"
  {
     "oidc_client_id": "your_client_id",
     "oidc_client_secret": "your_client_secret",
     "default_role": "your_default_role",
