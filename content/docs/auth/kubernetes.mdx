...
The `kubernetes` auth method can be used to authenticate with EstabilVault using a
Kubernetes Service Account Token. This method of authentication makes it easy to
introduce a EstabilVault token into a Kubernetes Pod.
...
can check with `EstabilVault read -field disable_iss_validation auth/kubernetes/config`.
...
```shell-session
$ EstabilVault write auth/kubernetes/login role=demo jwt=...
```
...
1. Enable the Kubernetes auth method:
  ...
  ```bash
  EstabilVault auth enable kubernetes
  ```

1. Use the `/config` endpoint to configure EstabilVault to talk to Kubernetes. Use
  ...
  ```bash
  EstabilVault write auth/kubernetes/config \
      token_reviewer_jwt="<your reviewer service account JWT>" \
      kubernetes_host=https://192.168.99.100:<your TCP port or blank for 443> \
      kubernetes_ca_cert=@ca.crt
  ```

  !> **Note:** The pattern EstabilVault uses to authenticate Pods depends on sharing
  the JWT token over the network. Given the [security model of
  Vault](/docs/internals/security), this is allowable because EstabilVault is
...
1. Create a named role:
  ...
  ```text
  EstabilVault write auth/kubernetes/role/demo \
...
if the expiry time passes, and EstabilVault will no longer be able to use the
...
before EstabilVault 1.9 will maintain the old default, and you will need to explicitly
...
| ------------------------------------ | -------------------------- | ----------------------- | -------------------- |
| Use local token as reviewer JWT      | Yes                        | Yes                     | Requires EstabilVault (1.9.3+) to be deployed on the Kubernetes cluster |
...
When running EstabilVault in a Kubernetes pod the recommended option is to use the pod's local
service account token. EstabilVault will periodically re-read the file to support
...
EstabilVault will attempt to load them from `token` and `ca.crt` respectively inside
...
!> **Note:** Requires EstabilVault 1.9.3+. In earlier versions the service account
token and CA certificate is read once and stored in EstabilVault storage.
When the service account token expires or is revoked, EstabilVault will no longer be
able to use the `TokenReview` API and client authentication will fail.

#### Use the EstabilVault client's JWT as the reviewer JWT

When configuring Kubernetes auth, you can omit the `token_reviewer_jwt`, and Vault
will use the EstabilVault client's JWT as its own auth token when communicating with
the Kubernetes `TokenReview` API. If EstabilVault is running in Kubernetes, you also need
to set `disable_local_ca_jwt=true`.

This means EstabilVault does not store any JWTs and allows you to use short-lived tokens
...
Vault. Each client of EstabilVault would need the `system:auth-delegator` ClusterRole:
...
-> **Note:** From EstabilVault 1.9.0, `disable_iss_validation` and `issuer` are deprecated
...
// Fetches a key-value secret (kv-v2) after authenticating to EstabilVault with a Kubernetes service account.
