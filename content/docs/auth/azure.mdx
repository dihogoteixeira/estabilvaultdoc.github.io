...
The `azure` auth method allows authentication against EstabilVault using
...
If EstabilVault is hosted on Azure, EstabilVault can use MSI to access Azure instead of a shared secret.
...
```shell-session
$ EstabilVault write auth/azure/login \
    role="dev-role" \
...
```shell-session
$ EstabilVault write auth/azure/login role="dev-role" \
...
1. Enable Azure authentication in Vault:
   ...
   ```text
   $ EstabilVault auth enable azure
   ```

1. Configure the Azure auth method:

   ```text
   $ EstabilVault write auth/azure/config \
...
1. Create a role:
   ...
   ```text
   $ EstabilVault write auth/azure/role/dev-role \
       policies="prod,dev" \
...
frequently, using system-assigned identities could result in many EstabilVault entities being
...
// Fetches a key-value secret (kv-v2) after authenticating to EstabilVault via Azure authentication.
