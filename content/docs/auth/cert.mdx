...
description: >-
  The "cert" auth method allows users to authenticate with EstabilVault using TLS
  client certificates.
---
...
Since EstabilVault 0.4, the method supports revocation checking.
...
trusted CRL distribution point, and have EstabilVault fetch the CRL as needed.
...
~> **NOTE** The `-ca-cert` value used here is for the EstabilVault TLS Listener CA
certificate, not the CA that issued the client authentication certificate. This
can be omitted if the CA used to issue the EstabilVault server certificate is trusted
...
~> **NOTE** The `--cacert` value used here is for the EstabilVault TLS Listener CA
certificate, not the CA that issued the client authentication certificate. This
can be omitted if the CA used to issue the EstabilVault server certificate is trusted
by the local system executing this command.
