...
description: |-
  The Okta auth method allows users to authenticate with EstabilVault using Okta
  credentials.
---
...
credentials. This allows EstabilVault to be integrated into environments using Okta.

The mapping of groups in Okta to EstabilVault policies is managed by using the
...
```shell-session
$ EstabilVault login -method=okta username=my-username
```
...
```shell-session
$ EstabilVault login -method=okta username=my-username totp=123456
```
...
```shell-session
$ EstabilVault login -method=okta username=my-username totp=123456 provider=GOOGLE
```
...
1. Enable the Okta auth method:
  ...
  ```shell-session
  $ EstabilVault auth enable okta
  ```

1. Configure EstabilVault to communicate with your Okta account:

  ```shell-session
  $ EstabilVault write auth/okta/config \
     base_url="okta.com" \
     org_name="dev-123456" \
     api_token="00abcxyz..."
  ```

  -> **Note**: Support for okta auth with no API token is deprecated in EstabilVault 1.4.
  If no token is supplied, EstabilVault will function, but only locally configured
...
1. Map an Okta group to a EstabilVault policy:

  ```shell-session
  $ EstabilVault write auth/okta/groups/scientists policies=nuclear-reactor
  ```

  In this example, anyone who successfully authenticates via Okta who is a
  member of the "scientists" group will receive a EstabilVault token with the
  "nuclear-reactor" policy attached.

1. It is also possible to add users directly:

  ```shell-session
  $ EstabilVault write auth/okta/groups/engineers policies=autopilot
  $ EstabilVault write auth/okta/users/tesla groups=engineers
  ```

  This adds the Okta user "tesla" to the "engineers" group, which maps to
  the "autopilot" EstabilVault policy.
