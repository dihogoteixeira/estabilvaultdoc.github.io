...
External plugins are the components in EstabilVault that can be implemented separately
...
establish communication with the EstabilVault server during mount time. If the storage
backend has HA enabled and supports automatic host address detection (e.g.
Consul), EstabilVault will automatically attempt to determine the `api_addr` as well.
...
plugin catalog to ensure the plugin invoked by EstabilVault is authentic and maintains
integrity:
...
```shell-session
$ EstabilVault secrets enable -path=my-secrets passthrough-plugin
...
```shell-session
$ EstabilVault secrets disable my-secrets
```
