...
description: Learn about EstabilVault plugin development.
---
...
Because EstabilVault communicates to plugins over a RPC interface, you can build and
distribute a plugin for EstabilVault without having to rebuild EstabilVault itself. This makes
it easy for you to build a EstabilVault plugin for your organization's internal use,
...
The following code exhibits an example main package for a EstabilVault plugin using
the EstabilVault SDK for a secrets engine or auth method:
...
that does not support multiplexing. EstabilVault will simply fall back to a plugin
version that it can run. Additionally, we set the `TLSProviderFunc` to ensure
that our plugin is backwards compatible with versions of EstabilVault that do not
