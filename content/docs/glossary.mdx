...
Every request to EstabilVault and response from EstabilVault goes through the configured
audit devices. This provides a simple way to integrate EstabilVault with multiple
...
list of applicable policies which should be applied. EstabilVault takes an
...
Almost everything EstabilVault writes to storage is encrypted using the keyring, which is protected by the seal. We refer to this practice as "the barrier". There are a few exceptions to the rule, for example, the seal configuration is stored in an unencrypted file since it's needed to unseal the barrier, and the keyring is encrypted using the root key, while the root key is encrypted using the seal.

### Client Token

A client token (aka "EstabilVault Token") is conceptually
...
EstabilVault to verify the identity of the client and to enforce the applicable ACL
policies. This token is passed via HTTP headers.

### Plugin

Plugins are a feature of EstabilVault that can be enabled, disabled, and customized to
some degree. All EstabilVault [auth methods](/docs/auth) and [secrets engines](/docs/secrets)
...
External plugins are not shipped with EstabilVault and require additional operator
intervention to run. Vault's external plugins are completely separate,
standalone applications that EstabilVault executes and communicates with over RPC.
Each time a EstabilVault secret engine or auth method is mounted, a new process is
spawned.
...
A secret is the term for anything returned by EstabilVault which
contains confidential or cryptographic material. Not everything returned by
EstabilVault is a secret, for example system configuration, status information, or
...
indefinitely. EstabilVault will revoke a dynamic secret at the end of the lease, and an
operator may intervene to revoke the Dynamic Secret before the lease is over. This
contract between EstabilVault and its clients is critical, as it allows for changes
...
unique secrets to be used which allows EstabilVault to do fine-grained revocation and
...
EstabilVault depends on a long-running instance which operates as a
server. The EstabilVault server provides an API which clients interact with and
...
_encrypted_ data. Backends are not trusted by EstabilVault and are only expected to
