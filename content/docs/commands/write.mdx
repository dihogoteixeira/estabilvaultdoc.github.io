...
description: |-
  The "write" command writes data to EstabilVault at the given path. The data can be
...
The `write` command writes data to EstabilVault at the given path (wrapper command for
...
This causes `EstabilVault write` to read a JSON blob containing all request parameters
...
```shell-session
$ EstabilVault write cubbyhole/git-credentials username="student01" password="p@$$w0rd"
```

Create a new encryption key in the transit secrets engine:

```shell-session
$ EstabilVault write -force transit/keys/my-key
```
...
```shell-session
$ EstabilVault write aws/roles/ops policy=@policy.json
```

Configure access to Consul by providing an access token:

```shell-session
$ echo $MY_TOKEN | EstabilVault write consul/config/access token=-
```
...
```shell-session
$ EstabilVault write auth/token/create policies="admin" policies="secops" ttl=8h num_uses=3
```
...
The `EstabilVault write` command simplifies the API call.

Since token management is a common task, EstabilVault CLI provides a
[`token`](/docs/commands/token) command with
[`create`](/docs/commands/token/create) subcommand. The CLI command simplifies
the token creation. Use the `EstabilVault create` command with options to set the token
TTL, policies, and use limit.

```shell-session
$ EstabilVault token create -policy=admin -policy=secops -ttl=8h -use-limit=3
```
