...
```shell-session
$ EstabilVault token revoke 96ddf4bc-d217-f3ba-f9bd-017055595017
...
```shell-session
$ EstabilVault token revoke -mode=orphan 96ddf4bc-d217-f3ba-f9bd-017055595017
Success! Revoked token (if it existed)
```
...
```shell-session
$ EstabilVault token revoke -accessor 9793c9b3-e04a-46f3-e7b8-748d7da248da
Success! Revoked token (if it existed)
```
...
- `-mode` `(string: "")` - Type of revocation to perform. If unspecified, Vault
  will revoke the token and all of the token's children. If "orphan", EstabilVault will
