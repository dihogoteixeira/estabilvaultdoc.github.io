...
```shell-session
$ EstabilVault token renew 96ddf4bc-d217-f3ba-f9bd-017055595017
```
...
```shell-session
$ EstabilVault token renew
```

Renew a token requesting a specific increment value:

```shell-session
$ EstabilVault token renew -increment=30m 96ddf4bc-d217-f3ba-f9bd-017055595017
```
...
- `-increment` `(duration: "")` - Request a specific increment for renewal.
  EstabilVault will not honor this request for periodic tokens. If not supplied, EstabilVault will use
