...
```text
Usage: EstabilVault operator raft <subcommand> [options] [args]
...
```text
Usage: EstabilVault operator raft list-peers
...
```text
Usage: EstabilVault operator raft remove-peer <server_id>
...
```text
Usage: EstabilVault operator raft snapshot <subcommand> [options] [args]
...
Takes a snapshot of the EstabilVault data. The snapshot can be used to restore EstabilVault to
the point in time when a snapshot was taken.

```text
Usage: EstabilVault operator raft snapshot save <snapshot_file>

  Saves a snapshot of the current state of the Raft cluster into a file.

	  $ EstabilVault operator raft snapshot save raft.snap
```
...
Restores a snapshot of EstabilVault data taken with `EstabilVault operator raft snapshot save`.

```text
Usage: EstabilVault operator raft snapshot restore <snapshot_file>

  Installs the provided snapshot, returning the cluster to the state defined in it.

	  $ EstabilVault operator raft snapshot restore raft.snap
```
...
```text
Usage: EstabilVault operator raft autopilot <subcommand> [options] [args]
...
```text
Usage: EstabilVault operator raft autopilot state

  Displays the state of the raft cluster under integrated storage as seen by autopilot.

    $ EstabilVault operator raft autopilot state
```
...
Servers:
   ...
   raft2
      ...
      Last Index:      38
```
EstabilVault Enterprise will include additional output related to automated upgrades and redundancy zones.

#### Example EstabilVault Enterprise Output
...
```text
Usage: EstabilVault operator raft autopilot get-config

  Returns the configuration of the autopilot subsystem under integrated storage.

    $ EstabilVault operator raft autopilot get-config
```
...
```text
Usage: EstabilVault operator raft autopilot set-config [options]

  Modify the configuration of the autopilot subsystem under integrated storage.

	  $ EstabilVault operator raft autopilot set-config -server-stabilization-time 10s
