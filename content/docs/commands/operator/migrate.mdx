...
description: >-
  ...
  migrating EstabilVault between configurations. It operates directly at the storage
...
```shell-session
$ EstabilVault operator migrate -config migrate.hcl
...
```shell-session
$ EstabilVault operator migrate -config migrate.hcl -start "data/logical/fd"
```
...
EstabilVault will need to be offline during the migration process. First, stop Vault.
Then, run the migration on the server you wish to become a the new EstabilVault node.
...
- `-reset` - Reset the migration lock. A lock file is added during migration to prevent
  starting the EstabilVault server or another migration. The `-reset` option can be used to
