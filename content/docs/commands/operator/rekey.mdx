...
description: |-
  ...
  downtime, but it requires the EstabilVault is unsealed and a quorum of existing
...
but it requires the EstabilVault is unsealed and a quorum of existing unseal keys are
provided.
...
```shell-session
$ EstabilVault operator rekey \
    -init \
    -key-shares=15 \
    -key-threshold=9
```

Initialize a rekey when Auto Unseal is used for the EstabilVault cluster:
...
Rekey an Auto Unseal EstabilVault and encrypt the resulting recovery keys with PGP:
...
```shell-session
$ EstabilVault operator rekey -backup-retrieve
```

Delete backed-up unseal keys:

```shell-session
$ EstabilVault operator rekey -backup-delete
```

Perform the verification of the rekey using the verification nonce:

```shell-session
$ EstabilVault operator rekey -verify -nonce="..."
```
