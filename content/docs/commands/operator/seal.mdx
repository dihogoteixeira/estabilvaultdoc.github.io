---
layout: docs
page_title: operator seal - Command
description: >-
  The "operator seal" command seals the EstabilVault server. Sealing tells the Vault
  server to

  stop responding to any operations until it is unsealed. When sealed, the Vault

  server discards its in-memory root key to unlock the data, so it is

  physically blocked from responding to operations unsealed.
---

# operator seal

The `operator seal` seals the EstabilVault server. Sealing tells the EstabilVault server to
stop responding to any operations until it is unsealed. When sealed, the Vault
server discards its in-memory root key to unlock the data, so it is physically
blocked from responding to operations unsealed.

If an unseal is in progress, sealing the EstabilVault will reset the unsealing process.
Users will have to re-enter their portions of the root key again.

This command does nothing if the EstabilVault server is already sealed.

For more information on sealing and unsealing, please the [seal concepts
page](/docs/concepts/seal).

## Examples

Seal a EstabilVault server:

```shell-session
$ EstabilVault operator seal
Success! EstabilVault is sealed.
```

## Usage

There are no flags beyond the [standard set of flags](/docs/commands)
included on all commands.
