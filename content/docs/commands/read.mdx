...
description: |-
  The "read" command reads data from EstabilVault at the given path. This can be used
...
The `read` command reads data from EstabilVault at the given path (wrapper command for
...
```shell-session
$ EstabilVault read identity/entity/id/2f09126d-d161-abb8-2241-555886491d97
```
...
```shell-session
$ EstabilVault read secret/data/customers
```
...
Since K/V secrets engine is a commonly used feature, EstabilVault CLI provides the
[`kv`](/docs/commands/kv) command. Read secrets from the `secret/data/customers`
path using the `kv` CLI command:

```shell-session
$ EstabilVault kv get -mount=secret customers
```

-> **Comparison:** All three commands retrieve the same data, but display the
output in a different format. By default, `EstabilVault read` prints output in
