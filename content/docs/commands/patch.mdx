...
description: |-
  The "patch" command updates data in EstabilVault at the given path. The data can be
...
The `patch` command updates data in EstabilVault at the given path (wrapper command for
...
This causes `EstabilVault patch` to read a JSON blob containing all request parameters
...
```shell-session
$ EstabilVault patch pki/roles/example allow_localhost=false
```
...
The `EstabilVault patch` command simplifies the API call.
