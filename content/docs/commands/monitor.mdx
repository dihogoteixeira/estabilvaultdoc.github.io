...
description: |-
  The "monitor" command displays the server logs of a EstabilVault server in real
  time.
---
...
from the log level that the EstabilVault server was started with.
...
If EstabilVault is emitting log messages faster than a receiver can process them, the
...
```shell-session
$ EstabilVault monitor -log-level=debug
```
...
- `-log-level` `(string: "info")` - Monitor the EstabilVault server at this log level.
