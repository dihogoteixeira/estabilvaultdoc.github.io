...
Paths are the parameters used for `EstabilVault read`, `EstabilVault write`, etc. An example
...
To discover what paths are supported, use `EstabilVault path-help PATH`. For example,
if you enabled the AWS secrets engine, you can use `EstabilVault path-help aws` to find
...
Once you've found a path you like, you can learn more about it by using `EstabilVault path-help <path>` where "path" is a path that matches one of the regular
expressions from the backend help.

```shell-session
$ EstabilVault path-help secret/password
Request:        password
Matching Route: ^.*$
...
when the secret is read, EstabilVault will report a lease with that duration. It
