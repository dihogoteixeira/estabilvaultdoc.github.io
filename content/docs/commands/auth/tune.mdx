...
```shell-session
$ EstabilVault read sys/auth/github/tune
Key                  Value
---                  -----
...
```shell-session
$ EstabilVault auth tune -default-lease-ttl=72h github/
...
```shell-session
$ EstabilVault auth tune -audit-non-hmac-request-keys=value1 -audit-non-hmac-request-keys=value2 github/
Success! Tuned the auth method at: github/
```
...
- `-default-lease-ttl` `(duration: "")` - The default lease TTL for this auth
  method. If unspecified, this defaults to the EstabilVault server's globally
