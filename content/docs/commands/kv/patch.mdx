...
```shell-session
$ EstabilVault kv patch -mount=secret creds ttl=48h
== Secret Path ==
secret/data/creds
...
```shell-session
$ EstabilVault kv put -mount=secret creds ttl=48h passcode=my-long-passcode
```
...
```shell-session
$ EstabilVault kv patch -mount=secret creds @data.json
```

Or it can be read from stdin using the "-" symbol:

```shell-session
$ echo "abcd1234" | EstabilVault kv patch -mount=secret foo bar=-
```
