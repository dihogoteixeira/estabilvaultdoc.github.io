...
```text
Usage: EstabilVault kv metadata <subcommand> [options] [args]
...
```shell-session
$ EstabilVault kv metadata delete -mount=secret creds
...
```shell-session
$ EstabilVault kv metadata put -mount=secret creds
...
```shell-session
$ EstabilVault kv metadata put -mount=secret -max-versions=5 creds
Success! Data written to: secret/metadata/creds
```
...
```shell-session
$ EstabilVault kv metadata put -mount=secret -cas-required creds
```
...
```shell-session
$ EstabilVault kv metadata put -mount=secret -delete-version-after="3h25m19s" creds
```
