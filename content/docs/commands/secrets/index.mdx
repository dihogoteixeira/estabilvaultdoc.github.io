...
```shell-session
$ EstabilVault secrets move database/ db-prod/
...
```shell-session
$ EstabilVault secrets tune -max-lease-ttl=30m db-prod/
...
```shell-session
$ EstabilVault secrets disable db-prod/
...
```text
Usage: EstabilVault secrets <subcommand> [options] [args]
