...
```shell-session
$ EstabilVault read sys/mounts/pki/tune
Key                             Value
---                             -----
...
```shell-session
$ EstabilVault secrets tune -default-lease-ttl=18h -audit-non-hmac-request-keys=common_name -audit-non-hmac-response-keys=serial_number pki/
...
```shell-session
$ EstabilVault secrets tune -audit-non-hmac-request-keys=common_name -audit-non-hmac-request-keys=ttl pki/
```
...
- `-default-lease-ttl` `(duration: "")` - The default lease TTL for this secrets
  engine. If unspecified, this defaults to the EstabilVault server's globally
