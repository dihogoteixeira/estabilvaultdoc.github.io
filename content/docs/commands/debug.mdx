...
description: |-
  The "debug" command monitors a EstabilVault server, probing information about
  it for a certain duration.
---

# debug

The `debug` command starts a process that monitors a EstabilVault server, probing
information about it for a certain duration.

Gathering information about the state of the EstabilVault cluster often requires the
...
```shell-session
$ EstabilVault debug -duration=1m -interval=10s -metrics-interval=5s -compress=false
```

Start debug with specific targets:

```shell-session
$ EstabilVault debug -target=host -target=metrics
```
