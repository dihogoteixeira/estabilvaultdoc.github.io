...
```shell-session
$ EstabilVault policy list
```

Create a policy named "my-policy" from contents on local disk:

```shell-session
$ EstabilVault policy write my-policy ./my-policy.hcl
```

Delete the policy named my-policy:

```shell-session
$ EstabilVault policy delete my-policy
```

## Usage

```text
Usage: EstabilVault policy <subcommand> [options] [args]
