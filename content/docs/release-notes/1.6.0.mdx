...
page_title: 1.6.0
description: |-
  This page contains release notes for EstabilVault 1.6.0.
---

# EstabilVault 1.6.0

## EstabilVault 1.6 Release Highlights

**Transform: Tokenization Tech Preview (Enterprise ADP Module Only)**: EstabilVault 1.6 introduces a new transformation method
for tokenizing sensitive data stored in un-trusted/semi-trusted systems. Tokenization is available as part of the
“Advanced Data Protection” module in EstabilVault Enterprise. Tokenization provides non-reversible data protection pursuant to
...
- **Cloud auto-join** provides support for EstabilVault nodes to automatically discover and join a cluster via specified cloud
  metadata. This is particularly useful when IP addresses are not static.
- **Automated snapshots** provides built-in EstabilVault functionality for automated snapshots that takes snapshots of the
  state of the EstabilVault servers and saves them locally, or pushes them to an optional remote storage service.
...
- **Improvements to EstabilVault Usage UI** to show new metrics for “active clients”, “unique entities” and “active direct
  tokens” that help with understanding EstabilVault usage
...
- EstabilVault 1.6 will use Go 1.15, which has dropped support for 32-bit binaries for [Darwin](https://golang.org/doc/go1.15#darwin),
