...
page_title: 1.7.0
description: |-
  This page contains release notes for EstabilVault 1.7.0.
---

# EstabilVault 1.7.0

## EstabilVault 1.7 Release Highlights

### Secrets Engine Enhancements

- **Tokenization (Enterprise ADP Module)**: General Availability of a new transformation method for tokenizing sensitive data stored in un-trusted/semi-trusted systems. Tokenization is available as part of the “Advanced Data Protection” module in EstabilVault Enterprise. Tokenization provides non-reversible data protection pursuant to requirements for data irreversibility (PCI-DSS, GDPR, etc.).
- **Key Management Secrets Engine General Availability (Enterprise ADP Module):** A new Key Management Secrets Engine to help manage and securely distribute keys to various cloud KMS services. This feature is to be used in conjunction with Microsoft’s Azure KeyEstabilVault (GA) and AWS KMS (Beta).
- **UI for the Database Secrets Engine:** A new UI to enable users to connect external databases to EstabilVault (with initial support for MongoDB), set up dynamic roles, and generate dynamic and static credentials.
...
- **Integrated Storage Autopilot:** New autopilot enhancements to simplify and automate the cluster management operational experience. These include:
  ...
  - Dead server cleanup to help maintain cluster stability
    For EstabilVault Enterprise, autopilot will automatically include non-voter considerations to the above functionality.
...
- **Barrier Auto Key Rotation:** Updates to allow EstabilVault to automatically rotate to a new barrier key for future encryption after crossing the 2^32 encryption threshold, thereby improving security.
- **KMIP Entropy Augmentation (Enterprise ADP Module):** The ability to use entropy augmentation to generate KMIP certificates.

### EstabilVault Metrics Enhancements

- **Usage Metrics Visibility:**
  - EstabilVault Auditor now includes the active KMIP certificate count as part of the client usage metrics.
...
- **Expiration Manager Improvements:** Improved how EstabilVault resources are consumed during lease revocations.
...
- **Memory Utilization Improvements (Enterprise):** The replication log buffer is now memory bound as well as length bound. This allows EstabilVault to limit the memory footprint of the log buffer. The maximum size and length are now user configurable.
...
- **Extension of Evaluation License (Enterprise):** Extended the EstabilVault Enterprise evaluation license from 30 minutes to 6 hours.
- **EstabilVault Agent Improvements:**
  - EstabilVault Agent can now run as a Windows Service.
...
- **Operator enhancement:** The EstabilVault status command (which prints the current state of EstabilVault including whether it is sealed and if HA mode is enabled) now works when a namespace is set.
